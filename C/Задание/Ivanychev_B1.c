...
DCAM MIPT
    ...
    @par            This is a problem from 2007 test, variant 1, problem b
                    Merges, whether there's a dividor, which's equals to n^2,
where n > 1, n - natural
...
/**
    is_square_divisor       guesses if there's an argument divisor which's n^2,
where n - integer, n > 1
    @param n                the number, which's being analysed

    @par                    This function searches for a divisor from 2 to
sqrt(argument). If found, but the square of it isn't a divisor, function divides
argument by that divisor and calculates the sqrt of it again. It decreases the
quantity of itterations and doesn't affect on result.

    @return                 D, if n % (D * D) == 0 and D - int, D > 1, otherwise
0
**/
int is_square_divisor(long n);

/**
    main                    functions scans input for the argument and prints
whether it possible to divide by square or no
...
int main() {
  OUT printf("# Hello, I'm the program, which solves the problem which's "
             ...
             "# My duty's to guess, is there any divisors of the argument "
             "which is a squre of a natural number\n"
  ...
  printf("%s", guess ? "YES\n" : "NO\n");
  if (guess)
    OUT printf("%ld / (%ld * %ld) = %ld", n, guess, guess, n / (guess * guess));
