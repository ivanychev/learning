...
/**
    ...
    @par                    The sum is obvious to be very slow-calculating, if
we didn't use some verifications There's a #define called IS_BELOW_ACCURACY. If
the summand is hardly below precision, the cycle will break.
...
/**
    main                    function which rules input and output and sets the
precision
    @return                 0 if the input is correct, otherwise -1
**/

#define CORRECT_INPUT_                                                         \
  ((ret != 2) || (!((1 <= n) && (n <= 100000))) || (!((0 <= x) && (x <= 10))))
int main() {
  OUT printf("# Hello, I'm the program, which solves the problem which's "
             ...
             "# You're expected to write integer(N, N > 0, N <= 100000) number "
             "and real number (X, X > 0, X <= 10)\n"
             "# I'll calculate S(N,X) = sum (K from 0 to N, X^K / K!) with the "
             "precision of 6 by default\n"
             "# _ELC #define is turned on and you can see me, you're expected "
             "to write your own precison\n"
  ...
  OUT {
    printf(
        "# Would you like to set your own precision (6 by default) (y/n)?\n");
