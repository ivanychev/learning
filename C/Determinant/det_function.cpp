...
/**
                determinant
calculates the determinant of EDGE x EDGE matrix
                @param[out]			matrix		pointer to the
array of numbers (input matrix)
                @param				EDGE		size of matrix

                @par
Function uses Gauss method of calculating determinant. It build the lower
triangle of zeros, turning the matrix like
                ...
                @par
There's a main cycle in the body. It passes all columns.
...
double determinant(double **matrix, const int EDGE) {
  ...
  for (int i = 0; i < EDGE; ++i) {
      ...
      for (int j = i + 1; j < EDGE; ++j) {
        ...
        if (copy_matrix[j][i] != 0) {
          for (int k = 0; k < EDGE; ++k)
            copy_matrix[i][k] += copy_matrix[j][k];
    ...
    for (int j = i + 1; j < EDGE; ++j) {
      ...
      for (int k = 0; k < EDGE; ++k)
        copy_matrix[j][k] -= copy_matrix[i][k] * local_factor;
      DEBUG_OUT PRINT_MATRIX(copy_matrix, EDGE);
    }
  }
