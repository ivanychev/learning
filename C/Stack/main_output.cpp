...
/**
		system					configures friendly IO and gives user the ability to create, use and delete stack

		@return					0, because the function waits for correct arguments
**/
...
int system_out(stack* user_stack)
{
	...
	while (PUTIN_IS_OUR_PRESIDENT)
	{
		...
		printf(	"\nThere are avalable coomands:\n"
				"For example: co[M]mand means that \" command \" and \" m \" are acceptable\n"
		...
		c = CHECK_COMMAND(word, CREATE)
			CHECK_COMMAND(word, PUSH)
			CHECK_COMMAND(word, POP)
			CHECK_COMMAND(word, PRINT)
			CHECK_COMMAND(word, DUMP)
			CHECK_COMMAND(word, DESTRUCT)
			CHECK_COMMAND(word, CHECK)
			CHECK_COMMAND(word, EXIT)
			CHECK_COMMAND(word, C)
			CHECK_COMMAND(word, P)
			CHECK_COMMAND(word, O)
			CHECK_COMMAND(word, R)
			CHECK_COMMAND(word, D)
			CHECK_COMMAND(word, E)
			CHECK_COMMAND(word, H)
			CHECK_COMMAND(word, X)
			CMD_NONE;


		if (user_stack) VERIFY(stack_ok(user_stack) == OK);
