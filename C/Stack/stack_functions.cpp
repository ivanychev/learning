...
void stack_push(stack *user_stack, int input) {
  ASSERT(user_stack);
  VERIFY(user_stack->size >= 0);
  VERIFY(user_stack->size <
         user_stack->maxsize); // what means that size < maxsize
...
int stack_print(stack *user_stack) {
  ...
  for (int i = user_stack->size - 1; i >= 0; --i) {
    ...
    if (fprintf(stdout, "%d", user_stack->data[i]) < 0)
      return ERR_PRINT;
    putchar('\n');
  }
  return OK;
...
int dump_print(stack *user_stack) {
  ...
  fprintf(stdout,
          ...
          "Maxsize = %d\n"
          "Size = %d\n",
          user_stack, stack_ok(user_stack) ? "OK" : "BAD!", user_stack->maxsize,
          user_stack->size);
