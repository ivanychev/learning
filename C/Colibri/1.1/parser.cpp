...
#define CALL(func)                                                             \
  ...
  if (ret != SNTX_OK)                                                          \
    return ret;
#define TREE_CALL(func)                                                        \
  ret = func;                                                                  \
  if (ret != TREE_OK)                                                          \
    return SNTX_INIT_FAIL;
...
#define CHECK_ARGS_(func)                                                      \
  ...
  VERIFY1(tree_check(my_tree->head->root, &_size) != TREE_CHECK_OK,            \
          SNTX_INVALID_TREE, #func ": %s", sntx_errors[INJURED]);              \
  _size = 0;

#define CHECK_TREE_(func)                                                      \
  int _init_size = 0;                                                          \
  int init_check_tree = tree_check(my_tree->head->root, &_init_size);          \
  VERIFY(init_check_tree != TREE_CHECK_OK, SNTX_INVALID_TREE,                  \
         #func "END : Tree injured");
...
int get_pgm(token *code, tree_head **syntax_tree) {
  PRECOND(code == NULL, SNTX_ARG_NULL,
          "GET PGM: Argumented code array is NULL");
  PRECOND(syntax_tree == NULL, SNTX_ARG_NULL,
          "GET PGM: Argumented tree head array is NULL");

  int ret = head_ctr(syntax_tree);
  VERIFY(ret != TREE_OK, SNTX_CTR_FAILED,
         "GET PGM: Tree head construction failed");
  ret = tree_burn((*syntax_tree)->root,
                  TR_PGM); // Maybe save some info about compilation?
  ...
  VERIFY(token_counter == MAXTOKEN, SNTX_NO_END,
         "GET PGM: No end in token array or too long program");
...
int get_b(token **code, tree *my_tree) {
  CHECK_ARGS_(GET B);
  int ret = 0;
  VERIFY(((*code)->type != TKN_SEP) || (*((int *)((*code)->value)) != '{'),
         SNTX_BAD, "GET B: Haven't found '{' in the beginning of block");
  (*code)++;
  tree *cur_tree = NULL;

  TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET B", TR_B, RIGHT));

  while ((((*code)->type != TKN_SEP) && (*((int *)((*code)->value)) != '}')) ||
         ((*code)->type != TKN_END)) {
  ...
  tree *end_tree = NULL;
  TREE_CALL(
      tree_init(&end_tree, HEAD(my_tree), cur_tree, "GET B", TR_NONE, RIGHT));
...
int get_op(token **code, tree *my_tree) {
  CHECK_ARGS_(GET OP);
  VERIFY((*code)->type != TKN_KEY && (*code)->type != TKN_ID, SNTX_BAD_OP_TOKEN,
         "GET OP: Bad token at the beginning of operation");
  int ret = 0;

  tree *cur_tree = NULL;

  // TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET OP", TR_OP,
  // RIGHT));
  ...
  VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET OP: %s",
  ...
  if (type == TKN_ID) {
    ...
    print_token(*code, stdout);
    VERIFY((((*code)->type != TKN_SEP) || (*(int *)((*code)->value) != ';')),
           SNTX_BAD_OP_END, "GET OP: Bad ending of operation");
    (*code)++;
  ...
  ret = CHECK_KEY(if) CHECK_KEY(while) CHECK_KEY(do) CHECK_KEY(print)
      CHECK_KEY(print_val) 0;

  assert(ret != 0);
  print_token(*code, stdout);
  if (ret != SNTX_OK)
    return ret;
  VERIFY((key != WORD_if && key != WORD_while) &&
             (((*code)->type != TKN_SEP) || (*(int *)((*code)->value) != ';')),
         SNTX_BAD_OP_END, "GET OP: Bad ending of operation");
  if (key != WORD_if && key != WORD_while)
    (*code)++;
...
int get_assnop(token **code, tree *my_tree) {
  ...
  VERIFY(
      !((*code)->type == TKN_ID && (*(code) + 1)->type == TKN_ASSGN), SNTX_BAD,
      "GET ASSNOP: Bad assign begin, there's no variable or assign operator");
  int ret = 0;

  tree *cur_tree = NULL;

  // TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET ASSNOP",
  // TR_ASSN, LEFT));

  ret = tree_ctr(&cur_tree, my_tree->head);
  VERIFY1(ret != TREE_OK, SNTX_CTR_FAILED, "GET ASSNOP: %s",
          sntx_errors[CTR_FAIL]);
  ret = tree_burn(cur_tree, TR_ASSN, *(int *)((*code + 1)->value));
  VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET ASSNOP: %s",
          sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
  VERIFY1(ret != TREE_OK, SNTX_ADD_FAILED, "GET ASSNOP: %s",
...
int get_if(token **code, tree *my_tree) {
  CHECK_ARGS_(GET IF);
  int ret = 0;
  VERIFY((((*code)->type != TKN_KEY) || *(int *)((*code)->value) != WORD_if ||
          (*code + 1)->type != TKN_SEP || *(int *)((*code + 1)->value) != '('),
         SNTX_BAD, "GET IF: Invalid IF syntax at the beginning");
  (*code) += 2;

  tree *cur_tree = NULL;

  TREE_CALL(
      tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET IF", TR_IF, LEFT));
...
int get_while(token **code, tree *my_tree) {
  ...
  int ret = 0;
  VERIFY((((*code)->type != TKN_KEY) ||
          *(int *)((*code)->value) != WORD_while ||
          (*code + 1)->type != TKN_SEP || *(int *)((*code + 1)->value) != '('),
         SNTX_BAD, "GET WHILE: Invalid WHILE syntax at the beginning");
  (*code) += 2;

  tree *cur_tree = NULL;

  TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET WHILE", TR_WHILE,
                      LEFT));
...
int get_do(token **code, tree *my_tree) {
  CHECK_ARGS_(GET DO);
  int ret = 0;
  VERIFY((((*code)->type != TKN_KEY) || *(int *)((*code)->value) != WORD_do),
         SNTX_BAD, "GET WHILE: Invalid DO syntax at the beginning");
  (*code)++;
  tree *cur_tree = NULL;

  TREE_CALL(
      tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET DO", TR_DO, LEFT));

  my_tree = cur_tree;

  VERIFY(!(((*code)->type == TKN_SEP) && *(int *)((*code)->value) == '{'),
         SNTX_BAD, "GET DO: Invalid block beginning");
  print_token(*code, stdout);
  CALL(get_b(code, cur_tree));
  print_token(*code, stdout);

  VERIFY((*code)->type != TKN_SEP || *(int *)((*code)->value) != '(', SNTX_BAD,
         "GET DO: Bad beginning of comparison");
  (*code)++;
  print_token(*code, stdout);
  CALL(get_cmp(code, cur_tree));
  print_token(*code, stdout);
  VERIFY((*code)->type != TKN_SEP || *(int *)((*code)->value) != ')', SNTX_BAD,
         "GET DO: Bad ending of comparison");
  (*code)++;
...
int get_print(token **code, tree *my_tree) {
  CHECK_ARGS_(GET DO);
  VERIFY(
      (((*code)->type != TKN_KEY) || *(int *)((*code)->value) != WORD_print ||
       ((*code + 1)->type) != TKN_SEP || *(int *)((*code + 1)->value) != '(' ||
       ((*code + 2)->type) != TKN_STR || ((*code + 3)->type) != TKN_SEP ||
       *(int *)((*code + 3)->value) != ')'),
  ...
  TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET PRINT", TR_PRINT,
                      LEFT));

  ret = tree_ctr(&cur_tree, my_tree->head);
  VERIFY1(ret != TREE_OK, SNTX_CTR_FAILED, "GET PRINT: %s",
          sntx_errors[CTR_FAIL]);
  ret = tree_burn(cur_tree, TR_STR, (char *)((*code + 2)->value));
  VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET PRINT: %s",
          sntx_errors[BURN_FAIL]);
  ret = tree_add_right(my_tree->left, cur_tree);
  VERIFY1(ret != TREE_OK, SNTX_ADD_FAILED, "GET PRINT: %s",
          sntx_errors[ADD_FAIL]);
...
int get_print_val(token **code, tree *my_tree) {
  ...
  TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET PRINT_VAL",
                      TR_PRVAL, LEFT));
  ...
  VERIFY((*code)->type != TKN_SEP || *(int *)((*code)->value) != ')', SNTX_BAD,
         "GET PRINT_VAL: No closing bracket");
  (*code)++;
  //(*code) += 4;
...
/*
                        Now there's another strategy to parse expression. We
   argument expression functions pointers, which are the BEGINNINGS of what they
   are building
...
//	print_token(*code, stdout);
    ...
    my_tree = cur_tree;
    int ret = tree_ctr(&cur_tree, my_tree -> head);
    VERIFY(ret != TREE_OK,	SNTX_CTR_FAILED, "GET E: Tree construction
failed");

    ret = tree_add_right(my_tree, cur_tree);
    VERIFY1(ret != TREE_OK,	SNTX_ADD_FAILED, "GET E: %s",
sntx_errors[ADD_FAIL]);
    ...
    */
    ...
    VERIFY1(ret != TREE_OK, SNTX_BAD,
            "GET E: Node insertion failed, error [%d]", ret);
  ...
  VERIFY(ret != TREE_OK, SNTX_BAD,
         "GET E: Temporary tree hasn't been deleted properly");
...
int get_t(token **code, tree *my_tree, int side) {
  ...
  while (((*code)->type == TKN_OP) &&
    ...
    ret = node_insert(cur_tree->papa, &new_node, side_to_insert, LEFT);
    VERIFY1(ret != TREE_OK, SNTX_BAD,
            "GET T: Node insertion failed, error [%d]", ret);
    ...
    my_tree = cur_tree;
    ret = tree_ctr(&cur_tree, my_tree -> head);
    VERIFY(ret != TREE_OK,	SNTX_CTR_FAILED, "GET T: Tree construction
    failed"); ret = tree_add_right(my_tree, cur_tree); VERIFY1(ret != TREE_OK,
    SNTX_ADD_FAILED, "GET T: %s", sntx_errors[ADD_FAIL]);
  ...
  ret = tree_delete(cur_tree, LEFT);
  VERIFY(ret != TREE_OK, SNTX_BAD,
         "GET T: Temporary tree hasn't been deleted properly");
...
int get_p(token **code, tree *my_tree, int side) {
  ...
  VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET P: %s",
          sntx_errors[BURN_FAIL]);
  if (side == RIGHT)
    ret = tree_add_right(my_tree, cur_tree);
  else
  ...
  // TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree -> left, "GET P",
  // TR_N, LEFT)); //!!!!!!!!!!!!!!!
  ...
  if (side == RIGHT)
    child_side = my_tree->right;
  else
    child_side = my_tree->left;
  ...
  if (((*code)->type == TKN_SEP) && *(int *)((*code)->value) == '(') {
    ...
    VERIFY(((*code)->type != TKN_SEP) || *(int *)((*code)->value) != ')',
           SNTX_BAD, "GET P: No closin bracket found for bracketed expression");
  ...
  if (((*code)->type == TKN_ID) && ((*(code) + 1)->type == TKN_SEP) &&
      (*(int *)((*(code) + 1)->value) == '(')) {
...
int get_v(token **code, tree *my_tree, int side) {
  ...
  VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET V: %s",
          sntx_errors[BURN_FAIL]);
...
int get_f(token **code, tree *my_tree) {
  ...
  for (i = 0; i < FUNC_NUM; ++i)
    if (strcmp((char *)((*code)->value), (math_funcs[i]).name) == 0)
      break;

  VERIFY((i == FUNC_NUM), SNTX_BAD, "GET F: No such function");

  VERIFY2((argc != math_funcs[i].argc), SNTX_BAD,
          "GET F: Invalid number of arguments, waited for [%d], got [%d]",
          math_funcs[i].argc, argc);

  tree *cur_tree = NULL;

  // TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET F", TR_F,
  // RIGHT));

  ret = tree_ctr(&cur_tree, my_tree->head);
  VERIFY(ret != TREE_OK, SNTX_CTR_FAILED, "GET F: Tree construction failed");
  ret = tree_burn(cur_tree, TR_F, (char *)((*code)->value));
  VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET F: %s",
          sntx_errors[BURN_FAIL]);
  ret = tree_add_right(my_tree, cur_tree);
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    // TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET F", TR_NONE,
    // RIGHT));
    ...
    VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET F: %s",
            sntx_errors[BURN_FAIL]);
    ret = tree_add_right(my_tree, cur_tree);
    VERIFY1(ret != TREE_OK, SNTX_ADD_FAILED, "GET F: %s",
            sntx_errors[ADD_FAIL]);
  ...
  VERIFY(ret != TREE_OK, SNTX_BAD,
         "GET F: Temporary tree hasn't been deleted properly");
  print_token(*code, stdout);
  VERIFY(((*code)->type != TKN_SEP) || *(int *)((*code)->value) != ')',
         SNTX_BAD, "GET F: Invalid ending of args brackets, waited for '('");
  (*code)++;
  CHECK_TREE_(GET F);
  return SNTX_OK;
}
...
int get_cmp(token **code, tree *my_tree) {
  ...
  // TREE_CALL(tree_init(&cur_tree, HEAD(my_tree), my_tree, "GET CMP", TR_NONE,
  // LEFT));

  ret = tree_ctr(&cur_tree, my_tree->head);
  VERIFY(ret != TREE_OK, SNTX_CTR_FAILED, "GET CMP: Tree construction failed");
  ret = tree_burn(cur_tree, TR_NONE);
  VERIFY1(ret != TREE_OK, SNTX_BURN_FAILED, "GET CMP: %s",
          sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
  VERIFY1(ret != TREE_OK, SNTX_ADD_FAILED, "GET CMP: %s",
          sntx_errors[ADD_FAIL]);

  CALL(get_e(code, cur_tree, LEFT));
  VERIFY((*code)->type != TKN_COMP, SNTX_BAD,
         "GET CMP: No comparison sign after expression");
