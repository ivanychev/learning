...
int tokenize_stream(FILE *strin, token **token_stream, int *token_count) {
  ...
  VERIFY(i != i_copy, CPLR_TOKENIZE_FAULT,
         "TOKENIZE_STREAM(): i and i_copy don't match, i = %d, i_copy = %d" _ i
             _ i_copy);
  ...
  D_OUT(
      ">>>>>>>>>>>>>>>>>TOKENIZE FINISHED, GOT %d TOKENS<<<<<<<<<<<<<<<<<" _ i);
  *token_count = i;
  printf("[%08x]", (*token_stream));
  return CPLR_OK;
}
...
int token_destruct(token *my_token) {
  ...
  // VERIFY(*my_token == NULL, TOKEN_BAD,  "TOKEN_DESTRUCT(): token pointer is
  // null");
...
int token_write(token *my_token, int type, const void *source) {
  ...
  case TKN_OP:
    c = *((int *)source);
    D_OUT("Got [%c]" _ c);
    VERIFY((c != '+' && c != '-' && c != '*' && c != '/'), TOKEN_BAD,
           "TOKEN_WRITE(): Expected +-//*, got [%c]" _ c);
  ...
  case TKN_COMP:
    ...
    VERIFY((!(c >= CMP_B && c <= CMP_NE)), TOKEN_BAD,
           "TOKEN_WRITE(): got [%d]" _ c);
  ...
  case TKN_ID:
    ...
    VERIFY(str_len > MAXLINE, TOKEN_BAD,
           "TOKEN_WRITE(): got too big identificator \n%50s" _(char *) source);
    D_OUT("Got [%s]" _(char *) source);
    VERIFY(token_word_is_valid((char *)source) == 0, TOKEN_BAD,
           "TOKEN_WRITE(): got invalid identificator only [-_A-Za-z] allowed, "
           "'_' can't be first, got [%s]" _(char *) source);
    VERIFY(token_word_is_key((char *)source) != 0, TOKEN_BAD,
           "TOKEN_WRITE(): inputted identificator is key word, got [%s]" _(
               char *) source);
  ...
  case TKN_SEP:
    ...
    VERIFY(
        (c != ',' && c != ';' && c != '(' && c != ')' && c != '{' && c != '}'),
        TOKEN_BAD, "TOKEN_WRITE(): Expected ;{}(), got [%c]" _ c);
  ...
  case TKN_ASSGN:
    c = *((int *)source);
    D_OUT("Got [%d] code" _ c);
    VERIFY((!(ASSGN <= c && c <= ASSGN_DIV)), TOKEN_BAD,
           "TOKEN_WRITE(): input assign code is wrong [%d]" _ c);
  ...
  case TKN_END:
    c = *((int *)source);
    D_OUT("Got [%d]" _ c);
    VERIFY(c != 0, TOKEN_BAD,
           "TOKEN_WRITE(): strange end [%c]" _(char *) source);
  ...
  case TKN_KEY:
    ...
    D_OUT("Got [%d] code" _ c);
    my_token->type = type;
    VERIFY((!(WORD_if <= c && c <= WORD_print_val)), TOKEN_BAD,
           "TOKEN_WRITE(): input key code is wrong [%d]" _ c);
...
int get_token_got_op(token *save_loc, FILE *strin, int c) {
  ...
  while (isspace(cc = getc(strin)))
    ;
  D_OUT("Checking next symbol, it's [%c]" _ cc);
  if (cc == '=') {
...
int get_token_num(token *save_loc, FILE *strin) {
  ...
  double res = 0;
  D_OUT("Got [%c], result = %lg" _ c _ res);
...
int get_token_got_cmp(token *save_loc, FILE *strin, int c) {
  ...
  while (isspace(cc = getc(strin)))
    ;
  D_OUT("First - [%c], Analysing next symbol, it's [%c]"_ c _ cc)
  if (cc != '=' && c == '!') {
...
int get_token(token *save_loc, FILE *strin) {
  ...
  if (c == '>' || c == '<' || c == '!') {
    D_OUT("It's [%c], that's supposed to be compare operator, calling "
          "got_cmp()..." _ c);
  ...
  if (isalpha(c) || c == '_') {
    ...
    word[i] = '\0';
    ungetc(c, strin);
    D_OUT("Scanned identificator is [%s]" _ word);
    D_OUT("%s" _ word);
