...
void child_sigusr1_handler(int sigid) { OUT("[CHILD]  SIGUSR1 Caught\n"); }
...
void clear_child() { close(global_fileid); }
...
void parent_sigusr1_handler(int signal) { global_last_bit = 0; }

void parent_sigusr2_handler(int signal) { global_last_bit = 1; }

void parent_sigchld_handler(int signal) { global_child_terminated = 1; }
