...
#define F_CHECK_EXIT_CODE return -1;
  ...
  matrix current = {.size = 0, .data = NULL};
...
int proceed_arguments(int argc, char const *argv[], char *name,
                      long *nthreads) {
  CHECK(argc == 3, "Invalid number of arguments. Need name of a file AND "
                   "number of threads\n");
  ...
  *nthreads = 0;
  ...
  CHECK(*nthreads > 0 && *nthreads < SHRT_MAX,
        "Invalid number of expecting threads to be created");
