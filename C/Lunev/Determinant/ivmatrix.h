 ...
 * 		non-zero element below it and swap strings to continue
 * annulating column below [i, i]
 *
 * @param this 	current matrix
...
#define ELEM(matrix_ptr, string, column)                                       \
  *(((ELEM_TYPE *)((matrix_ptr)->data)) + (string) * ((matrix_ptr)->size) +    \
    (column))

#define IS_ZERO(num) (fabs(num) < DBL_EPSILON)
