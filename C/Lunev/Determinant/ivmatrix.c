...
int get_matrix(const char *filename, matrix *this) {
  ...
  CHECK(
      cond == 1,
      "Failed to get matrix size number, that must first in matrix definition");
...
pthread_t *get_threads(const matrix *this, long amount, double *results,
                       struct thread_meta **info_save, int semid) {
...
#define F_CHECK_EXIT_CODE /*semctl(*semid, 0, IPC_RMID);*/                     \
  ...
  struct thread_meta *info =
      (struct thread_meta *)calloc(amount, sizeof(struct thread_meta));
  CHECKN(info, IV_ALLOCFAIL);

#undef F_CHECK_EXIT_CODE
#define F_CHECK_EXIT_CODE                                                      \
  free(info); /*semctl(*semid, 0, IPC_RMID);*/                                 \
  free(array);                                                                 \
  return NULL;
  ...
  for (long i = 0; i < amount; ++i) {
    ...
    cond = pthread_create(&(array[i]), NULL /*&default_attr*/, thread_routine,
                          (void *)(info + i));
...
#define F_CHECK_EXIT_CODE                                                      \
  ...
  return (void *)-1;
    ...
    double delta_result =
        ELEM(&copy, 0, i) * temp_result * ((i % 2 == 0) ? 1.0 : -1.0);
    // printf("%"PRIu32" minor: %lg elem: %lg det: %lg\n", i, delta_result,
    // ELEM(&copy, 0, i), temp_result);
    result += delta_result;
    matrix_kill(minor);
    free(minor);
  }
...
inline void add_string(matrix *this, uint32_t added, uint32_t to,
                       double factor) {
  for (uint32_t i = 0; i < this->size; ++i)
...
#define F_CHECK_EXIT_CODE                                                      \
  ...
  CHECK(j == (this->size - 1) * (this->size - 1),
        "Failed to copy numbers to minor");
