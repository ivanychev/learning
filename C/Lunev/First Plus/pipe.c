...
int is_receiver(int *flag_save) { return is_it(flag_save, RECEIVE_FLAG); }

int is_sender(int *flag_save) { return is_it(flag_save, SENDER_FLAG); }
...
int is_it(int *flag_save, const char *name) {
  ...
  if (fcntl(flag_fifo_id, F_GETPIPE_SZ) != PAGESIZE) {
    ...
    int check_ctl = fcntl(flag_fifo_id, F_SETPIPE_SZ, PAGESIZE);
    OUT1("AFTER:  Pipe size = %d\n", fcntl(flag_fifo_id, F_GETPIPE_SZ));
    CHECK(fcntl(flag_fifo_id, F_GETPIPE_SZ) == PAGESIZE,
          "Unsuccessfully tried to reduce pipe size to page size");
  }
