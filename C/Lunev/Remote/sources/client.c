...
int configure_broadcast() {
  ...
  cond = setsockopt(sk, SOL_SOCKET, SO_BROADCAST, &broadcast_enable,
...
int discover(struct in_addr **array_tosave, int *num_tosave) {
  ...
  cond = sendto(sk, request, strlen(request), 0, (struct sockaddr *)&addr,
                sizeof(addr));
  if (cond == -1) {
  ...
  while ((cond = recvfrom(sk_acc, request, strlen(request), MSG_DONTWAIT,
                          (struct sockaddr *)&rcv_addr, &rcv_addr_len)) != -1) {
    fprintf(stderr, "Recvfrom invoked, cond = %d\n", cond);
    if (ips_got > 0 && ips_got % IP_ARR_STEP == 0) {
      new_arr = realloc(arr, sizeof(struct in_addr) * (ips_got + IP_ARR_STEP));
      if (new_arr == NULL) {
        print_error(CT_ALLOC_FAIL);
        goto fail;
      }
...
ssize_t udp_sender(int sk, struct sockaddr_in *addr, const void *ptr,
                   size_t size) {
  int attempt = 0;
  ssize_t sent = 0;
...
void *talker_tcp_routine(void *arg) {
  ...
  double res = 0;
  ssize_t ret = 0;
  struct sembuf decr = {.sem_num = 0, .sem_op = -1, .sem_flg = 0};
  ...
  while (1) {
    ...
    if (ret != sizeof(task.minor)) {
      fprintf(stderr, "{%d}! ret = %zd, expected = %zd\n", meta->num, ret,
              sizeof(ct_task));
      print_error(NEUTRAL_ERROR);
      goto fail;
    }
    ...
    fprintf(stderr, "Before success semaphores: %d %d\n",
            iv_getsemval(meta->sem, 0), iv_getsemval(meta->sem, 1));
...
fail:
  ...
  fprintf(stderr,
          ...
          "{%d} Connected IP: %s\n"
          "{%d} No tasks lost\n",
          meta->num, meta->num, meta->num, inet_ntoa(addr.sin_addr), meta->num);
  meta->finish_cond = -1;
  if (sk > 0) {
    close(sk);
    sk = 0;
...
int get_det(const matrix *current, double *res_tosave, int ips,
            struct in_addr *ips_array) {
  int cond = 0, failed = 0, i = 0, sem = 0, msgid = 0;
  ...
  for (i = 0; i < ips; ++i) {
    ...
    cond =
        pthread_create(&meta_arr[i].id, NULL, talker_tcp_routine, &meta_arr[i]);
