...
int __rcv_acc(int sk, int line) {
  ...
  while ((cond = recv(sk, &buf, sizeof(buf), MSG_DONTWAIT)) == -1 &&
         (errno == EAGAIN || errno == EWOULDBLOCK) && timeout < MAXTIMEO) {
    sleep(timeout);
    timeout *= 2;
  }
...
int socket_snd_rcv_timeout(int sk, int rcv, int snd) {
  struct timeval rcv_timeout = {.tv_sec = rcv, .tv_usec = 0};
  struct timeval snd_timeout = {.tv_sec = snd, .tv_usec = 0};
...
void thread_sigusr1_handler(int sigid) { pthread_exit(NULL); }
