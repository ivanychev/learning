 ...
 *
 * @details		If the first argument is empty, program works as a
 * receiver, otherwise it sends the argumented file
 *
 * @mainpage
 *
...
#define F_CHECK_EXIT_CODE                                                      \
  last_cleaner(); //! See header for description of F_CHECK_EXIT_CODE
...
int receive() {
  ...
  struct sembuf mutexes[5] = {
      {SND_MUTEX, -1, 0}, {RCV_MUTEX, 1, 0},    {SND_DIED, 0, IPC_NOWAIT},
      {RCV_MUTEX, 0, 0},  {RCV_CONNECT, -1, 0},
  };
...
int snd_protect_connection(int semid) {
  struct sembuf snd_ifdie = {SND_DIED, 1, 0};
  struct sembuf snd_ifdie2 = {SND_DIED, -1, SEM_UNDO};
  int cond = semop(semid, &snd_ifdie, 1);
  CHECK(cond == 0, "Failed to set SND_IFDIE semaphor");
  cond = semop(semid, &snd_ifdie2, 1);
  CHECK(cond == 0, "Failed to set SND_IFDIE semaphor");

  struct sembuf unlock_mutex_ifdead[2] = {{RCV_MUTEX, 1, SEM_UNDO},
                                          {RCV_MUTEX, -1, 0}};
...
int rcv_protect_connection(int semid) {
  ...
  struct sembuf unlock_mutex_ifdead[2] = {{SND_MUTEX, 1, SEM_UNDO},
                                          {SND_MUTEX, -1, 0}};
...
void *get_memptr(const char *filename, size_t size, int *id_to_save) {
  ...
  int need_to_init = 0;
  int shared_id = set_memory(filename, size, &need_to_init);
