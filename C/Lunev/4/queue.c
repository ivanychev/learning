...
int main(int argc, char const *argv[]) {
  ...
  long proc_num = 0;
  CHECK(get_long(&proc_num, argc, argv[1]) == GET_OK,
        "Failed to interpret as number");
...
#define QUEUE_EXISTS
  ...
  CHECK((queue_id = msgget(queue_key, IPC_CREAT | 0600)) != -1,
...
int parent_part(int queue_id, int proc_num) {
  ...
  CHECK(msgsnd(queue_id, &to_send, sizeof(to_send) - sizeof(long), 0) != -1,
        "Failed to send in parent");

  Long_msg received = {};
  int msg_size = msgrcv(queue_id, &received, sizeof(Long_msg) - sizeof(long),
                        proc_num + 1, 0);
  ...
  CHECK(msg_size == (int)(sizeof(Long_msg) - sizeof(long)),
        "Failed to read in child");
...
int child_part(int queue_id, int proc_num, int index) {
  ...
  int msg_size =
      msgrcv(queue_id, &received, sizeof(Long_msg) - sizeof(long), index, 0);
