...
#ifndef DEBUG
  return (checked != NULL && checked->size <= checked->alloc &&
          ((checked->alloc == 0 && checked->begin == NULL) ||
           (checked->alloc != 0 && checked->begin != NULL)) &&
          checked->size <= checked->maxsize &&
          checked->esize <= checked->emaxsize);
#else
  int cond = (checked != NULL && checked->size <= checked->alloc &&
              ((checked->alloc == 0 && checked->begin == NULL) ||
               (checked->alloc != 0 && checked->begin != NULL)) &&
              checked->size <= checked->maxsize &&
              checked->esize <= checked->emaxsize);
...
#ifndef DEBUG
  return (checked != NULL && checked->ptr != NULL &&
          (checked->index < checked->ptr->size ||
           (checked->index == 0 && checked->ptr->size == 0)));
#else
  int cond = (checked != NULL && checked->ptr != NULL &&
              (checked->index < checked->ptr->size ||
               (checked->index == 0 && checked->ptr->size == 0)) &&
...
void __vector_iter_dump(const vector_iter *iter) {
  ...
  fprintf(stderr,
          "Vector iterator [%p] dump: Vector [%p], index[%" PRIu32 "]\n", iter,
          iter->ptr, iter->index);
...
void __vector_dump(const vector *this) {
  ...
  fprintf(stderr,
          ...
          "Vector dump end"
          "\n",
          this, this->alloc, this->size, this->maxsize, this->esize,
          this->emaxsize, this->begin, this->destr);
...
inline void *__elem_ptr(const vector *this, uint32 index) {
  // fprintf(stderr, "this->begin = [%p], index = %"PRIu32", this->esize =
  // %"PRIu32"\n", 	this->begin, index, this->esize);
  return (void *)((unsigned long)(this->begin) + BYTES_IN_ELEMS(this, index));
}

inline void __copy_elem(const vector *this, uint32 index, void *dest) {
  // fprintf(stderr, "this = [%lu], index = %"PRIu32", from = [%lu], dest =
  // [%p]", 	(unsigned long)this, index, (unsigned long)__elem_ptr(this, index),
  // dest);
