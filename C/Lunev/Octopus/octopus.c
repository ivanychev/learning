...
int proceed(int kids_num, tentacle *kids_info) {
  ...
  struct timeval timeout_copy = {.tv_sec = 1, .tv_usec = 0};
  ...
  while (1) {
    ...
    // CHECK(! (cond == 0 && !(TRANSFER_INFO[FINISHED] && TRANSFER_INFO[SCANNED]
    // == TRANSFER_INFO[WRITTEN])), "Timeout expired but file hasn't been
    // transfered");
...
void tentacle_proceed(tentacle_in info) {
  ...
  while (1) {
    ...
    if (cond == 0) {
      ...
      //			OUT4("[KID %d]  SCANNED = %llu, WRITTEN = %llu,
      //FINISHED = %llu\n", info.index, TRANSFER_INFO[SCANNED],
      //TRANSFER_INFO[WRITTEN], TRANSFER_INFO[FINISHED]);
