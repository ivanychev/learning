...
turned to CHECK_FIELD_ (local define)
   ...
   @par        This is a problem from 2007 test, 1st variant, problem D
               Prints a table of b(N, K) values (0 <= N, K <= N), where b(N, K)
- the quantity of binary words of N characters, if there are K ones and there
are no two next ones User is expected to type the integer positive number\n# The
program will calculate and print N + 1 strings.

   @par        This program contains #define _EJC and DEBUG
               _EJC is turned off by default, switching on makes program print
addition information DEBUG, which has been defined, switches on ASSERTs
**/
...
/**
    word_amount     a recursive function which finds the amount of combinations
of binary words, which satisfy the objective
    @param len      - the length of the binary word
    @param ones     - the quantity of ones in the binary word

    @par            Firstly, all of simple cases (len == 0, 1, 2) has been
written using conditional operators. Basically, all cases are possible to be
represented as a combination of simple ones. That's why we use recursion. The
idea is that if there's a word (len > 2), then there are three basical variants
                    -the first char is one
                    -the second is one
                    -both of the first and the second characters are zeros
                    The value of such word_amount(len, ones) = words_amount(len
- 2, ones - 1) + words_amount(len - 3, ones - 1) + words_amount(len - 2, ones).
                    The function is obvious to be lead to the combination of
simple ones.
...
/**
    ...
    @return     main : returns 0, if the argumen is correct, in other cases
returns -1
**/
...
int main() {
  OUT printf("# Hello, I'm the program, which solves the problem which's "
             ...
             "# My duty is to print a table of b(N, K) values (0 <= N, K <= "
             "N), where b(N, K) - the quantity\n"
             "# of binary words of N characters, if there are K ones and there "
             "are no two next ones"
             "\n\n# You are requested to type the integer positive number\n# "
             "The program will calculate and print"
             "N + 1 strings\n"
             "# The n-th string contain b(n, 0) .. (b(n, k) where b(n, k) > 0 "
             "and b(n, k + 1) = 0\n"
