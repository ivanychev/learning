...
int tree_burn(tree* my_tree, int type, ...)
{
	...
	PRECOND((my_tree -> type != TR_NONE)||(my_tree -> value != NULL), TREE_BAD, "TREE UNIT: tre has been already initialized");
	int _size = 0;
...
int tree_check(tree* my_tree, int* size) /*and function pointer, that checks value of tree, general*/
{
	...
	if ( !((TR_V <= my_tree -> type) && (my_tree -> type <= TR_NONE)) )  RETURN_BAD(TREE_CHECK_BAD_TYPE, "TREE CHECK: Invalid type of tree, got [%d]" _ my_tree -> type);
...
int tree_get_token(FILE* strin, tree* my_tree, int* type)
{
	PRECOND(tree_check(my_tree -> head -> root, NULL) != TREE_CHECK_OK, TREE_BAD, "GET TOKEN ERROR: PRECONDITION FAILED\n");
	assert(strin   != NULL);
	assert(my_tree != NULL);
	...
	if (is_there_num(strin) > 0)
	{
		...
		my_tree -> value = value;
		*type = NUM;
		POSTCOND(tree_check(my_tree -> head -> root, NULL) != TREE_CHECK_OK, TREE_BAD, "GET TOKEN ERROR: POSTCONDITION FAILED\n");
		return TREE_OK;
	}
	else
	{
...
int tree_read_in_order(FILE* strin, tree* my_tree)
{
	PRECOND(tree_check(my_tree -> head -> root, NULL) != TREE_CHECK_OK, TREE_BAD, "READ ERROR: PRECONDITION FAILED\n");
	...
	if (got_type == NUM)
	{
		...
		POSTCOND(tree_check(my_tree -> head -> root, NULL) != TREE_CHECK_OK, TREE_BAD, "READ ERROR: POSTCONDITION FAILED\n");
		return TREE_OK;
	}
