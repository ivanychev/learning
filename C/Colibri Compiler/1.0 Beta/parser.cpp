...
#define CHECK_ARGS_(func)                                                      \
  ...
  PRECOND(my_tree == NULL, SNTX_ARG_NULL,                                      \
          #func ": %s" _ sntx_errors[TR_NULL]);                                \
  PRECOND(tree_check(my_tree->head->root, &_size) != TREE_CHECK_OK,            \
          SNTX_INVALID_TREE, #func ": %s" _ sntx_errors[INJURED]);             \
  _size = 0;
...
int get_b(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET B: %s" _ sntx_errors[BURN_FAIL]);
...
int get_op(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET OP: %s" _ sntx_errors[BURN_FAIL]);
  tree_add_right(my_tree, cur_tree);
...
int get_assnop(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_CTR_FAILED,
         "GET ASSNOP: %s" _ sntx_errors[CTR_FAIL]);
  ret = tree_burn(cur_tree, TR_ASSN, *(int *)((*code + 1)->value));
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET ASSNOP: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
  VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
         "GET ASSNOP: %s" _ sntx_errors[ADD_FAIL]);
...
int get_if(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET IF: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
...
int get_while(token **code, tree *my_tree) {
  ...
  int ret = tree_ctr(&cur_tree, my_tree->head);
  VERIFY(ret != TREE_OK, SNTX_CTR_FAILED,
         "GET WHILE: %s" _ sntx_errors[CTR_FAIL]);
  ret = tree_burn(cur_tree, TR_WHILE);
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET WHILE: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
  VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
         "GET WHILE: %s" _ sntx_errors[ADD_FAIL]);
  my_tree = cur_tree;
  //-----------------------------------------
...
int get_do(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET DO: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
...
int get_print(token **code, tree *my_tree) {
  ...
  ret = tree_ctr(&cur_tree, my_tree->head);
  VERIFY(ret != TREE_OK, SNTX_CTR_FAILED,
         "GET PRINT: %s" _ sntx_errors[CTR_FAIL]);
  ret = tree_burn(cur_tree, TR_PRINT);
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET PRINT: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
  VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
         "GET PRINT: %s" _ sntx_errors[ADD_FAIL]);
...
int get_print_val(token **code, tree *my_tree) {
  ...
  int ret = tree_ctr(&cur_tree, my_tree->head);
  VERIFY(ret != TREE_OK, SNTX_CTR_FAILED,
         "GET PRINT VAL: %s" _ sntx_errors[CTR_FAIL]);
  ret = tree_burn(cur_tree, TR_PRVAL);
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET PRINT VAL: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
  VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
         "GET PRINT VAL: %s" _ sntx_errors[ADD_FAIL]);

  /*
  ret = tree_ctr(&cur_tree, my_tree -> head);
  VERIFY(ret != TREE_OK,	SNTX_CTR_FAILED, "GET PRINT VAL: %s" _
  sntx_errors[CTR_FAIL]); ret = tree_burn(cur_tree, TR_V, (char*)((*code + 2) ->
  value)); VERIFY(ret != TREE_OK,	SNTX_BURN_FAILED,"GET PRINT VAL: %s" _
  sntx_errors[BURN_FAIL]); ret = tree_add_right(my_tree -> left, cur_tree);
  VERIFY(ret != TREE_OK,	SNTX_ADD_FAILED, "GET PRINT VAL: %s" _
...
int get_e(token **code, tree *my_tree, int side) //
{
  ...
  while (((*code)->type == TKN_OP) &&
    ...
    ret = tree_add_right(my_tree, cur_tree);
    VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
           "GET E: %s" _ sntx_errors[ADD_FAIL]);
  ...
  if (side_sticked == false)
  {
          if (side == RIGHT)
                  {
                          ret = tree_add_right(my_tree, cur_tree);
                          VERIFY(ret != TREE_OK,	SNTX_ADD_FAILED, "GET E:
  %s" _ sntx_errors[ADD_FAIL]);
                  }
                  else
                  {
                          ret = tree_add_left(my_tree, cur_tree);
  ...
  {
          ret = tree_add_right(my_tree, cur_tree);
          VERIFY(ret != TREE_OK,	SNTX_ADD_FAILED, "GET E: %s" _
  sntx_errors[ADD_FAIL]);
  }
  */
...
int get_t(token **code, tree *my_tree) {
  ...
  while (((*code)->type == TKN_OP) &&
    ...
    ret = tree_add_right(my_tree, cur_tree);
    VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
           "GET T: %s" _ sntx_errors[ADD_FAIL]);
  ...
  if (side_sticked == false)
          {
                  ...
                  ret = tree_add_left(my_tree, cur_tree);
                  VERIFY(ret != TREE_OK,	SNTX_ADD_FAILED, "GET T: %s" _
  sntx_errors[ADD_FAIL]);
...
int get_p(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET P: %s" _ sntx_errors[BURN_FAIL]);
...
int get_n(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET N: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_right(my_tree, cur_tree);
...
int get_v(token **code, tree *my_tree, int side) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET V: %s" _ sntx_errors[BURN_FAIL]);
...
int get_f(token **code, tree *my_tree) {
  ...
  VERIFY((argc != math_funcs[i].argc), SNTX_BAD,
         "GET F: Invalid number of arguments, waited for [%d], got [%d]" _
             math_funcs[i]
                 .argc _ argc);
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET F: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_right(my_tree, cur_tree);
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    ret = tree_add_right(my_tree, cur_tree);
    VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
           "GET F: %s" _ sntx_errors[ADD_FAIL]);
...
int get_cmp(token **code, tree *my_tree) {
  ...
  VERIFY(ret != TREE_OK, SNTX_BURN_FAILED,
         "GET CMP: %s" _ sntx_errors[BURN_FAIL]);
  ret = tree_add_left(my_tree, cur_tree);
  VERIFY(ret != TREE_OK, SNTX_ADD_FAILED,
         "GET CMP: %s" _ sntx_errors[ADD_FAIL]);
