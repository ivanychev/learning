...
int tokenize_stream(FILE *strin, token **token_stream, int *token_count) {
  ...
  PRECOND(strin == NULL, CPLR_TOKENIZE_FAULT,
          "TOKENIZE_STREAM(): Argumented file stream is NULL");
  PRECOND(token_stream == NULL, CPLR_TOKENIZE_FAULT,
          "TOKENIZE_STREAM(): argumented token pointer is NULL");

  token *temp_token = (token *)calloc(1, sizeof(token));
  assert(temp_token);

  int ret = token_construct(&temp_token);
  VERIFY(ret != TOKEN_OK, CPLR_TOKENIZE_FAULT,
         "TOKENIZE_STREAM(): Temporary token construction fault");

  ret = get_token(temp_token, strin);
  VERIFY(ret != TOKEN_OK, CPLR_TOKENIZE_FAULT,
         "TOKENIZE_STREAM(): get_token() fail");
  ...
  VERIFY(ret != TOKEN_OK, CPLR_TOKENIZE_FAULT,
         "TOKENIZE_STREAM(): token_destruct() fail");
  ...
  int i_copy =
      i; // Verifying, that we're going to get the same ammount of tokens
  i = 0;
  ...
  VERIFY2(i != i_copy, CPLR_TOKENIZE_FAULT,
          "TOKENIZE_STREAM(): i and i_copy don't match, i = %d, i_copy = %d", i,
          i_copy);
  ...
  D_OUT1(">>>>>>>>>>>>>>>>>TOKENIZE FINISHED, GOT %d TOKENS<<<<<<<<<<<<<<<<<",
         i);
  *token_count = i;
  printf("[%08x]", (*token_stream));
  return CPLR_OK;
...
int token_construct(token **my_token) {
  ...
  VERIFY(my_token == NULL, TOKEN_BAD,
         "TOKEN_CONSTRUCT(): token pointer is NULL");
...
int token_destruct(token *my_token) {
  ...
  VERIFY(my_token == NULL, TOKEN_BAD,
         "TOKEN_DESTRUCT(): argumented pointer is null");

  my_token->type = POISON;
  if (my_token->value != NULL) {
    DBG_FREE fprintf(stdout,
                     "[%08x] token.cpp, token_destruct, my_token -> value\n",
                     my_token->value);
    free(my_token->value);
  }
  my_token->value = NULL;
...
int token_word_is_key(char *word) {
  D_OUT1("%s", word);
  int c = word[0];
  if (c != 'i' && c != 'e' && c != 'w' && c != 'd' && c != 'u' && c != 'p')
    return 0;

  int ret = CHECK_KEY_(word, if) CHECK_KEY_(word, else) CHECK_KEY_(word, while)
      CHECK_KEY_(word, do) CHECK_KEY_(word, until) CHECK_KEY_(word, print)
          CHECK_KEY_(word, print_val) 0;
  return ret;
}
...
int token_write(token *my_token, int type, const void *source) {
  ...
  case TKN_OP:
    c = *((int *)source);
    D_OUT1("Got [%c]", c);
    VERIFY1((c != '+' && c != '-' && c != '*' && c != '/'), TOKEN_BAD,
            "TOKEN_WRITE(): Expected +-//*, got [%c]", c);
  ...
  case TKN_COMP:
    c = *((int *)source);
    D_OUT1("Got [%d] code", c);
    VERIFY1((!(c >= CMP_B && c <= CMP_NE)), TOKEN_BAD,
            "TOKEN_WRITE(): got [%d]", c);
  ...
  case TKN_ID:
    ...
    VERIFY1(str_len > MAXLINE, TOKEN_BAD,
            "TOKEN_WRITE(): got too big identificator \n%50s", (char *)source);
    D_OUT1("Got [%s]", (char *)source);
    VERIFY1(token_word_is_valid((char *)source) == 0, TOKEN_BAD,
            "TOKEN_WRITE(): got invalid identificator only [-_A-Za-z] allowed, "
            "'_' can't be first, got [%s]",
            (char *)source);
    VERIFY1(token_word_is_key((char *)source) != 0, TOKEN_BAD,
            "TOKEN_WRITE(): inputted identificator is key word, got [%s]",
            (char *)source);
  ...
  case TKN_SEP:
    ...
    VERIFY1(
        (c != ',' && c != ';' && c != '(' && c != ')' && c != '{' && c != '}'),
        TOKEN_BAD, "TOKEN_WRITE(): Expected ;{}(), got [%c]", c);
  ...
  case TKN_ASSGN:
    c = *((int *)source);
    D_OUT1("Got [%d] code", c);
    VERIFY1((!(ASSGN <= c && c <= ASSGN_DIV)), TOKEN_BAD,
            "TOKEN_WRITE(): input assign code is wrong [%d]", c);
  ...
  case TKN_STR:
    ...
    VERIFY1(str_len > MAXLINE, TOKEN_BAD,
            "TOKEN_WRITE(): got too big string \n%50s", (char *)source);
    D_OUT1("Got [%s]", (char *)source);
    my_token->type = type;
    my_token->value = calloc(str_len + 2, sizeof(char));
    assert(my_token->value);
  ...
  case TKN_END:
    c = *((int *)source);
    D_OUT1("Got [%d]", c);
    VERIFY1(c != 0, TOKEN_BAD, "TOKEN_WRITE(): strange end [%c]",
            *(char *)source);
  ...
  case TKN_KEY:
    ...
    D_OUT1("Got [%d] code", c);
    my_token->type = type;
    VERIFY1((!(WORD_if <= c && c <= WORD_print_val)), TOKEN_BAD,
            "TOKEN_WRITE(): input key code is wrong [%d]", c);
...
int get_token_got_op(token *save_loc, FILE *strin, int c) {
  ...
  int drop = 0;
  int cc = 0;
  while (isspace(cc = getc(strin)))
    ;
  D_OUT1("Checking next symbol, it's [%c]", cc);
  if (cc == '=') {
    ...
    case '/':
      ...
      break;
    default:
      assert(("Unexpected happened", 0));
    }
  } else {
    ungetc(cc, strin);
    D_OUT("It's not assignition");
...
int get_token_got_equal(token *save_loc, FILE *strin) {
  ...
  while (isspace(cc = getc(strin)))
    ;
  D_OUT("Got '=', checking next symbol...");

  if (cc == '=') {
    int drop = CMP_E;
    D_OUT("Found '==', saving token...");
...
int get_token_got_cmp(token *save_loc, FILE *strin, int c) {
  ...
  while (isspace(cc = getc(strin)))
    ;
  D_OUT2("First - [%c], Analysing next symbol, it's [%c]", c, cc)
  if (cc != '=' && c == '!') {
...
int get_token(token *save_loc, FILE *strin) {
  ...
  // VERIFY(token_construct(save_loc) != TOKEN_OK, TOKEN_BAD, "GET_TOKEN():
  ...
  if (c == '>' || c == '<' || c == '!') {
    D_OUT1("It's [%c], that's supposed to be compare operator, calling "
           "got_cmp()...",
           c);
  ...
  if (c == '\"') {
    char word[MAXLINE] = {};
    D_OUT("Found \", string in front...");
    int i = 0;
    int cc = 0;
    while ((c = getc(strin)) != '"' && c != EOF && c != '\0' &&
           i < MAXLINE - 1) {
      if (c == '\\') {
        cc = getc(strin);
        switch (cc) {
  ...
  if (isalpha(c) || c == '_') {
    ...
    word[i] = '\0';
    ungetc(c, strin);
    D_OUT1("Scanned identificator is [%s]", word);
    D_OUT1("%s", word);
    int is_key = token_word_is_key(word);
    if (is_key == 0)
      return token_write(save_loc, TKN_ID, (void *)(word));
    else
      return token_write(save_loc, TKN_KEY, (void *)(&is_key));
  }
...
int print_token(token *my_token, FILE *strout) {
  ...
  case TKN_OP:
    fprintf(strout, "value: [%c]\n", *((int *)(my_token->value)));
    break;
  case TKN_COMP:
    fprintf(strout, "value: [%d]\n", *((int *)(my_token->value)));
    break;
  case TKN_ID:
    fprintf(strout, "value: [%s]\n", ((char *)(my_token->value)));
    break;
  case TKN_NUM:
    fprintf(strout, "value: [%lg]\n", *((double *)(my_token->value)));
    break;
  case TKN_SEP:
    fprintf(strout, "value: [%c]\n", *((char *)(my_token->value)));
    break;
  ...
  case TKN_KEY:
    ...
    break;
  default:
    assert(0); // VERIFY1(true,		TOKEN_BAD,		"PRINT TOKEN:
               // GOT WRONG TOKEN, TYPE [%d]", my_token -> type);
  }
