...
//!          <mail@txlib.ru> $ $Date: 2013-11-04 03:00:00 +0400 $
//!
...
//!          href=http://txlib.ru> официальном сайте.</a>
//!
...
#define _TX_VERSION                                                            \
  _TX_V_FROM_CVS($VersionInfo:, TXLib.h, 00172a, 102,                          \
                 2013 - 11 - 04 03 : 00 : 00 + 0400,                           \
                 "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR                                                             \
  _TX_A_FROM_CVS($VersionInfo:, TXLib.h, 00172a, 102,                          \
                 2013 - 11 - 04 03 : 00 : 00 + 0400,                           \
                 "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//!          {300, 200}}; txPolygon (star, 5);  // Я кривая звездочка
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//! @table   @tr LR_CREATEDIBSECTION @td Создает DIB (device-indepandent
//! bitmap), удобную для прямого доступа к данным
//!          @tr LR_LOADFROMFILE     @td Загружает из файла (если этот флаг не
//!          указан, то загружает из ресурса)
//!          @tr Остальные флаги загрузки см. на MSDN.com, поиск "LoadImage
//!          function".
//!          @endtable
//!
//! @see     txCreateWindow(), txCreateCompatibleDC(), txLoadImage(),
...
HDC txLoadImage(const char filename[], unsigned imageFlags = IMAGE_BITMAP,
...
//!
//!          // А можно и так:
//!          Win32::TransparentBlt (txDC(), 0, 0, 800, 600, superman, 0, 0, 80,
//!          60, -1); // 10-кратное увеличение
//!          // Познай мощь Win32 GDI, отказавшись от TXLib'а! :) см.
//!          TransparentBlt в MSDN.com
//!
//!          ...
...
bool txTransparentBlt(HDC dest, double xDest, double yDest, double width,
                      double height, HDC src, double xSrc = 0, double ySrc = 0,
...
//! @endcode
//
//                 +--<<< Это текст помощи, который вы уже читали. Ищите дальше!
...
//     +--<<< Это _прототип_ функции, а надо найти ее _определение_. Ищите
//     дальше! Жмите [F3] или "Найти далее"
//     |
//     v
...
void _txTrace(const char file[], int line, const char func[]) {
  ...
  txOutputDebugPrintf("%s - %p %c%c%c%c%c [%c] - %s (%d) %s\n", _TX_VERSION,
                      &_txInitialized, "cC"[_txConsole], "mM"[_txMain],
                      "dD"[_txIsDll], "rR"[_txRunning], "eE"[_txExit], mark,
...
unsigned volatile _txCanvas_ThreadId =
    0; // ID потока, владеющего окном холста TXLib
...
int volatile _txCanvas_RefreshLock =
...
bool volatile _txConsole_IsBlinking =
...
WNDPROC volatile _txAltWndProc =
    NULL; // Альтернативная оконная функция. См. txSetWindowsHook().

const char *volatile _txFile =
    NULL; // Current execution point tracking, see $ macro
...
int _txInitialize() {
  _TX_ON_DEBUG(OutputDebugString("\n"); OutputDebugString(
                   _TX_VERSION " - The Dumb Artist Library, " _TX_AUTHOR
                               ...
                               ", " _TX_BUILDMODE " mode, module: " _TX_MODULE
                               "\n");
               OutputDebugString("\n"));
...
void _txCleanup() {
  ...
  $ if ((canvas ? _txMain : _txConsole && !waitableParent) && !_txExit &&
    ...
    $ if (console) { $ _txPauseBeforeTermination(canvas); }
  }
...
void _txPauseBeforeTermination(HWND canvas) {
  ...
  $ for (int i = 1;; i++) {
    ...
    if (!Win32::GetConsoleWindow()) {
      $ break;
    } // Console was destroyed
...
LRESULT CALLBACK _txCanvas_WndProc(HWND wnd, UINT msg, WPARAM wpar,
  ...
  case WM_CLOSE:
    $ if (_txCanvas_OnCLOSE(wnd)) break;
    else return 0;
  case WM_DESTROY:
...
bool _txCanvas_OnDESTROY(HWND wnd) {
  ...
  $ if (dcs.size()) {
    ...
    $ for (int i = dcs.size() - 1; i >= 0; i--) _txBuffer_Delete(&dcs[i]);
    $ dcs.clear();
  }
...
#define SZARG_(n) sizeof(what) - 1 - (n) - (s - what) _TX_TRUNCATE
  ...
  s += _snprintf_s(
      s, SZARG_(1),
      "#%d: %s, Instance: 0x%p, Flags: %c%c%c%c%c, Thread: 0x%08X%s", nCalls,
      _TX_VERSION, &_txInitialized, "cC"[_txConsole], "mM"[_txMain],
      "dD"[_txIsDll], "rR"[_txRunning], "eE"[_txExit], threadId,
...
#undef SZARG_
  ...
  $ txOutputDebugPrintf("%s - %s", _TX_VERSION,
                        tools::compressSpaces(str, what));
...
HDC txLoadImage(const char filename[], unsigned imageFlags /*= IMAGE_BITMAP*/,
  ...
  $ HBITMAP image = (HBITMAP)Win32::LoadImage(
      (loadFlags & LR_LOADFROMFILE) ? NULL : GetModuleHandle(NULL), filename,
      imageFlags, 0, 0, loadFlags);
...
bool txBitBlt(HDC dest, double xDest, double yDest, double width, double height,
              HDC src, double xSrc /*= 0*/, double ySrc /*= 0*/,
              DWORD rOp /*= SRCCOPY*/) {
  $1 _TX_IF_TXWINDOW_FAILED return false;
...
bool txTransparentBlt(HDC dest, double xDest, double yDest, double width,
                      double height, HDC src, double xSrc /*= 0*/,
                      double ySrc /*= 0*/, COLORREF transColor /*= TX_BLACK*/) {
  $1 _TX_IF_TXWINDOW_FAILED return false;
  $ _TX_IF_ARGUMENT_FAILED(dest) return false;
  $ _TX_IF_ARGUMENT_FAILED(src) return false;
...
bool txAlphaBlend(HDC dest, double xDest, double yDest, double width,
                  double height, HDC src, double xSrc /*= 0*/,
                  double ySrc /*= 0*/, double alpha /*= 1.0*/) {
  $1 _TX_IF_TXWINDOW_FAILED return false;
  $ _TX_IF_ARGUMENT_FAILED(dest) return false;
  $ _TX_IF_ARGUMENT_FAILED(src) return false;
