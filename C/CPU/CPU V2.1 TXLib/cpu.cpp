...
/**
                cpu_pop
pops one element from cpu stack, saves it to the input pointer
...
/**
                cpu_mox_ax
function saves the value argument to the ax cpu register
...
/**
                cpu_space
function prints SPACE charecter to the output
                @param[out]			stream		output file
stream
                @param[out]			my_cpu		pointer of cpu
...
/**
                cpu_enter
function prints ENTER charecter to the output
                @param[out]			stream		output file
stream
                @param[out]			my_cpu		pointer of cpu
...
/**
                cpu_tab
function prints TAB charecter to the output
                @param[out]			stream		output file
stream
                @param[out]			my_cpu		pointer of cpu
...
Beta 2) out rg ax   -   printing the value of ax register
...
int cpu_out(FILE *fi, FILE *fo, FILE *strerr,
            cpu *my_cpu) // переделать pop ax, посоображать с выводом (может
                         // аргумент к out?)
{
  if (cpu_check(my_cpu) != CPU_CHECK_OK)
    return CPU_BROKEN_START;
  if (fi == NULL)
    return CPU_OUT_STREAM_IN_NULL;
  if (fo == NULL)
  ...
  if (strcmp(type, "rg") == 0) {
    ...
    fscanf(fi, "%s", reg);
    _strupr(reg);
    int reg_number = CHECK_REG_(reg, AX) CHECK_REG_(reg, BX) CHECK_REG_(reg, CX)
        CHECK_REG_(reg, DX) REG_NONE;
    switch (reg_number) {
    case REG_AX:
