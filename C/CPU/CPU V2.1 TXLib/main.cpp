/**
	...
	@par				Available commands (1.1 Beta 2) :
						- push {float number} or {ax}	- pushes number to stack
						- pop							- pops number and saves it to the argument pointer of that functions
						- add
						- sub
						- mul
						- div
						...
						- tan
						- out:
						-- out rg {name of register: ax, bx, cx, dx}
						-- out st    -  from stack
	...
	@todo				Enchance cpu_pop() to read pop ax, pop out constructions
	@todo				Add cpu_swap(), cpu_dub()
	@todo				Extend push to make push bx, push cx, push dx possible
**/
...
int main()
{
	...
	ret = plot("code.txt", "values.txt", "log.txt", my_cpu, X1, X2, DX, ymin, ymax, DISPLAY_WIDTH, DISPLAY_HEIGHT);
