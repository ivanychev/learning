...
//{          [Best viewed with "Fold all on file open" option enabled] [best
//screen width is 115 chars]
//=================================================================================================================
//!
//! @file    TXLib.h
//! @brief   Библиотека Тупого Художника (The Dumb Artist Library, TX Library,
//! TXLib).
//!
//!          $Version: 00172a, Revision: 101 $
//!          $Copyright: (C) Ded (Ilya Dedinsky, http://txlib.ru)
//!          <mail@txlib.ru> $ $Date: 2013-04-17 00:54:44 +0400 $
//!
//!          TX Library - компактная библиотека двумерной графики для Win32 на
//!          С++. Это небольшая "песочница" для начинающих реализована с целью
//!          помочь им в изучении простейших принципов программирования.
//!          Документация на русском языке.
//!
//!          Философия TX Library - облегчить первые шаги в программировании и
//!          подтолкнуть к творчеству и самостоятельности.
//!
//!          TX Library is a tiny 2D graphics library for Win32 written in C++.
//!          This is a small sandbox for the very beginners to help them to
//!          learn basic programming principles. The documentation is currently
//!          in Russian.
//!
//!          Официальный сайт библиотеки: <a href=http://txlib.ru>txlib.ru.</a>
//!
//!          См. также <a href=http://txlib.sourceforge.net>страницу проекта на
//!          SourceForge.</a>
//!
//! @warning <b>Это альфа-версия. Для использования требуется согласование с
//! автором библиотеки.</b> @nn
//!          Правила использования материалов библиотеки и сайта см. на <a
//!          href=http://txlib.ru/index/0-6> официальном сайте.</a>
//!
//! @par     Трекеры на SourceForge:
//!        - <a
//!        href=https://sourceforge.net/tracker/?func=add&group_id=213688&atid=1026710>
//!        <b>Сообщить об ошибке  </b></a>
//!        - <a
//!        href=https://sourceforge.net/tracker/?func=add&group_id=213688&atid=1026713>
...
//! @cond INTERNAL
#define _TX_V_FROM_CVS(_1, file, ver, rev, date, auth, _2)                     \
  "TXLib [Ver: " #ver ", Rev: " #rev "]"
...
//!            (пример) #define _TX_AUTHOR  "Copyright (C) Ded (Ilya Dedinsky,
//!            http://txlib.ru)"
//! @endcode
...
#define _TX_VERSION                                                            \
  _TX_V_FROM_CVS($VersionInfo:, TXLib.h, 00172a, 101,                          \
                 2013 - 04 - 17 00 : 54 : 44 + 0400,                           \
                 "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR                                                             \
  _TX_A_FROM_CVS($VersionInfo:, TXLib.h, 00172a, 101,                          \
                 2013 - 04 - 17 00 : 54 : 44 + 0400,                           \
                 "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//!
//!            Формат: старшее слово - номер версии, младшее - номер ревизии, в
//!            двоично-десятичном формате. Например, @c 0x172a0050 - версия @c
//!            0.172a, ревизия @c 50.
//!
...
#if defined(_WINDOWS_H) || defined(_INC_WINDOWS) || defined(_WINDOWS_) ||      \
    defined(__WINDOWS__)
...
#if defined(_STRING_H_) || defined(_INC_STRING) || defined(_STDIO_H_) ||       \
    defined(_INC_STDIO)
...
#pragma GCC diagnostic ignored                                                 \
    "-Wunused-label" // Just for fun in _txCanvas_OnCmdAbout()
#pragma GCC diagnostic ignored "-Winline"

#endif

#define _TX_CHECK_FORMAT(fmtIdx)                                               \
  __attribute__((format(printf, (fmtIdx), (fmtIdx) + 1)))

template <typename T> inline T _txNOP(T value) {
  return value;
} // To suppress performance warnings in assert etc
...
#pragma warning(disable : 28125) // the function must be called from within a
                                 // try/except block
...
#define ctime_s(buf, sizeof_buf, time)                                         \
  (strncpy((buf), ctime(time), (sizeof_buf)-1))
#define getenv_s(sz, buf, sizeof_buf, name)                                    \
  ((void)(sz), strncpy((buf), getenv(name), (sizeof_buf)-1))
...
#pragma warning(disable : 304) // remark: access control not specified ("public"
                               // by default)
#pragma warning(disable : 522) // remark: function "..." redeclared "inline"
                               // after being called
#pragma warning(                                                               \
        disable : 981) // remark: operands are evaluated in unspecified order
#pragma warning(                                                               \
...
#ifdef _MSC_VER_6
#pragma warning(                                                               \
        push, 3) // MSVC 6: At level 4, some std headers emit warnings O_o
#endif
...
#if defined(_GCC_VER) && (_GCC_VER >= 400)
#include <cxxabi.h>
#endif
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Technical
//! @brief   Анонимное пространство имен для защиты от конфликтов при сборке
...
//!
//! @return  Дескриптор (системный номер) окна TXLib. Если окно не создано,
//! возвращается NULL.
//!
//! @note    Устанавливаются параметры рисования по умолчанию, см. функцию
//! txSetDefaults().
//!
//! @warning Если используется многофайловый проект (с раздельной компиляцией),
//! то графические функции TXLib,
//!          вызванные из файла проекта, будут работать только с тем окном,
//!          которое создано <b>в этом же файле</b> проекта. Если проект
//!          состоит, скажем, из файлов @c main.cpp и @c game.cpp, и в файле @c
//!          main.cpp создается графическое окно, то функции из @c game.cpp не
//!          смогут рисовать в нем. (Однако @c game.cpp сможет создать свое
//!          собственное окно.) @n Если такой программе нужно одно окно, то в
//!          проект следует включить файл, ответственный за рисование, скажем,
//!          @c graphics.cpp, и выводить графику только через функции этого
//!          файла. Такой файл (или библиотеку) в больших проектах часто
//!          называют графическим движком. @nn То же касается и использования
//!          TXLib в @b DLL.
//!
//! @note    Вспомогательные окна могут создаваться по одному на каждый файл
//! многофайлового проекта или загруженную
//!          DLL. Для закрытия вспомогательных окон используется
//!          txDestroyWindow(). Для закрытия главного надо выйти из main().
//!
//! @warning Одновременное создание нескольких окон не потокобезопасно (not
//! thread-safe). @nn
//!          Многооконная програма на TXLib тормозит, да и однооконная тоже не
//!          отличается высокой скоростью. Чтобы избавиться от этого, бросьте
//!          TXLib и используйте другие оконные библиотеки (<a
//!          href=http://qt-project.org>Qt</a>, <a
//!          href=http://wxwidgets.org>wxWidgets</a>, <a
//!          href=http://gtk.org>GTK+</a> и т.д., см. также <a
//!          href=http://libsdl.org> SDL</a>, <a
//!          href=http://opengl.org>OpenGL</a> и т.п.) или напишите свою.
//!          Помните, что цель TXLib - облегчить первые шаги, но потом стать
//!          ненужной.
//!
//! @see     txOK(), txWindow(), txDC(), _txWindowStyle, _txConsoleMode,
//! _txConsoleFont, _txCursorBlinkInterval,
//!          _txWindowUpdateInterval, _TX_NOINIT, _TX_ALLOW_TRACE, TX_TRACE
//!
//! @usage @code
//!          txCreateWindow ( 800, 600);         // Окно  800х600, центрировано
//!          txCreateWindow (1024, 768, false);  // Окно 1024х768, не
//!          центрировано
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//! txColors, RGB(), txSelectFont(), txSetROP2()
//!
//! @usage @code
//!          txSetDefaults();
//! @endcode
...
//!
//! @return  Состояние библиотеки: true - библиотека в порядке, false - не в
//! порядке.
//!
//!          "Библиотека не в порядке" означает, что ее внутренние данные
//!          неверны. Самая простая причина - не открыто окно, однако могут быть
//!          и другие проблемы.
//!
//! @see     txCreateWindow()
//!
...
//!          if (!txOK())
//!              {
//!              txMessageBox ("Не смогла создать окно", "Извините",
//!              MB_ICONSTOP); return;
//!              }
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @return  Размер окна рисования в виде структуры POINT, содержащей компоненты
//! @c x и @c y.
//!
...
//!          txTextOut (txGetExtentX() / 2, 100, "Oh, oh, you're in the
//!          [army]middle now");
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @return  Дескриптор (системный номер, handler) контекста рисования (device
//! context, DC) холста (HDC).
//!
//! @note    HDC возвращается в виде ссылки, что позволяет подменить его. Перед
//! подменой надо сохранить старый
//!          дескриптор или освободить его с помощью txDeleteDC(). До подмены
//!          рисование должно быть заблокировано с помощью txLock() и после
//!          подмены разблокировано с помощью txUnlock().
//!
...
//!
//! @usage @code
//!          SetWindowText (txWindow(), "Новые заголовки - теперь и в ваших
//!          окнах!"); txMessageBox ("Распишитесь", "Получите",
//!          MB_ICONINFORMATION);
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Technical
//! @brief   Возвращает имя исполняемого файла или изначальный заголовок окна
//! TXLib.
//!
//! @param   fileNameOnly  Возвратить только полное имя исполняемого файла,
//! полученного через Win32 функцию
...
//!              {
//!              char title [1024] = "";
//!              sprintf (title, "%s - [%-10.*s] %d%%", txGetModuleFileName
//!              (false),
//!                       done/10, "||||||||||", done);
//!
...
//!
//!          Если кому-то эти цвета не нравятся (что неудивительно), всегда
//!          можно сделать свои с помощью RGB(). См. пример ниже.
//!
...
//!
//!          txSetFillColor (MY_DEEP_ROMANTIC_BLUE);  // А.И. Куинджи, "Лунная
//!          ночь на Днепре" txSetColor     (MY_SHINING_MOONLIGHT);   //
//!          http://tanais.info/art/pic/kuindzhi1.html
//! @endcode
//!
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------
...
const COLORREF
    ...
    enum txColors {
      ...
      TX_BROWN = RGB(128, 128, 0),   //!< Коричневый цвет. <i>Некрасивый. Do it
                                     //!< yourself with RGB().</i>
      ...
      TX_LIGHTRED = RGB(
          255, 0,
          128), //!< Светло-красный цвет.   <i>Не самого лучшего оттенка.</i>
      TX_LIGHTMAGENTA = RGB(
          255, 0,
          255), //!< Светло-малиновый цвет. <i>Еще менее лучшего оттенка.</i>
      TX_PINK = RGB(255, 128, 255),   //!< Розовый гламурный :)
      TX_YELLOW = RGB(255, 255, 128), //!< Желтый цвет.
      TX_WHITE = RGB(255, 255, 255),  //!< Белый цвет.
      TX_TRANSPARENT =
          0xFFFFFFFF, //!< Прозрачный цвет. <i>Отключает рисование.</i>
      TX_NULL =
          TX_TRANSPARENT, //!< Прозрачный цвет. <i>Отключает рисование.</i>

      //  Цветовые каналы (компоненты) - см. txExtractColor(), txRGB2HSL(),
      //  txHSL2RGB()
...
//!
//! @see     txSetColor(), txGetColor(), txSetFillColor(), txGetFillColor(),
//! txColors, RGB(), txExtractColor(),
//!          txRGB2HSL(), txHSL2RGB()
//!
//! @usage @code
//!          txSetColor (RGB (255, 128, 0));                // Красный +
//!          половина зеленого = оранжевый
//!
//!          int red = 20, green = 200, blue = 20;
//!          COLORREF color = RGB (red, green, blue);
//!          txSetFillColor (color);
//!
//!          const COLORREF SKY_COLOR = RGB (0, 128, 255);  // Создаем константу
//!          для нового цвета
//!
//!          ...
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//! @see     txColor(), txGetColor(), txFillColor(), txGetFillColor(), txColors,
//! RGB()
//!
//! @usage @code
//!          txSetColor (TX_RED);
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//! @see     txFillColor(), txGetFillColor(), txColor(), txGetColor(), txColors,
//! RGB()
//!
//! @usage @code
...
//!
//!          При рисовании графическая библиотека может не просто красить
//!          пиксели на экране, а смешивать цвета экрана и текущие цвета линий и
//!          заполнения.
//!
...
//! @table   @tr R2_COPYPEN     @td Пиксели = цвета кисти (самый нормальный
//! режим :)
//!          @tr R2_NOTCOPYPEN  @td Пиксели = ~кисть
//!          @tbr
//!          @tr R2_BLACK       @td Пиксели = черный цвет (цвет кисти
//!          игнорируется)
//!          @tr R2_WHITE       @td Пиксели = белый  цвет (цвет кисти
//!          игнорируется)
//!          @tr R2_NOT         @td Пиксели = ~пиксели    (цвет кисти
//!          игнорируется)
//!          @tbr
//!          @tr R2_XORPEN      @td Пиксели =    пиксели ^  кисть (удобный
//!          режим, cм. пример ниже)
...
//! @see     txSetColor(), txGetColor(), txSetFillColor(), txGetFillColor(),
//! txColors, RGB(),
//!          txLine(), txRectangle(), txPolygon(), txEllipse(), txCircle(),
//!          txArc(), txPie(), txChord()
//!
//! @usage @code
//!          txSetColor (TX_WHITE, 5);       // При рисовании белым цветом в
//!          режиме txSetROP2 (R2_XORPEN);          //   R2_XORPEN цвета на
//!          экране инвертируются
//!
//!          txLine (100, 100, 200, 200);    // Рисуем первый раз - линия
//!          появляется txSleep (1000); txLine (100, 100, 200, 200);    //
//!          Рисуем второй раз - линия исчезает (немного уличной магии)
//!
...
//!
//!          Другие примеры см. в функциях AppearText(), AppearEarth() Примера 5
//!          ("Циклы").
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//!          Цвет и толщина линий задается функцией txSetColor(), цвет
//!          заполнения - txSetFillColor().
//!
//! @see     txSetColor(), txGetColor(), txSetFillColor(), txGetFillColor(),
//! txColors, RGB(),
//!          txLine(), txRectangle(), txPolygon(), txEllipse(), txCircle(),
...
//!
//!          Win32::RoundRect (txDC(), 100, 200, 400, 500, 30, 30);  // И такое
//!          есть. См. RoundRect в MSDN.com
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @usage @code
//!          POINT star[5] = {{150, 300}, {200, 100}, {250, 300}, {100, 200},
//!          {300, 200}}; txPolygon (star, 5);
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//! @brief   Рисует эллипс.
//!
//! @param   x0  X-координата верхнего левого угла прямоугольника, описанного
//! вокруг эллипса
...
//! описанного вокруг эллипса, содержащего дугу (см. txEllipse)
...
//! @param   startAngle  Угол между направлением оси OX и началом дуги (в
//! градусах)
...
bool txArc(double x0, double y0, double x1, double y1, double startAngle,
           double totalAngle);
...
//! @param   x1          X-координата нижнего правого угла прямоугольника
//! @param   y1          Y-координата нижнего правого угла прямоугольника
//! @param   startAngle  Угол между направлением оси OX и началом сектора (в
//! градусах)
...
bool txPie(double x0, double y0, double x1, double y1, double startAngle,
           double totalAngle);
...
//! @param   x1          X-координата нижнего правого угла прямоугольника
//! @param   y1          Y-координата нижнего правого угла прямоугольника
//! @param   startAngle  Угол между направлением оси OX и началом хорды (в
//! градусах)
...
bool txChord(double x0, double y0, double x1, double y1, double startAngle,
             double totalAngle);
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//!          Цвет заполнения задается функцией txSetFillColor(). Не
//!          рекомендуется для применения - работает довольно медленно.
//!
//! @title   Режимы заливки: @table
//!          @tr FLOODFILLSURFACE @td - Заливать область,    указанную цветом
//!          color.
//!          @tr FLOODFILLBORDER  @td - Заливать до границы, указанной цветом
//!          color.
//!          @endtable
//!
...
bool txFloodFill(double x, double y, COLORREF color = TX_TRANSPARENT,
...
bool txTriangle(double x1, double y1, double x2, double y2, double x3,
                double y3) {
  (void)x1;
  (void)y1;
  (void)x2;
  ...
  MessageBox(txWindow(),
             "Эта функция не реализована в библиотеке, потому что вы сами "
             "легко можете реализовать ее\n"
             "как функцию с параметрами, используя txPolygon(). См. \"Пример с "
...
//! @param   wink   Моргание глаз (0 - оба открыты, -1 - закрыт левый, +1 -
//! закрыт правый)
...
//!        |handL|handR|twist|head|eyes|wink|crazy|smile|hair|wind
//!        //-----------+---+----+-----+-----+----------+-----+-----+-----+----+----+----+-----+-----+----+-----
//!        //           |   |    |     |     |          |     |     |     |    |
//!        |    |     |     |    |
//!           txDrawMan (125, 250, 200,  200, TX_WHITE,    0,    0,    0,   0,
//!           0.8,  0,   0,   1.0,   0,  0); txDrawMan (325, 250, 100,  200,
//!           TX_YELLOW,   0,    0,    0,   0,   0.8,  0,   0,  -1.0,   2,  0);
//!           txDrawMan (525, 250, 200,  100, TX_ORANGE,   0,    0,    0,
//!           0,   1.0,  0,  -1,   0.3,   1,  0); txDrawMan (725, 250, 100, 100,
//!           TX_LIGHTRED, 0,    0,    0,   0,   1.0,  0,   1,  -0.3,   3,  0);
//!
//!           txDrawMan (125, 550, 200,  200, TX_WHITE,    0.3,  0.3,  0,   0,
//!           0.8, -1,   1,   0.5,   2, -1); txDrawMan (325, 550, 100,  200,
//!           TX_YELLOW,  -0.5, -0.5,  0,   0.1, 0.8,  1,   0,  -0.5,   3,  5);
//!           txDrawMan (525, 550, 200,  100, TX_ORANGE,  -0.5,  0.3,  0.2, 0,
//!           0.8, -1,   1,   0.0,  10, -5); txDrawMan (725, 550, 100,  100,
//!           TX_LIGHTRED, 0.3, -0.5, -0.4, 0,   0.8,  1,  -1,   0.0,   1,  1);
//! @endcode
//}----------------------------------------------------------------------------------------------------------------

void txDrawMan(int x, int y, int sizeX, int sizeY, COLORREF color, double handL,
               double handR, double twist, double head, double eyes,
               double wink, double crazy, double smile, double hair,
               double wind);
//! @}
//}
//=================================================================================================================
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//!          Цвет текста задается функцией txSetColor(), выравнивание -
//!          txSetTextAlign().
//!
//! @see     txSetColor(), txGetColor(), txSetFillColor(), txGetFillColor(),
//! txColors, RGB(),
//!          txSelectFont(), txSetTextAign(), txGetTextExtent(),
//!          txGetTextExtentX(), txGetTextExtentY()
//!
//! @usage @code
...
//!          txSetTextAlign().
//!
//! @note    Не выводит ничего, если координаты идут в неверном порядке (если x0
//! > x1 или y0 > y1).
//!
//!          Флаги форматирования текста см. в MSDN (http://msdn.com), искать
//!          "DrawText Function (Windows)":
//!          http://msdn.microsoft.com/en-us/library/dd162498%28VS.85%29.aspx.
//!
//!          <b>Автоматический перенос</b> текста на несколько строк включается,
//!          если текст состоит из нескольких строк (есть хотя бы один символ
//!          новой строки @b @c \\n).
//!
//!          Если надо отформатировать текст не по центру, а по левому или
//!          правому краю, то не забудьте указать остальные флаги
//!          форматирования, если они нужны: @c DT_VCENTER (вертикальное
//!          центрирование) @c |
//!          @c DT_WORDBREAK (перенос по словам) @c | @c DT_WORD_ELLIPSIS
//!          (ставить многоточие в конце, если текст не умещается). См. значение
//!          флагов по умолчанию.
//!
//! @see     txSetColor(), txGetColor(), txSetFillColor(), txGetFillColor(),
//! txColors, RGB(),
//!          txTextOut(), txSelectFont(), txGetTextExtent(), txGetTextExtentX(),
//!          txGetTextExtentY()
//!
//! @usage @code
//!          txSetColor     (TX_BLACK);
//!          txSetFillColor (TX_DARKGRAY); Win32::RoundRect (txDC(), 105, 105,
//!          205, 255, 30, 30); txSetFillColor (TX_WHITE);    Win32::RoundRect
//!          (txDC(), 100, 100, 200, 250, 30, 30);
//!
//!          txSelectFont ("Arial", 20, 0, FW_BOLD);
//!          txDrawText  (100, 100, 200, 250, "I hate it when I'm studying "
//!                                           "and a Velociraptor throws bananas
//!                                           on me.\n");
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @return  Всегда true. Если шрифт не был найден, то устанавливается системный
//! шрифт Windows
//!          @c (SYSTEM_FIXED_FONT, см. MSDN). Существование шрифта можно
//!          проверить функцией txFontExist.
//!
...
//!          15); txTextOut (100, 200, "Но ее почему-то нет.");
//! @endcode
//}----------------------------------------------------------------------------------------------------------------

bool txSelectFont(const char name[], double sizeY, double sizeX = -1,
...
//!
//! @see     txTextOut(), txSelectFont(), txGetTextExtent(), txGetTextExtentX(),
//! txGetTextExtentY()
//!
//! @usage @code
...
//!
//! @usage @code
//!          txTextOut (100 + txGetTextExtentX (text) / 2, 200 +
//!          txGetTextExtentY (text) / 2, text);
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!          @tr TA_BOTTOM   @td Точка (X,Y) определяет нижнюю сторону
//!          описанного прямоугольника  (текст лежит выше этой точки).
//!          @tr TA_TOP      @td Точка (X,Y) определяет верхнюю сторону
//!          описанного прямоугольника (текст лежит ниже этой точки).
//!          @tbr
//!          @tr TA_CENTER   @td Текст будет выровнен по горизонтали
//!          относительно точки (X,Y).
//!          @tr TA_LEFT     @td Точка (X,Y) определяет левую сторону описанного
//!          прямоугольника  (текст лежит правее этой точки).
//!          @tr TA_RIGHT    @td Точка (X,Y) определяет правую сторону
//!          описанного прямоугольника (текст лежит левее этой точки).
//!          @endtable
//!
//! @see     txTextOut(), txSelectFont(), txGetTextExtent(), txGetTextExtentX(),
//! txGetTextExtentY()
...
//! @param   name  Название шрифта
//!
//! @return  Информация о шрифте в структуре LOGFONT. Если шрифт не найден,
//! возвращает NULL.
//!
//! @see     txTextOut(), txSelectFont()
//!
//! @usage @code
//!          if (txFontExist ("Comic Sans MS")) txSelectFont ("Comic Sans MS",
//!          30); else                               txSelectFont ("Times New
//!          Roman", 30);
//!
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Drawing
//! @brief   Создает дополнительный холст (контекст рисования, Device Context,
//! DC) в памяти.
//!
...
//!
//! @return  Дескриптор (системный номер, выданный Windows) созданного холста
//! (контекста рисования).
//!
//! @warning Созданный контекст затем будет нужно @b обязательно удалить при
//! помощи txDeleteDC(). @n
//!          <small>When the program will be shutting down, TXLib will try to
//!          delete DCs which were not deleted, but this is not
//!          guaranteed.</small>
//!
//! @see     txCreateWindow(), txCreateCompatibleDC(), txLoadImage(),
//! txDeleteDC()
//!
//! @usage @code
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Drawing
//! @brief   Загружает из файла изображение в формате BMP. Делает это довольно
//! медленно.
//!
//! @param   filename  Имя файла с изображением в формате BMP
//!
//! @return  Дескриптор созданного контекста рисования в памяти, с загруженным
//! изображением.
//!          Если изображение не загружено (не найден файл, неверный формат
//!          файла и т.д.), то NULL.
//!
//! @warning Изображение загружается в создаваемый контекст рисования в памяти
//! ("виртуальный холст"), который затем
//!          будет нужно <b>обязательно удалить</b> при помощи txDeleteDC(). @n
//!          <small>When the program will be shutting down, TXLib will try to
//!          delete DCs which were not deleted, but this is not
//!          guaranteed.</small>
//!
//! @note    Изображения поддерживаются <b>только в формате BMP.</b> Если взять
//! файл другого формата, например JPG,
//!          и переименовать его со сменой расширения на BMP, то от этого формат
//!          не изменится. Такое изображение загружено не будет.
//!
//!          Если функция вернула NULL, то надо прежде всего <b>проверить
//!          наличие файла изображения</b> по указанному в программе пути и
//!          формат файла. Если путь к файлу не указан (или указан как
//!          неполный), то путь отсчитывается от текущей папки программы,
//!          которая может не совпадать текущей папкой среды программирования.
//!          Текущую папку программы можно посмотреть по команде About в
//!          системном меню (она указана там как "Run from").
//!
//! @note    <b>Не надо часто загружать</b> одно и то же изображение, особенно в
//! цикле. От этого программа начинает
//!          тормозить! @n
//! @note    Загрузите один раз @a перед циклом, потом используйте много раз.
//! Посмотрите, как это сделано в примере
//!          TX\Examples\Tennis\Tennis.cpp.
//!
//! @see     txCreateWindow(), txCreateCompatibleDC(), txLoadImage(),
//! txDeleteDC(), txBitBlt(), txAlphaBlend(), txTransparentBlt()
//!
...
//!          if (!background) txMessageBox ("Не могу загрузить Background.bmp",
//!          "Да, я скопировал это из примера");
//!
//!          // НЕ НАДО часто загружать одно и то же изображение, особенно в
//!          цикле - программа будет тормозить!
...
//!                 Если передан указатель, то после уничтожения по указателю
//!                 записывается NULL.
//!
//! @return  Если операция была успешна - true, иначе - false.
//!
...
//! @code
//!          HDC background = txLoadImage ("Resources\\Images\\Background.bmp");
//!          if (!background) txMessageBox ("Не могу загрузить Background.bmp, и
//!          я скопировал это из примера.", "Oh, not now");
//!
//!          // НЕ НАДО часто загружать одно и то же изображение, особенно в
//!          цикле - программа будет тормозить!
//!          // Загрузите один раз перед циклом, потом используйте много раз.
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Drawing
//! @brief   Копирует изображение с одного холста (контекста рисования, DC) на
//! другой.
//!
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//! @warning Если контексты назначения или источника равны NULL, то они не
//! существуют и копирование вызовет ошибку.
//!          Наиболее частая причина - ошибка при загрузке файла изображения и
//!          отсутствие проверки на эту ошибку. Пример с проверкой на
//!          правильность загрузки см. ниже.
//!
//! @title   Режимы копирования:
//! @table   @tr SRCCOPY     @td Просто копирует :) - самый распространенный
//! режим
//!          @tbr
//!          @tr BLACKNESS   @td Заполняет холст-приемник черным цветом
//!          (холст-источник игнорируется).
//!          @tr WHITENESS   @td Заполняет холст-приемник белым цветом
//!          (холст-источник игнорируется).
//!          @tr DSTINVERT   @td Инвертирует цвета на холсте-приемнике
//!          (холст-источник игнорируется).
//!          @tr PATCOPY     @td Копирует цвет текущей кисти холста-приемника.
//!          @tbr
//!          @tr MERGECOPY   @td Приемник =   приемник & цвет текущей кисти
//!          источника.
...
//!          @tr PATPAINT    @td Приемник = (кисть приемника | ~источник) |
//!          приемник
...
//!          @endtable
//!
//! @see     txAlphaBlend(), txTransparentBlt(), txSetColor(), txGetColor(),
...
//!          HDC background = txLoadImage ("Resources\\Images\\Background.bmp");
//!          if (!background) txMessageBox ("Не могу загрузить Background.bmp, и
//!          да, я взял этот код из примера.", "Once again :(");
//!
//!          // НЕ НАДО часто загружать одно и то же изображение, особенно в
//!          цикле - программа будет тормозить!
//!          // Загрузите один раз перед циклом, потом используйте много раз.
...
//!          правильность загрузки см. ниже.
//!
//!          Стандартная функция TransparentBlt из Win32 API может
//!          масштабировать изображение. В txTransparentBlt это убрано для
//!          упрощения использования. If you need image scaling, use original
//!          function TransparentBlt and don't mess with stupid TX-based tools.
//!          (See implementation of txTransparentBlt in TXLib.h).
//!
//! @note    Если TransparentBlt не работает, используйте функцию AlphaBlend,
//! она вообще лучше.
//!
//! @see     txBitBlt(), txTransparentBlt(), txLoadImage(),
//! txCreateCompatibleDC()
//!
//! @usage   Пример использования см. в файле TX\Examples\Tennis\Tennis.cpp.
//! @code
//!          HDC superman = txLoadImage ("Resources\\Images\\Superman.bmp");
//!          if (!superman) txMessageBox ("Cannot load superman, all the
//!          monsters will succeed (I copied them from TXLib Help)", "Sorry");
//!
//!          // НЕ НАДО часто загружать одно и то же изображение, особенно в
//!          цикле - программа будет тормозить!
//!          // Загрузите один раз перед циклом, потом используйте много раз.
...
bool txTransparentBlt(HDC dest, double xDest, double yDest, double width,
                      double height, HDC src, double xSrc, double ySrc,
...
//! @param   width   Ширина копируемого изображения, неотрицательная
//! @param   height  Высота копируемого изображения, неотрицательная
//! @param   src     Контекст источника (откуда копировать). Должен иметь
//! 32-битовый формат и альфа-канал (см. ниже).
//! @param   xSrc    Х-координата верхнего левого угла изображения-источника,
...
//!          правильность загрузки см. ниже.
//!
//!          Изображение-источник и изображение-приемник не могут налагаться
//!          друг на друга.
//!
//! @note    Изображение должно быть загружено с помощью txLoadImage() и иметь
//! <b>32-битовый RGBA-формат.</b>
//!          Дополнительный канал <b>(A, альфа-канал)</b> этого формата отвечает
//!          за прозрачность участков изображения. 24-битовый формат (TrueColor
//!          RGB) функция txAlphaBlend не поддерживает.
//!
...
//!          полной непрозрачности. <b>При этом в прозрачных областях само
//!          изображение (в каналах R, G, B) должно быть черным, и чем
//!          прозрачнее, тем чернее.</b> См. изображение с альфа-каналом в
//!          примере TX\Examples\Tennis\Tennis.cpp (файл с теннисной ракеткой:
...
//!
//!        - Если значение альфа-канала для некоторого пикселя равно   0 (полная
//!        прозрачность), тогда значения
//!          каналов R,G,B для этого пикселя также станут 0 (это черный цвет).
//!        - Если значение альфа-канала для некоторого пикселя равно 255 (полная
//!        непрозрачность), тогда значения
//!          каналов R,G,B для этого пикселя не изменятся.
//!        - Для других значений альфа-канала, пиксели изображения станут
//!        темнее.
//!
//!          В Photoshop это можно сделать командой <b>Image @d Apply Image</b>
//!          с параметрами:
...
//! @endtable
//!
//! @note    Стандартная функция AlphaBlend из Win32 API может масштабировать
//! изображение. В txAlphaBlend это
//!          убрано для упрощения использования. If you still need image
//!          scaling, use original function AlphaBlend and don't mess with
//!          stupid TX-based tools. (See implementation of txAlphaBlend in
//!          TXLib.h).
//!
//! @see     txBitBlt(), txTransparentBlt(), txLoadImage(),
//! txCreateCompatibleDC()
...
//!          if (!batman) txMessageBox ("Call to Batman failed because I copied
//!          it from TXLib Help", "Do save yourself");
//!
//!          // НЕ НАДО часто загружать одно и то же изображение, особенно в
//!          цикле - программа будет тормозить!
//!          // Загрузите один раз перед циклом, потом используйте много раз.
...
//!
//!          ...
//!          txDeleteDC (batman);  // Don't worry, batman will return in "Batman
//!          returns" movie
//!          ...
//!
...
bool txAlphaBlend(HDC dest, double xDest, double yDest, double width,
                  double height, HDC src, double xSrc, double ySrc,
                  double alpha = 1.0);
//! @}
//}
...
//!
//! @note    <b>Перед началом задержки изображение в окне обязательно
//! обновится,</b> даже если рисование
...
//!
//!          Если в программе требуется задержка, то используйте функцию
//!          txSleep(), так как она автоматически обновляет изображение,
//!          независимо от состояния блокировки.
//!
//! @warning Избегайте блокирования на долгое время. Это может привести к
//! дефектам изображения в окне.
//!
//! @note    Если нажата клавиша Alt+PrintScreen, то блокировка временно
//! отменяется.
//!
...
//!          txClear();                        // Это вызвало бы мигание без
//!          txBegin() txSetFillColor (TX_RED); txRectangle (100, 100, 200,
//!          200); txEnd();                          // Здесь мы сразу увидим
...
//! требуется столько же раз вызвать
//!          txEnd().
//!
//! @note    Если нажата клавиша Alt+PrintScreen, то блокировка временно
//! отменяется.
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Service
//! @brief   Разрешает или запрещает автоматическое обновление изображения в
//! окне.
//!
...
//!
//!          В отличие от txBegin() и txEnd(), которые поддерживают вложенные
//!          вызовы и работают как "скобки для замерзания картинки",
//!          txUpdateWindow() позволяет явно установить или снять блокировку
//!          автоматического обновления.
//!
//!          Более полную информацию о блокировке см. в функциях txBegin(),
//!          txEnd() и txSleep().
//!
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//!          У этой функции есть синоним с простым понятным названием, поищите
//!          его в файле библиотеки, около
//!          @a определения этой функции. Или можно @strike скопировать
//!          @endstrike набрать это километровое имя и посмотреть, что
//!          получится.
//!
...
//!
//!              txSetTextAlign (TA_CENTER);
//!              txTextOut (txGetExtentX()/2, txGetExtentY()/2, "Press any key
//!              to exit!");
//! @endcode
//
//                 +--<<< Это текст помощи, который вы читали. Ищите дальше!
//                 Жмите [F3] или "Найти далее"
//                 |
//                 v
...
//     +--<<< Это прототип функции, а надо найти ее определение. Ищите дальше!
//     Жмите [F3] или "Найти далее"
//     |
//     v
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//! @warning Если уничтожается главное окно TXLib, функция main() прерывается и
//! программа автоматически завершается.
//!          При этом @b не гарантируется правильное завершение программы,
//!          поэтому так делать @b не рекомендуется.
//!
//! @see     txCreateWindow()
//!
//! @usage @code
...
//!              {
//!              if (In (txMousePos(), area)) txTextOut (100, 100, "What are you
//!              doing here?!"); txSleep (0);
//!              }
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//!          В возвращаемом значении выставленный в единицу 1-й (младший) бит
//!          означает нажатую левую Кнопку Мыши, 2-й - правую. @n Например,
//!          возвращенное число 3 (двоичное 11) означает одновременное нажатие
...
//!              {
//!              if (txMouseButtons() & 1) txCircle (txMouseX(), txMouseY(),
//!              20); if (txMouseButtons() & 2) txLine   (txMouseX(),
//!              txMouseY(), 0, 0); txSleep (0);
//!              }
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//!          @b Атрибуты - это цвет текста (colorText) и цвет фона
//!          (colorBackground), объединенные вместе: @nn <tt>colors = colorText
//!          + colorBackground * 16</tt> @nn либо @nn <tt>colors = colorText |
//!          (colorBackground \<\< 4)</tt> @nn Цвета атрибутов @b не имеют
//!          никакого отношения к цветам рисования, задаваемыми @ref txColors
//!          "TX_..." константами, RGB(), txSetColor(), txColor(),
...
//!          @tbr
//!          @tr  0 = @td @c 0x0 @td = Черный,       @td  8 = @td @c 0x8 @td =
//!          Темно-серый,
//!          @tr  1 = @td @c 0x1 @td = Синий,        @td  9 = @td @c 0x9 @td =
//!          Светло-синий,
//!          @tr  2 = @td @c 0x2 @td = Зеленый,      @td 10 = @td @c 0xA @td =
//!          Светло-зеленый,
//!          @tr  3 = @td @c 0x3 @td = Сине-зеленый, @td 11 = @td @c 0xB @td =
//!          Светло-сине-зеленый,
//!          @tr  4 = @td @c 0x4 @td = Красный,      @td 12 = @td @c 0xC @td =
//!          Светло-красный,
//!          @tr  5 = @td @c 0x5 @td = Малиновый,    @td 13 = @td @c 0xD @td =
//!          Светло-малиновый,
//!          @tr  6 = @td @c 0x6 @td = Темно-желтый, @td 14 = @td @c 0xE @td =
//!          Желтый,
//!          @tr  7 = @td @c 0x7 @td = Серый,        @td 15 = @td @c 0xF @td =
//!          Белый.
//! @endtable
//!
//!          В шестнадцатиричной системе счисления атрибуты задавать можно
//!          проще: если нужен, скажем, желтый цвет на синем фоне, то его код
//!          будет @c 0x1e (старшая цифра - старшие 4 бита - это цвет фона,
//!          младшая цифра - младшие 4 бита - это цвет текста).
//!
//! @see     txTextCursor(), txGetConsoleAttr(), txSetConsoleCursorPos(),
...
//!
//! @see     txTextCursor(), txSetConsoleAttr(), txSetConsoleCursorPos(),
//! txGetConsoleCursorPos(),
//!          txGetConsoleFontSize(), txClearConsole()
//!
//! @usage @code
...
//! @return  Если операция была успешна - true, иначе - false.
//!
//!          При стирании используются текущие атрибуты (цвета текста и фона)
//!          консоли.
//!
//! @see     txTextCursor(), txSetConsoleAttr(), txGetConsoleAttr(),
//! txGetConsoleCursorPos(),
//!          txGetConsoleFontSize(), txClearConsole()
//!
//! @usage @code
...
//!
//! @note    Нельзя установить совсем любую позицию. Текст в консоли расположен
//! по прямоугольной сетке, размер
//!          которой зависит от размеров шрифта консоли. Устанавливаемая позиция
//!          округляется, чтобы курсор попал в ячейку сетки. См. пример к
//!          функции txGetConsoleFontSize().
//!
//! @see     txTextCursor(), txSetConsoleAttr(), txGetConsoleAttr(),
//! txGetConsoleCursorPos(),
//!          txGetConsoleFontSize(), txClearConsole()
//!
//! @usage @code
//!          txSetConsoleCursorPos (txGetExtentX(), txGetExtentY());  // Центр
//!          Вселенной
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!          txSetConsoleCursorPos (5 * size.x, 10 * size.y);  // А теперь мигай
//!          там
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @see     txSetConsoleAttr(), txGetConsoleAttr(), txSetConsoleCursorPos(),
//! txGetConsoleCursorPos(),
//!          txGetConsoleFontSize(), txClearConsole(), txCreateWindow(),
//!          txUpdateWindow(), txLock(), txUnlock(), txGDI()
//!
//! @usage @code
//!          txTextCursor (false);
//!          ...
...
//!          @tr SND_ASYNC       @td Звук проигрывается одновременно с работой
//!          программы. @n
//!                                  Чтобы отменить звучание, вызовите
//!                                  txPlaySound (NULL).
//!          @tr SND_SYNC        @td Выполнение программы приостанавливается до
//!          окончания воспроизведения звука.
//!          @tr SND_LOOP        @td Зацикливать звук при воспроизведении. @n
//!                                  Чтобы отменить звучание, вызовите
//!                                  txPlaySound (NULL).
//!          @tbr
//!          @tr SND_NODEFAULT   @td Не использовать звук по умолчанию, если
//!          нельзя проиграть указанный звуковой файл.
//!          @tr SND_NOSTOP      @td Если какой-либо звук уже проигрывается, не
//!          останавливать его для запуска указанного звука.
//!          @tr SND_APPLICATION @td Проигрывать звук, используя программу,
//!          зарегистрированную для данного типа звуковых файлов.
//!          @endtable
//!
//! @note    Поддерживаются только файлы в формате WAV. Остальные форматы (MP3 и
//! др.) надо перекодировать.
//!          Переименование со сменой расширения не поможет, как и в случае с
//!          форматом картинок в txLoadImage().
//!
//! @usage @code
...
//!
//! @warning Текст не должен превышать _TX_BIGBUFSIZE символов, а заголовок @d
//! _TX_BIGBUFSIZE символов, иначе они
//!          обрезаются.
//!
//! @note    Вместо <tt><b>txMessageBox (text, header, flags)</b></tt> можно
//! использовать стандартную функцию Win32
//!          <tt><b>MessageBox (txWindow(), text, header, flags)</b></tt>.
//!          Отличия txMessageBox в том, что она автоматически подставляет
//!          окно-родитель, и в том, что при выводе в окно строчки переводятся в
//!          формат UNICODE. Это важно лишь в том случае, когда <i>в
//!          региональных настройках контрольной панели Windows неверно
//!          установлена кодовая страница для программ, не поддерживающих
//!          UNICODE.</i> В остальных случаях нужды в @c txMessageBox нет.
//!
...
//!              {
//!              MessageBox (txWindow(), "Хватит и обычного MessageBox()",
//!              "Win32 сообщает", 0);
//!              }
//!          else
//!              txMessageBox ("Спасаем от кракозябл вместо русских букв.
//!              Гарантия. Недорого.");
//! @endcode
//}----------------------------------------------------------------------------------------------------------------

unsigned txMessageBox(const char *text, const char *header = "TXLib сообщает",
                      unsigned flags = 0);
...
//!
//!          Функция формирует сообщение по правилам printf() и выводит во
//!          всплывающем окне.
//!
//! @warning
//!        - Эта функция требует, чтобы при компиляции константа версии Internet
//!        Explorer @c (_WIN32_IE) была
//!          задана не ниже 0x0500. Для этого надо либо <b>включить TXLib.h
//!          вместо @c windows.h или перед ним.</b> Либо надо самостоятельно
//!          определить @c (\#define) эту константу. <small>С версией Internet
//!          Explorer это связано потому, что при его установке в Windows
//!          обновляются многие компоненты (например, @c shell32.dll и @c
//!          comctl32.dll), которые влияют на функциональность системы
//!          независимо от использования браузера). Сам Internet Explorer в
//!          отображении сообщения не участвует.</small>
//!        - Сообщение не должно превышать _TX_BUFSIZE символов, иначе оно
//!        обрезается.
//!
...
//!                        "Поезд на %s отправляется в %d:%d.", station, hours,
//!                        minutes);
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//!          Функция формирует сообщение по правилам printf() и передает его в
//!          OutputDebugString(). Ее вывод можно перехватить отладчиком или
//!          утилитами-логгерами, например, <a
//!          href=http://technet.microsoft.com/ru-ru/sysinternals/bb896647%28en-us%29.aspx>DbgView</a>.
//!          Если этого не сделать, и не задать первый символ @c '\\a' (см.
//!          ниже), то о сообщении никто не узнает. :(
//! @note
//!        - Если первый            символ в строке @c '\\a', то сообщение также
//!        дублируется txMessageBox().
//!        - Если первый или второй символ в строке @c '\\f', то сообщение также
//!        дублируется printf().
//!
//! @warning Сообщение не должно превышать _TX_BIGBUFSIZE символов, иначе оно
//! обрезается.
//!
...
//!
//!          Макрос sizearr() вычисляет размер массива в элементах, проверяя,
//!          можно ли его правильно вычислить при компиляции.
//!
//!          Макрос SIZEARR() просто делит размер всего массива в байтах на
//!          размер его элемента, получается размер массива в элементах. Он
//!          <b>не проверяет,</b> можно ли его правильно вычислить, и при
//!          неправильном использовании может выдать <b>неверный размер.</b>
//!
//! @warning SIZEARR() выдает неверный размер, если определение массива вместе с
//! его размером, известным при
//!          компиляции, недоступно в месте использования SIZEARR(). См. пример
//!          ниже.
//!
//! @note    В Microsoft Visual Studio 6 макрос sizearr() недоступен - у ее
//! компилятора недостаточно сил, чтобы
//!          скомпилировать его. :(
//!
//! @usage @code
//!          void test()
//!              {
//!              // Размер этого массива, хоть и не указан, но может быть
//!              автоматически определен
//!              // компилятором при компиляции программы. Он равен 4 (четыре
//!              структуры POINT).
//!
//!              POINT coord[] = { {110, 110}, {120, 120}, {130, 110}, {140,
//!              120} };
//!
//!              // Здесь размер массива известен при компиляции, т.к. он был
//!              определен тут же.
//!
//!              for (int i = 0; i < sizearr (coord) - 1; i++)
//!                  txLine (coord[i].x, coord[i].y, coord[i+1].x,
//!                  coord[i+1].y);
//!
//!              DrawLines1 (coord);                  // Попытка передать массив
//!              без передачи размера. DrawLines2 (coord, sizearr (coord)); //
//!              Правильная передача размера массива.
//!
//!              DrawLines3 (coord);                  // В принципе, можно и
//!              так, но тут ВОДЯТСЯ ШАБЛОНЫ.
//!              }
//!
...
//!              {
//!              // Массивы в Си передаются как указатели на начало массива.
//!              Поэтому:
//!              // 1) sizearr здесь выдаст ошибку компиляции, и ее легко будет
//!              найти.
//!              // 2) SIZEARR здесь неверно посчитает размер, что намного хуже,
//!              //      т.к. он будет равен sizeof (POINT*) / sizeof (POINT) ==
//!              4/8 == 0.
//!
//!              for (int i = 0; i < sizearr (coord) - 1; i++)
//!                  txLine (coord[i].x, coord[i].y, coord[i+1].x,
...
#define sizearr(arr)                                                           \
  (sizeof(get_size_of_an_array_with_unknown_or_nonconst_size_(arr)))
...
char (&get_size_of_an_array_with_unknown_or_nonconst_size_(T (&)[N]))[N]; // ;)
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Misc
//! @brief   Проверка, находится ли параметр х внутри замкнутого интервала [a;
//! b]
//!
...
//!          while (txMouseButtons() != 1)
//!              {
//!              if (In (txMouseX(), 110, 120)) txTextOut (100, 100, "Meet the
//!              wall!"); txSleep (0);
//!              }
//! @endcode
//}----------------------------------------------------------------------------------------------------------------

template <typename T> inline bool In(T x, T a, T b);
...
//! @param   rect  Прямоугольник     в виде <tt> RECT  {left, top, right,
//! bottom} </tt>
//!
...
//!                  txRectangle (button.left, button.top, button.right,
//!                  button.bottom);
//!
//!                  if (txMouseButtons())
//!                      {
...
//!
//!          Вы еще помните, что означают разные скобочки в обозначении
//!          интервалов? :)
//!
//! @usage @code
//!          char message[100] = "Maybe...";
//!          sprintf ("You SUDDENLY got %d bucks now. But note that tax rate is
//!          $%d.", random (100), 100); txMessageBox (message, "Lottery");
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//!          Вы все еще помните, что означают разные скобочки в обозначении
//!          интервалов? :)
//!
//! @usage @code
...
//!              {
//!              char message[100] = "Maybe...";
//!              sprintf ("Проиграли в лотерею? Отдайте долг в %d рублей",
//!              -money); txMessageBox (message, "Быстро!");
//!              }
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @usage @code
//!          if (MIN (3, 7) != 3) printf ("Your CPU is still broken, throw it
//!          away twice.");
//! @endcode
//!
//! @hideinitializer
...
//!
//! @usage @code
//!          double weight = 5.5;       // 5.5 kilos is the weight of Maru in
//!          2012. int Maru = ROUND (weight); // Should use ROUND() because Maru
//!          is so round.
//! @endcode
//!
//! @hideinitializer
...
//!
//! @usage @code
//!          if (txPI == 1) txMessageBox ("Вы попали в другую Вселенную.",
//!          "Поздравляем", MB_ICONSTOP);
//! @endcode
//!
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @return  Квадрат, полученный путем возведения в него числа, заданного для
//! возведения в квадрат
//!
//! @note    Это пример, как <b> не надо </b> писать код: txSqr() @d функция с
//! "медвежьей услугой". Иногда встречаются
//!          те, кто любит печатать в функции результат ее вычислений <small>
//!          (не данные для отладки, а именно результат),
//!          </small> вместо того, чтобы просто возвращать его туда, где эту
//!          функцию вызывали. Пусть эти люди воспользуются приведенной txSqr()
//!          для какого-нибудь нужного дела, особенно в цикле. Пример, конечно,
//!          несколько преувеличен. См. в исходном тексте код этой навязчивой
//!          радости.
//!
//! @usage @code
...
//!              printf ("%6.2lf...", r);
//!
//!              double square = M_PI * txSqr (r);  // Надолго запомним эту
//!              площадь! printf ("\b\b\b   \t");
//!
//!              printf ("%-.2lf\n", square);
//!              }
//! @endcode
...
inline double txSqr(double x) {
  double sqr =
      pow(sqrt(x) * sqrt(x), sqrt(4.0)); // Бурная вычислительная деятельность

  char str[1024] = "";
  _snprintf_s(str, sizeof(str),
              "Возведение дало %g!"
              "!!"
              "!!"
              " Вы рады????    ",
...
//! @param   type  Имя типа
//!
//! @return  Значение типа @p type, покомпонентно инициализированное по
//! умолчанию (для встроенных типов - нулем).
//!
//! @usage @code
//!          void f (POINT p);
//!          ...
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Misc
//! @brief   Автоматический вызов функции при завершении другой функции (аналог
//! @c __finally)
//!
//! @param   param_t  Тип параметра автоматически вызываемой функции
//! @param   param    Имя параметра автоматически вызываемой функции
//! @param   func     Тело автоматически вызываемой функции (фигурные скобки не
//! обязательны)
//!
//! @par     Макрос <tt>TX_AUTO_FUNC (param_t, param, func)</tt>
//! @note
//!        - Для автоматически вызываемой функции допускается только @a один
//!        параметр.
//!        - Его тип @c param_t и имя @c param должны соответствовать
//!        определению переменной, доступной в текущей
//!          области видимости.
//!          Параметр вызываемой функции будет связан с этой переменной через
//!          ссылку.
//!        - Синоним: TX_FINALLY
//!
//! @warning В Microsoft Visual Studio 6 и 2003 в отладочной конфигурации
//! (Debug) этот макрос работать не будет,
//!          см. <a href="http://support.microsoft.com/kb/199057">MS KB Article
//!          199057</a>. Можно обходиться макросом @c _TX_AUTO_FUNC, см. его
//!          определение в исходном тексте рядом с определением @c TX_AUTO_FUNC.
//!
//! @par     Макрос <tt>tx_auto_func (func)</tt>
//! @note
//!        - @a Все переменные вызываемой функции связываются с переменными
//!        внешней функции по ссылке.
//!        - Их названия и типы @a не указываются. Указывается только тело
//!        вызываемой функции.
//!        - Эта форма использует лямбда-функции @c C++0x, поэтому при
//!        компиляции требуется <i>MSVS 2010</i>
//!          или <i>GCC не ниже версии 4.5 с ключом компиляции @c -std=c++0x или
//!          @c -std=c++11.</i>
//!        - Синоним: tx_finally
//!
//! @see     txAutoLock
...
//!              {
//!              int x = 1;
//!              TX_AUTO_FUNC (int, x, $(x));              // Will be printed at
//!              return
//!
//!              FILE* f = fopen (__FILE__".o.txt", "w");  // Will be closed at
//!              return TX_AUTO_FUNC (FILE*, f, fclose (f));
//!
//!              fprintf (f, "start: x = %d\n", x);        // Do some job
//!              x = 2;                                    // Do some job
//!              }
//!
//!          void f2()                                     // Do the same. For
//!          C++0x only
//!              {
//!              int x = 1;
...
#define _tx_auto_fun2(n, func)                                                 \
  auto _tx_auto_func_##n = _tx_auto_func([&]() { func; })

template <typename T> struct _tx_auto_func_ {
...
template <typename T> _tx_auto_func_<T> _tx_auto_func(T func) {
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Misc
//! @brief   Замена стандартного макроса assert(), с выдачей сообщения через
//! txMessageBox(), консоль и
//!          OutputDebugString().
//!
...
//!
//!          Если условие, проверяемое assert(), истинно, то макрос ничего не
//!          делает. @n Если условие оказывается ложно, то выводится
//!          диагностическое сообщение и программа аварийно завершается.
//!
//! @warning <b>При компиляции в режиме Release (или если определен NDEBUG)
//! assert превращается в пустой оператор.</b> @n
//!          Не надо помещать в assert() действия, которые важны для работы
//!          алгорима!
//!
//! @note    Если условие @c cond может быть вычислено уже во время компиляции
//! как ложное, компилятор может
//!          предупредить об этом (как о делении на 0).
//! @note    <small>See: <a
//! href=http://lars-lab.jpl.nasa.gov/JPL_Coding_Standard_C.pdf> "JPL
//! Institutional Coding
//!          Standard for the C Programming Language", Jet Propulsion
//!          Laboratory, California Institute of Technology, JPL DOCID D-60411,
//!          Ver. 1.0, March 3, 2009</a>, page 15.</small>
//!
//! @see     asserted, verified, verify(), TX_ERROR(), TX_DEBUG_ERROR(),
//! txOutputDebugPrintf(), txMessageBox(),
...
#undef assert
#define assert(cond)                                                           \
  _txNOP(!(cond) ? (TX_ERROR("\a"                                              \
                             "ВНЕЗАПНО: Логическая ошибка: "                   \
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Misc
//! @brief   Выводит диагностическое сообщение в случае нулевого или ложного
//! результата.
//!
//! @return  Всегда 0
//!
...
//!
//! @note    <b>Предполагается, что операция в случае неуспеха возвращает 0 или
//! false.</b> @n@n
//!          <b>При компиляции в режиме Release (или если определен NDEBUG)
//!          asserted превращается в пустое место.</b>
//!
//! @see     assert(), verify(), verified, TX_ERROR(), TX_DEBUG_ERROR(),
//! txOutputDebugPrintf(), txMessageBox(),
//!          txNotifyIcon(), __TX_FILELINE__, __TX_FUNCTION__
//!
//! @usage @code
...
#define asserted                                                               \
  || TX_ERROR("\a"                                                             \
              "Обнаружен нулевой или ложный результат.")
...
//!
//!          Если условие, проверяемое verify(), истинно, то макрос ничего не
//!          делает. @n Если условие оказывается ложно, то выводится
//!          диагностическое сообщение и программа аварийно завершается.
//!
//! @note    Действие макроса аналогично assert(), но при компиляции в режиме
//! Release (или если определен NDEBUG)
//!          verify @b не превращается в пустой оператор.
//!
//! @see     verified, assert(), asserted, TX_ERROR(), TX_DEBUG_ERROR(),
//! txOutputDebugPrintf(), txMessageBox(),
//!          txNotifyIcon(), __TX_FILELINE__, __TX_FUNCTION__
//!
//! @usage @code
...
//!
//! @param   msg  Сообщение с произвольным количеством параметров в стиле
//! функции @c printf().
//!
//! @note    @c GCC в режиме строгого соответствия стандарту ANSI (с ключом
//! командной строки <tt>-ansi</tt>) и
//!          Microsoft Visual Studio версий 6 и 2003 не поддерживают макросы с
//!          переменным числом параметров. Поэтому, если параметров несколько,
//!          они разделяются @b _ (@ref _ "символом подчеркивания",
//!          переопределенным в запятую) или символом TX_COMMA, вместо настоящей
//!          запятой, так как TX_ERROR @d макрос. @n Если в проекте используются
//!          библиотеки <a href=http://boost.org><tt>boost</tt></a>, то их надо
//!          включать
//!          @b до @c TXLib.h и вместо символа подчеркивания пользоваться
//!          TX_COMMA, так как @c boost использует символ подчеркивания как свой
//!          собственный служебный макрос в модуле @c boost::preprocessor,
//!          @strike где творится дефайновый адЪ. @endstrike
//!
//! @return  Всегда false
//!
//! @see     _, TX_COMMA, assert(), asserted, verify(), verified,
//! TX_DEBUG_ERROR(), txOutputDebugPrintf(),
//!          txMessageBox(), txNotifyIcon(), __TX_FILELINE__, __TX_FUNCTION__
//!
//! @usage @code
//!          TX_ERROR ("Не смог прочитать 'Войну и мир'. Отмазка %d: не нашел
//!          '%s'", reasonNum, fileName);
//! @endcode
//!
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------

// Variadic macros not supported in Strict ANSI mode and in MSVC prior to MSVC 8
// (2005)
...
//!
//! @see     _, TX_COMMA, assert(), asserted, verify(), verified, TX_ERROR(),
//! txOutputDebugPrintf(),
//!          txMessageBox(), txNotifyIcon(), __TX_FILELINE__, __TX_FUNCTION__
//!
//! @usage @code
//!          TX_DEBUG_ERROR ("Так и не смог прочитать 'Войну и мир'. Отмазка %d:
//!          потерял '%s'", reasonNum, fileName);
//! @endcode
//!
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------
...
//!          После распечатки атрибуты консоли сбрасываются в 0x07 (светло-серый
//!          на черном).
//!
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Misc
//! @brief   Макрос, позволяющий передать переменное число параметров в
//! какой-либо другой макрос.
//!
//! @note    <b>Символ подчеркивания и символ TX_COMMA просто переопределяются в
//! запятую.</b>
//!
//! @see     TX_ERROR(), TX_DEBUG_ERROR()
//!
//! @usage @code
//!          TX_ERROR ("Слишком умный абзац: роман 'Война и мир', файл '%s',
//!          строка %d" _ fileName _ lineNum);
//! @endcode
//!
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------
...
#define __TX_COMPILER__                                                                   \
  "GNU g++ " TX_PREPROCESS(__GNUC__) "." TX_PREPROCESS(__GNUC_MINOR__) "." TX_PREPROCESS( \
      __GNUC_PATCHLEVEL__) ", std=" TX_PREPROCESS(__cplusplus)
#elif defined(_MSC_VER)
#define __TX_COMPILER__                                                        \
  "MSVS " TX_PREPROCESS(_MSC_VER) ", std=" TX_PREPROCESS(__cplusplus)

#elif defined(__INTEL_COMPILER)
#define __TX_COMPILER__                                                        \
  "Intel C++ " TX_PREPROCESS(__INTEL_COMPILER) ", std=" TX_PREPROCESS(         \
      __cplusplus)
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Misc
//! @brief   Макрос, раскрывающийся в имя файла и номер строки файла, где он
//! встретился.
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @warning Если определение имени функции не поддерживается компилятором, то
//! __TX_FUNCTION__ раскрывается в имя
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Service
//! @brief   Устанавливает альтернативную функцию обработки оконных сообщений
//! Windows (оконную функцию) для окна
//!          TXLib.
//!
//! @param   wndProc  Новая оконная функция окна TXLib. @n
//!                   Если NULL, то текущая оконная функция удаляется и
//!                   устанавливается стандартная.
//!
//! @return  Адрес предыдущей оконной функции для окна TXLib.
//!
//!          Заданная оконная функция будет вызываться @b до обработки события
//!          средствами TXLib. Она должна быть функцией со следующим прототипом:
//! @code
//!          LRESULT CALLBACK NewWndProc (HWND window, UINT message, WPARAM
//!          wParam, LPARAM lParam);
//! @endcode
//!
//! @warning Оконная функция будет вызываться из вспомогательного (второго)
//! потока, создаваемого @ref
//!          txCreateWindow(). Это @b не тот же самый поток, в котором
//!          выполняется main(). В связи с этим будьте внимательны при работе с
//!          глобальными переменными или их аналогами, т.к. может возникнуть
//!          "гонка потоков" (race condition).
//!
//! @warning Если оконная функция вернет значение, не равное 0, то стандартная
//! обработка сообщений средствами TXLib
//!          @b не будет произведена. Из-за этого, возможно, окно даже не сможет
//!          нормально закрыться. Придется завершать программу с помощью
//!          Alt-Ctrl-Del из диспетчера задач, или из более продвинутого
//!          диспетчера <a
//!          href=http://technet.microsoft.com/en-us/sysinternals/bb896653.aspx>
//!          Process Explorer</a>. Если Вы берете на себя обработку оконных
//!          сообщений, делайте ее по правилам Win32 (см. MSDN), включая вызов
//!          DefWindowProc().
//!
//! @note    Полностью поменять оконную функцию можно с помощью функций
//! SetWindowLong или SetWindowLongPtr:
//! @code
//!          WNDPROC OldWndProc = (WNDPROC) SetWindowLongPtr (txWindow(),
//!          GWL_WNDPROC, (LONG_PTR) NewWndProc);
//! @endcode
//!          При этом надо обязательно всегда вызывать старую оконную функцию с
//!          помощью CallWindowProc, (см. MSDN), иначе последствия будут такими
//!          же плачевными, как описаны выше.
//!
//! @see     txCreateWindow(), txDialog, txInputBox()
//!
//! @usage @code
//!          LRESULT CALLBACK MyWndProc (HWND window, UINT message, WPARAM
//!          wParam, LPARAM lParam);
//!
//!          int main()
//!              {
...
//!              Прропппеллллерррр! return 0; // Продолжить обработку сообщения
//!              средствами TXLib
//!              }
//! @endcode
//}----------------------------------------------------------------------------------------------------------------
...
//!          холст функцией txLock() и затем разблокировать с помощью
//!          txUnlock(). Это связано с тем, что обновление содержимого окна (для
//!          тех, кто знает Win32: обработка сообщения WM_PAINT) в библиотеке
//!          TXLib происходит в отдельном вспомогательном потоке. Надолго
//!          блокировать его нельзя - при заблокированном потоке окно не
//!          обновляется.
//!
...
//!          окно перестает реагировать на действия пользователя и
//!          перерисовываться. Нельзя также вызывать txSleep() или Sleep() при
//!          заблокированном потоке.
//!
//!          txLock() / txUnlock() - это низкоуровневый механизм. Он отличается
//!          от более простого высокоуровневого механизма txBegin() / txEnd() /
//!          txUpdateWindow(), который не приостанавливает поток, а просто
//!          отключает принудительное постоянное обновление окна.
//!
//! @see     txDC(), txLock(), txUnlock(), txGDI()
//!
//! @usage   См. исходный текст функций _txCanvas_OnPAINT() и _txConsole_Draw()
//! в TXLib.h.
//}----------------------------------------------------------------------------------------------------------------
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Service
//! @brief   Вызов функции Win32 GDI с автоматической блокировкой и
//! разблокировкой.
//!
//! @param   command  Команда GDI (возможно, возвращающая значение)
//!
//! @return  Значение, возвращаемое вызываемой функцией GDI. Если система
//! рисования не готова, возвращается
//!          значение false.
//!
//! @note    Если система рисования не готова (txDC() возвращает NULL), то
//! команда GDI не выполняется, а txGDI()
//!          возвращает значение false. @n
//!
//! @note    Если в вызове функции GDI используются запятые, то используйте
//! двойные скобки, чтобы получился один
//!          параметр, так как txGDI() все-таки макрос.
//!
//! @see     txDC(), txLock(), txUnlock()
//!
//! @usage @code
//!          txGDI (( Rectangle (txDC(), x1, y1, x2, y2) ));  // Не забудьте про
//!          две ((ДВЕ)) скобки
//! @endcode
//!
//! @hideinitializer
...
//!
//!          Если константа определена с помощью \#define @a до включения
//!          TXLib.h в программу, ранняя инициализация (до запуска функции @c
//!          main) @b не проводится.
//!
//! @note    Ранняя инициализация включает:
//!        - Открытие окна консоли,
//!        - Установку кодовой страницы _TX_CP (1251) консоли для поддержки
//!        русского языка,
//!        - Установку русской локали стандартной библиотеки,
//!        - Переинициализация библиотек stdio и iostream на случай, если
//!        приложение не скомпоновано как консольное
//!          и библиотеки остались неинициализированы,
//!        - Перехват системных сигналов (деление на 0, обращение по неверному
//!        адресу и т.д.),
//!        - Перехват необработанных исключений,
//!        - Смена заголовка консольного окна,
//!        - Установка режима паузы по завершении программы, чтобы окно
//!        закрывалось не сразу,
//!        - Подавление паузы при запуске из сред программирования, заставляющей
//!        нажимать на клавишу два раза. @nn
//!
//! @note    Если ранняя инициализация запрещена, но в EXE-файле создается окно
//! TXLib с помощью txCreateWindow(),
//!          то библиотека все равно будет инициализирована. В @b DLL ранняя
//!          инициализация никогда @b не проводится. @nn
//!
//! @note    Ранняя инициализация не потокобезопасна (not thread-safe).
//!
//! @see     txCreateWindow(), _TX_ALLOW_KILL_PARENT, _TX_WAITABLE_PARENTS,
//! _txConsoleMode
//!
//! @usage @code
//!          #define _TX_NOINIT
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Technical
//! @brief   Режим отображения консольного окна. Допустимы любые флаги функции
//! ShowWindow.
//!
...
//!
//! @usage @code
//!          _txWindowStyle &= ~WS_CAPTION; // FullScreen: окно без заголовка,
//!          размером с экран txCreateWindow (GetSystemMetrics (SM_CXSCREEN),
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Technical
//! @brief   Список запускающих программ, которые ждут нажатия клавиши после
//! завершения процесса TXLib.
//!
//!          Если программа перечислена в списке и TXLib запущена из нее, то при
//!          завершении TXLib указанная программа будет закрыта. (Это
//!          произойдет, если не открыто графическое окно TXLib, а есть только
//!          окно консоли.)
//!
//!          Программы разделяются пробелом или запятой. Допускается указание
//!          родителя запускающей программы, после двоеточия.
//!
...
#define _TX_WAITABLE_PARENTS                                                   \
  "cmd.exe:devenv.exe, "                   /* MSVS 2003-2010  */               \
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Technical
//! @brief   Разрешать принудительное завершение вызывающих программ, ждущих
//! нажатия клавиш после завершения TXLib.
//!
//!          Иначе отменяется собственная пауза до нажатия клавиши, встроенная в
//!          TXLib, и пусть тогда паузу делает вызывающий процесс.
//!
//!          Список вызывающих программ, которые могут делать такую паузу,
//!          задается в _TX_WAITABLE_PARENTS.
//!
//!          Может задаваться перед включением TXLib.h в программу.
//!
...
#if !defined(                                                                  \
    _TX_ALLOW_KILL_PARENT) // DISCLAIMER: Я не призываю к убийству родителей!
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Technical
//! @brief   Включает/отключает внутреннюю трассировку исполнения кода
//! библиотеки.
//!
...
//!          ее вывод можно перехватить утилитами-логгерами, например, <a
//!          href=http://technet.microsoft.com/ru-ru/sysinternals/bb896647%28en-us%29.aspx>
//!          DbgView</a>. Это можно изменить, переопределив макрос TX_TRACE до
//!          включения TXLib.h в программу.
//!
//! @warning Трассировка @b очень тормозит выполнение программы, особенно при
//! отладке в Microsoft Visual Studio.
//!          В этом случае лучше пользоваться DbgView (см. выше) и запускать
//!          отлаживаемую программу отдельно, а не из-под отладчика Visual
//!          Studio.
//!
//!          _TX_ALLOW_TRACE и TX_TRACE задаются перед включением TXLib.h в
//!          программу.
//!
//! @usage @code
...
#define $                                                                      \
  {                                                                            \
    _txFile = __FILE__;                                                        \
    _txLine = __LINE__;                                                        \
    _txFunc = __TX_FUNCTION__;                                                 \
    TX_TRACE;                                                                  \
  }
#undef $1
#define $1                                                                     \
  _txFuncEntry __txFuncEntry;                                                  \
  $
...
//!          ее вывод можно перехватить утилитами-логгерами, например, <a
//!          href=http://technet.microsoft.com/ru-ru/sysinternals/bb896647%28en-us%29.aspx>
//!          DbgView</a>. Для "раскраски" лога есть файл
//!          <tt>TX\Dev\DbgView.ini,</tt> его надо загрузить в DbgView через
//!          меню Edit/Filter/Load (Ctrl+L).
//!
//!          С помощью TX_TRACE можно трассировать код самой библиотеки TXLib.
//!          Для этого надо разрешить трассировку TXLib, определив макрос
//!          _TX_ALLOW_TRACE перед включением файла TXLib.h в программу. По
//!          умолчанию трассировка TXLib выключена.
//!
//!          TX_TRACE можно переопределить в свой код. Тогда, если трассировка
//!          библиотеки разрешена, он будет вызываться почти перед каждой
//!          исполняемой строкой TXLib. Может быть, это кому-нибудь будет
//!          интересно.
//!
//! @usage @code
//!          int main()
//!              {
//!              ...
//!              TX_TRACE  // Через DbgView увидим имя файла и номер выполняемой
//!              строки
//!              ...
//!              }
//! @endcode
...
void _txTrace(const char file[], int line, const char func[]) {
  ...
  char mark =
      marks[id == _txMainThreadId][id == _txCanvas_ThreadId][_txInTX > 0];

  txOutputDebugPrintf("%s - [%c] %s (%d) %s\n", _TX_VERSION, mark,
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Service
//! @brief   Класс для автоматической блокировки и разблокировки критической
//! секции.
//!
//!          Начиная с <a
//!          href=http://progbook.ru/2008/08/03/sovremennoe-proektirovanie-na-c-andrejj.html>
//!          Александреску,</a> его пишут все и он прост, как пробка: в
//!          конструкторе @d EnterCriticalSection(), в деструкторе @d
//!          LeaveCriticalSection(). Это @c RAII в чистом виде: вы никогда не
//!          забудете разблочить секцию and your thread show will always go on.
//!
//!          Некоторые добавления: есть возожность вызвать
//!          TryEnterCriticalSection(), и, если она не заблочила секцию,
//!          деструктор ее не разблочивает. Есть оператор для проверки,
//!          заблочилась ли секция или нет (см. конструкторы класса и оператор
//!          @c bool).
//!
//! @note    Класс не инициализирует и не удаляет критическую секцию. Только
//! синхронизирует. Остальное сами-сами :)
//!
...
public:
  ...
  //! @param   mandatory  Если @c true,  то блокировать обязательно
  //! (EnterCriticalSection). @n
  //!                     Если @c false, то только пытаться блокировать
  ...
  //!          void foo()
  //!              {
  //!              txAutoLock lock (&cs);        // Здесь вызовется
  ...
  txAutoLock(CRITICAL_SECTION *cs, bool mandatory = true) : cs_(cs) {
    $1 if (!cs_) return;

    if (mandatory) {
      $ EnterCriticalSection(cs_);
    } else {
      $ TryEnterCriticalSection(cs_) ? 0 : (cs_ = NULL);
    }
  }
  ...
  //!
  //! @param   mandatory  Если @c true,  то блокировать @b обязательно,     как
  //! в @ref txLock (true). @n
  //!                     Если @c false, то только @b пытаться блокировать, как
  //!                     в @ref txLock (false).
  //! @usage @code
  //!          void foobar()
  //!              {
  ...
  txAutoLock(bool mandatory = true) : cs_(NULL) {
  ...
  operator bool() const { $1 return (cs_ != NULL); }
...
//!           функцией txDialog::setLayout(), или
//!              указать карту расположения при показе диалогового окна функцией
//!              txDialog::dialogBox();
//!           -# Переопределить в своем классе функцию txDialog::dialogProc()
//!           для обработки событий диалогового
//!              окна или построить карту реакций на команды с помощью макросов
...
public:
  ...
  //!           - Остальные элементы описывают контролы (порядок задания
  //!           параметров контрола похож на порядок
  //!             параметров в ресурсном скрипте)
  //!           - Последний элемент - txDialog::END или {NULL}
  //!
  //! @see     txDialog::setLayout(), txDialog::dialogBox(),
  //! txDialog::dialogProc()
  //!
  //! @usage @code
  //!          Cм. реализацию функции txInputBox().
...
public:
  ...
  //!
  //! @return  В большинстве случаев false, детальнее см. DialogProc в <a
  //! href=msdn.com>MSDN</a>.
  //!
  //!          Эту функцию надо переопределить для обработки событий окна, или
  //!          построить ее с помощью макросов TX_BEGIN_MESSAGE_MAP(),
  //!          TX_END_MESSAGE_MAP, TX_COMMAND_MAP.
  //!
  //! @see     txDialog::dialogBox(), TX_BEGIN_MESSAGE_MAP(),
  //! TX_END_MESSAGE_MAP, TX_COMMAND_MAP
  //!
  //! @usage @code
  //!          Cм. реализацию функции txInputBox().
  ...
  //!                   Если не указан - используется значение, заданное
  //!                   txDialog::setLayout() или конструктором
  //!                   txDialog::txDialog (const txDialog::Layout*).
  //! @param   bufsize  Размер буфера для создания шаблона диалога (если не
  //! указан - размер по умолчанию)
  //!
  ...
  //!
  //! @see     txDialog::dialogProc(), txDialog::setLayout(), txDialog::Layout,
  //! txDialog
  //!
  //! @usage @code
  //!          Cм. реализацию функции txInputBox().
...
private:
  ...
  //! in32 должна быть статической).
  //}----------------------------------------------------------------------------------------------------------------

private:
  static ptrdiff_t CALLBACK dialogProc__(HWND wnd, UINT msg, WPARAM wParam,
                                         LPARAM lParam);
...
//!
//! @see     TX_BEGIN_MESSAGE_MAP(), TX_END_MESSAGE_MAP, TX_HANDLE(),
//! TX_COMMAND_MAP, txDialog::dialogProc(), txDialog
//!
//! @usage @code
//!          Cм. реализацию функции txInputBox().
//! @endcode
...
#define TX_BEGIN_MESSAGE_MAP()                                                 \
  virtual int dialogProc(HWND _wnd, UINT _msg, WPARAM _wParam,                 \
                         LPARAM _lParam) {                                     \
    (void)_wnd;                                                                \
    (void)_msg;                                                                \
    (void)_wParam;                                                             \
...
//!
//! @warning Возвращаемая строка - статическая, и обновляется при каждом вызове
//! функции. Если txInputBox() будет
//!          вызываться несколько раз, то для сохранения строки ее необходимо
//!          копировать в другую строку при помощи <i>strcpy()</i>.
//!
//! @see     txDialog, TX_BEGIN_MESSAGE_MAP, TX_BEGIN_COMMAND_MAP, TX_HANDLE,
//! TX_END_MESSAGE_MAP
//!
//! @usage @code
//!          const char* name = txInputBox ("So what's ur name?!?!", "System",
//!          "Sorry I'm Vasya Pupkin"); txMessageBox (name, "Aaand nooowww.. the
//!          winner iiis...");
//! @endcode
//}----------------------------------------------------------------------------------------------------------------

const char *txInputBox(const char *text = NULL, const char *caption = NULL,
                       const char *input = NULL);

const char *txInputBox(const char *text, const char *caption,
                       const char *input) {
  //-------------------------------------------------------------------------------------------------------------
  // Если не указаны параметры, приходится использовать хоть какие-то надписи.
  // txGetModuleFileName() - имя EXE-файла, на случай, если кое-кто поленился
  // задать название.
  //-------------------------------------------------------------------------------------------------------------
  ...
  if (!text)
    text = "Введите строку:";
  if (!caption)
    caption = txGetModuleFileName(false);
  if (!input)
    input = "";

    //-------------------------------------------------------------------------------------------------------------
    // Идентификаторы элементов диалога. Они требуются в GetDlgItemText().
    // Если диалог строится не вручную, а редактором ресурсов, то они задаются в
    // нем автоматически. У нас же тут - хардкор стайл, к сожалению. Причина в
    // том, что у разных сред программирования разные редакторы ресурсов и
    // системы сборки. Поэтому для независимости от них все будет строиться на
    // этапе выполнения, динамически. Вы еще гляньте, как это реализовано в
...
#define ID_INPUT_ 102
  ...
  // С помощью особого порядка полей в структуре txDialog::Layout и констант из
  // класса txDialog этот массив становится похож на описание ресурса диалога в
  // .rc - файле. См. описание синтаксиса rc-файла в документации по Win32
  // (MSDN, http://msdn.com).
  //-------------------------------------------------------------------------------------------------------------
  ...
  txDialog::Layout layout[] =
      ...
      //----------------------+----------+-----------+-----------------+---------------------------------------------
      //     Тип элемента     | Имя      | Иденти-   |   Координаты    | Флаги
      //     элементов диалога          | элемента | фикатор |-----------------|
      //     (см. описание элементов
      //                      |          | элемента  | X | Y |Шир.|Выс.| окон
      //                      диалога в MSDN)
      //----------------------+----------+-----------+---+---+----+----+---------------------------------------------
      //                      |          |           |   |   |    |    |
      {{txDialog::DIALOG, caption, 0, 0, 0, 240, 85},
       {txDialog::STATIC, text, ID_TEXT_, 10, 10, 150, 40, SS_LEFT},
       {txDialog::EDIT, input, ID_INPUT_, 10, 60, 220, 15,
        ES_LEFT | WS_BORDER | ES_AUTOHSCROLL | WS_TABSTOP},
       {txDialog::BUTTON, "&OK", IDOK, 180, 10, 50, 15,
        BS_DEFPUSHBUTTON | WS_TABSTOP},
       {txDialog::BUTTON, "&Cancel", IDCANCEL, 180, 30, 50, 15,
        BS_PUSHBUTTON | WS_TABSTOP},
       {txDialog::END}};

  //-------------------------------------------------------------------------------------------------------------
  // Класс диалога для InputBox. Внутренний, т.к. зачем ему быть внешним.
  // Нужен в основном для задания строки ввода (str) и оконной функции
  // диалогового окна, требуемой Win32 (она построена макросами
  // TX_BEGIN_MESSAGE_MAP и другими). Можно не делать внутреннего класса, но
  // тогда оконную функцию придется писать в глобальной области видимости, и str
  // объявлять глобально тоже (или передавать ее адрес через DialogBoxParam и
  // записывать его в класс во время обработки WM_INITDIALOG).
  //-------------------------------------------------------------------------------------------------------------
  struct inputDlg : txDialog {
    ...
    TX_BEGIN_MESSAGE_MAP() // Карта сообщений. На самом деле это начало оконной
                           // функции.

    TX_COMMAND_MAP // Здесь обрабатываются WM_COMMAND. На самом деле это switch.

        //-------------------------------------------------------------------------------------------------
        // При нажатии кнопки OK копируем строку из поля ввода в нашу переменную
        // str, т.к. после закрытия диалога строка ввода умрет и текст уже из
        // нее получить. Этот макрос на самом деле превращается в case из
        // оператора switch. _wnd - это параметр оконной функции, см.
        // определение макроса TX_BEGIN_MESSAGE_MAP().
        //-------------------------------------------------------------------------------------------------

            TX_HANDLE(IDOK)
                GetDlgItemText(_wnd, ID_INPUT_, str, sizeof(str) - 1);
  ...
  // От этого они получаются "локального действия", как будто у них была бы
  // область видимости - функция. На самом деле это сделано вручную через
  // #undef. Чтобы подчеркнуть их локальную природу, у них имена заканчиваются
  // на _. Такие дейфайны потом не перекосячат весь код после того как,
  // фактически, стали уже не нужны.
  //-------------------------------------------------------------------------------------------------------------
...
#undef ID_INPUT_
  ...
  //-------------------------------------------------------------------------------------------------------------
  // Это статический объект, потому что строка в нем должна жить после
  // завершения функции.
  //-------------------------------------------------------------------------------------------------------------
  ...
  //-------------------------------------------------------------------------------------------------------------
  // Возвращаем адрес строки из статического объекта. Так можно делать, потому
  // что статический объект не умрет при выходе из функции и строка в нем,
  // соответственно, тоже. Адрес нестатических переменных передавать
...
LRESULT CALLBACK _txCanvas_WndProc(HWND wnd, UINT msg, WPARAM wpar,
                                   LPARAM lpar);
...
bool _txCreateShortcut(const char shortcutName[], const char fileToLink[],
                       const char args[] = NULL, const char workDir[] = NULL,
                       const char description[] = NULL,
                       int cmdShow = SW_SHOWNORMAL,
                       const char iconFile[] = NULL, int iconIndex = 0,
                       int fontSize = 0, COORD bufSize = ZERO(COORD),
                       COORD wndSize = ZERO(COORD), COORD wndOrg = ZERO(COORD));

void *_tx_DLGTEMPLATE_Create(void *globalMem, size_t bufsize, DWORD style,
                             DWORD exStyle, WORD controls, short x, short y,
                             short cx, short cy, const char caption[],
                             const char font[], WORD fontsize, HANDLE menu);

void *_tx_DLGTEMPLATE_Add(void *dlgTemplatePtr, size_t bufsize, DWORD style,
                          DWORD exStyle, short x, short y, short cx, short cy,
                          WORD id, const char wclass[], const char caption[]);

const char *_txError(const char file[] = NULL, int line = 0,
                     const char func[] = NULL, const char msg[] = NULL, ...)
    _TX_CHECK_FORMAT(4);
...
void _txOnPureCall();
void _txOnInvalidParameter(const wchar_t *expr, const wchar_t *func,
                           const wchar_t *file, unsigned line, uintptr_t);

FARPROC _txDllImport(const char dllFileName[], const char funcName[],
                     bool required = true);
...
#if !defined(NDEBUG)
#define _TX_IF_ARGUMENT_FAILED(cond)                                           \
  if (!assert(cond) && (SetLastErrorEx(ERROR_BAD_ARGUMENTS, 0), 1))

#define _TX_IF_TXWINDOW_FAILED                                                 \
  if (!txOK() && (SetLastErrorEx(ERROR_INVALID_DATA, 0), 1) &&                 \
      (TX_ERROR("\a"                                                           \
                "Окно рисования не создано или не в порядке"),                 \
       1))
#else
#define _TX_IF_ARGUMENT_FAILED(cond)                                           \
  if (!(cond) && (SetLastErrorEx(ERROR_BAD_ARGUMENTS, 0), 1))
...
// Definition ({ expr; }) would be better, but some old dinosaurs (yes it is
// MSVC 6) reject it. So sad. :'(
...
// This is a macro because cond is an expression and not always a function. Lack
// of lambdas in pre-C++0x.

#define _txWaitFor(cond, time)                                                 \
  {                                                                            \
    for (DWORD _t = GetTickCount() + (time); !(cond) && GetTickCount() < _t;   \
...
FARPROC _txDllImport(const char dllFileName[], const char funcName[],
  ...
  if (!dll && dllFileName)
    dll = LoadLibrary(dllFileName);
  if (!dll && required)
    TX_ERROR("\a"
             "Cannot load library \"%s\"" _ dllFileName);
  ...
  if (!addr && required)
    TX_ERROR(
        "\a"
        "Cannot import \"%s\" from library \"%s\"" _ funcName _ dllFileName);
...
#define _TX_DLLIMPORT(lib, retval, name, params)                               \
  retval(WINAPI *name) params =                                                \
      (retval(WINAPI *) params)_txDllImport(lib ".dll", #name, true)

#define _TX_DLLIMPORT_OPT(lib, retval, name, params)                           \
  retval(WINAPI *name) params =                                                \
      (retval(WINAPI *) params)_txDllImport(lib ".dll", #name, false)

//-----------------------------------------------------------------------------------------------------------------
// Some IDEs don't link with these libs by default in console projects, so do
// sunrise by hand. :(
//{ <tears>
...
_TX_DLLIMPORT("GDI32", HBITMAP, CreateCompatibleBitmap,
...
_TX_DLLIMPORT("GDI32", int, GetObjectA,
              (HGDIOBJ obj, int bufsize, LPVOID buffer));
...
_TX_DLLIMPORT("GDI32", HFONT, CreateFontA,
              (int height, int width, int escapement, int orientation,
               int weight, DWORD italic, DWORD underline, DWORD strikeout,
               DWORD charSet, DWORD outputPrec, DWORD clipPrec, DWORD quality,
               DWORD pitchAndFamily, LPCTSTR face));
_TX_DLLIMPORT("GDI32", int, EnumFontFamiliesExA,
              (HDC dc, LPLOGFONT logFont, FONTENUMPROC enumProc, LPARAM lParam,
               DWORD reserved));
_TX_DLLIMPORT("GDI32", COLORREF, SetPixel,
              (HDC dc, int x, int y, COLORREF color));
_TX_DLLIMPORT("GDI32", COLORREF, GetPixel, (HDC dc, int x, int y));
_TX_DLLIMPORT("GDI32", HPEN, CreatePen,
              (int penStyle, int width, COLORREF color));
...
_TX_DLLIMPORT("GDI32", BOOL, Polygon,
              (HDC dc, CONST POINT points[], int count));
_TX_DLLIMPORT("GDI32", BOOL, Rectangle,
              (HDC dc, int x0, int y0, int x1, int y1));
_TX_DLLIMPORT("GDI32", BOOL, RoundRect,
              (HDC dc, int x0, int y0, int x1, int y1, int sizeX, int sizeY));
_TX_DLLIMPORT("GDI32", BOOL, Ellipse, (HDC dc, int x0, int y0, int x1, int y1));
_TX_DLLIMPORT("GDI32", BOOL, Arc,
              (HDC dc, int x0, int y0, int x1, int y1, int xStart, int yStart,
               int xEnd, int yEnd));
_TX_DLLIMPORT("GDI32", BOOL, Pie,
              (HDC dc, int x0, int y0, int x1, int y1, int xStart, int yStart,
               int xEnd, int yEnd));
_TX_DLLIMPORT("GDI32", BOOL, Chord,
              (HDC dc, int x0, int y0, int x1, int y1, int xStart, int yStart,
               int xEnd, int yEnd));
_TX_DLLIMPORT("GDI32", BOOL, TextOutA,
              (HDC dc, int x, int y, LPCTSTR string, int length));
_TX_DLLIMPORT("GDI32", UINT, SetTextAlign, (HDC dc, UINT mode));
_TX_DLLIMPORT("GDI32", BOOL, GetTextExtentPoint32A,
              (HDC dc, LPCTSTR string, int length, LPSIZE size));
_TX_DLLIMPORT("GDI32", BOOL, ExtFloodFill,
              (HDC dc, int x, int y, COLORREF color, UINT type));
_TX_DLLIMPORT("GDI32", BOOL, BitBlt,
              (HDC dest, int xDest, int yDest, int width, int height, HDC src,
               int xSrc, int ySrc, DWORD rOp));
_TX_DLLIMPORT("GDI32", int, SetDIBitsToDevice,
              (HDC dc, int xDest, int yDest, DWORD width, DWORD height,
               int xSrc, int ySrc, UINT startLine, UINT numLines,
               const VOID *data, const BITMAPINFO *info, UINT colorUse));
_TX_DLLIMPORT("GDI32", int, GetDIBits,
              (HDC hdc, HBITMAP hbmp, UINT uStartScan, UINT cScanLines,
               LPVOID lpvBits, LPBITMAPINFO lpbi, UINT usage));
_TX_DLLIMPORT("GDI32", BOOL, PatBlt,
              (HDC dc, int x0, int y0, int width, int height, DWORD rOp));
_TX_DLLIMPORT("GDI32", int, SetROP2, (HDC dc, int mode));
_TX_DLLIMPORT("GDI32", HRGN, CreateRectRgn, (int x0, int y0, int x1, int y1));
_TX_DLLIMPORT("GDI32", BOOL, GetBitmapDimensionEx,
              (HBITMAP bitmap, LPSIZE dimensions));
_TX_DLLIMPORT("User32", int, DrawTextA,
              (HDC dc, LPCTSTR text, int length, LPRECT rect, UINT format));
_TX_DLLIMPORT("User32", HANDLE, LoadImageA,
              (HINSTANCE inst, LPCTSTR name, UINT type, int sizex, int sizey,
               UINT mode));
_TX_DLLIMPORT("WinMM", BOOL, PlaySound,
              (LPCSTR sound, HMODULE mod, DWORD mode));

_TX_DLLIMPORT_OPT("MSImg32", BOOL, TransparentBlt,
                  (HDC dest, int destX, int destY, int destWidth,
                   int destHeight, HDC src, int srcX, int srcY, int srcWidth,
                   int srcHeight, UINT transparentColor));
_TX_DLLIMPORT_OPT("MSImg32", BOOL, AlphaBlend,
                  (HDC dest, int destX, int destY, int destWidth,
                   int destHeight, HDC src, int srcX, int srcY, int srcWidth,
                   int srcHeight, BLENDFUNCTION blending));

_TX_DLLIMPORT("Kernel32", HWND, GetConsoleWindow, (void));
_TX_DLLIMPORT_OPT("Kernel32", BOOL, SetConsoleFont,
                  (HANDLE con, DWORD fontIndex));
_TX_DLLIMPORT_OPT("Kernel32", BOOL, GetConsoleFontInfo,
                  (HANDLE con, BOOL fullScr, DWORD nFonts,
                   CONSOLE_FONT_INFO *info));
_TX_DLLIMPORT_OPT("Kernel32", DWORD, GetNumberOfConsoleFonts, (void));
_TX_DLLIMPORT_OPT("Kernel32", BOOL, GetCurrentConsoleFont,
                  (HANDLE con, BOOL maxWnd, CONSOLE_FONT_INFO *curFont));
_TX_DLLIMPORT_OPT("Kernel32", BOOL, GetCurrentConsoleFontEx,
                  (HANDLE con, BOOL maxWnd, CONSOLE_FONT_INFOEX *curFont));
_TX_DLLIMPORT_OPT("Kernel32", BOOL, SetCurrentConsoleFontEx,
                  (HANDLE con, BOOL maxWnd, CONSOLE_FONT_INFOEX *curFont));

_TX_DLLIMPORT("OLE32", HRESULT, CoInitialize, (void *));
_TX_DLLIMPORT("OLE32", HRESULT, CoCreateInstance,
              (REFCLSID clsid, LPUNKNOWN, DWORD, REFIID iid, PVOID *value));
_TX_DLLIMPORT("OLE32", void, CoUninitialize, (void));

_TX_DLLIMPORT("Shell32", HINSTANCE, ShellExecuteA,
              (HWND wnd, LPCTSTR operation, LPCTSTR file, LPCTSTR parameters,
               LPCTSTR directory, INT showCmd));

_TX_DLLIMPORT_OPT("NTDLL", char *, wine_get_version, ());

//-----------------------------------------------------------------------------------------------------------------
// Another issue, some of structs, consts and interfaces aren't defined in MinGW
// some ealry headers. Copied from Windows SDK 7.0a.
...
const GUID IID_IShellLink = {0x000214ee,
                             0x0000,
                             0x0000,
...
//
//           Данные не упакованы в структуру или класс, для того, чтобы это
//           сделали Вы сами :)
//
//           Если вы пишете свою библиотеку и используете TXLib.h как примеру,
//           не следуйте ему и не делайте так же. Здесь это сделано только в
//           образовательных целях.
//
//           Будьте практичнее, сделайте структуру и глобальную функцию для
//           доступа к ней.
//=================================================================================================================
//! @{
...
HANDLE _txCanvas_Thread = NULL;  // Дексриптор этого потока
HWND _txCanvas_Window = NULL;    // Дескриптор окна холста TXLib

UINT _txCanvas_RefreshTimer = 0; // Timer to redraw TXLib window
int _txCanvas_RefreshLock =
    0; // Blocks auto on-timer canvas update, see txBegin/txEnd

HDC _txCanvas_BackBuf[2] = {
    NULL,  // [0] Main working TXLib memory DC, where user picture lies
    NULL}; // [1] Image ready for auto-refresh, see txCanvas_OnPAINT()
CRITICAL_SECTION _txCanvas_LockBackBuf = {
    0, -1}; // Prevent simultaneous access to back buffer, see txLock()

std::vector<HDC> _txCanvas_UserDCs; // List of DCs allocated, for auto-free

bool _txConsole_IsBlinking =
    true; // To blink or not to blink, that is the question.

bool _txRunning = false; // main() is still running
bool _txExit = false;    // exit() is active
bool _txMain =
    false; // First TXLib window opened (closing it terminates the process)
...
WNDPROC _txAltWndProc =
    NULL; // Альтернативная оконная функция. См. txSetWindowsHook().

const char *_txFile = NULL; // Current execution point tracking, see $ macro
int _txLine = 0;
const char *_txFunc = NULL;
int _txInTX = 0; // We are inside one of TXLib's functions
...
int _txInitialize() {
  _TX_ON_DEBUG(OutputDebugString("\n"); OutputDebugString(
                   _TX_VERSION " - The Dumb Artist Library, " _TX_AUTHOR
                               ": \"" __FILE__ "\" "
                               "compiled " __DATE__ " " __TIME__
                               ", " _TX_BUILDMODE " mode: " _TX_MODULE "\n");
               OutputDebugString("\n"));
...
#if defined(_MSC_VER) && (_MSC_VER >= 1500) // MSVC 9 (2008) or greater
    $ _set_purecall_handler(_txOnPureCall);
    $ _set_invalid_parameter_handler(_txOnInvalidParameter);
#endif
  ...
  $(void)
  Win32::SetDIBitsToDevice; // Just to suppress "defined but not used" warning
...
HWND txCreateWindow(double sizeX, double sizeY, bool centered /*= true*/) {
  ...
  $ unsigned id = 0;
  $ _txCanvas_Thread =
      (HANDLE)_beginthreadex(NULL, 0, _txCanvas_ThreadProc, &size, 0, &id);
#else
  $ DWORD id = 0;
  $ _txCanvas_Thread = CreateThread(
      NULL, 0, (PTHREAD_START_ROUTINE)_txCanvas_ThreadProc, &size, 0, &id);
#endif

  $ if (!_txCanvas_Thread) return TX_DEBUG_ERROR("\a"
                                                 "Cannot start canvas thread."),
      (HWND)NULL;

  $ _txWaitFor(_txRunning, 30 * _TX_TIMEOUT);

  $ if (!_txRunning) return TX_DEBUG_ERROR("\a"
                                           "Cannot create canvas window."),
      (HWND)NULL;
  $ if (!txOK()) return TX_DEBUG_ERROR("\a"
                                       "Canvas window is not OK."),
      (HWND)NULL;
...
bool txSetDefaults() {
  ...
  $ HGDIOBJ font =
      txFontExist(_txConsoleFont)
          ? Win32::CreateFont((int)(1.0 * szCanvas.cy / szTxt.cy),
                              (int)(1.0 * szCanvas.cx / szTxt.cx), 0, 0,
  ...
  //{ Scroll the console for text to go above top of window and don't mix with
  //graphics
  ...
  $ SMALL_RECT src = {0, 0, (short)(con.dwSize.X - 1),
                      (short)(con.dwSize.Y - 1)};
  ...
  || (ScrollConsoleScreenBuffer(out, &src, NULL, dest,
                                &fill), // Or scroll the buffer
...
inline bool txOK() { $1 return _txCanvas_OK(); }
...
void _txCleanup() {
  ...
  $ bool externTerm =
      (thread != _txMainThreadId && thread != _txCanvas_ThreadId);
  $ DWORD parent = 0;
  $ bool waitableParent = !externTerm && _txIsParentWaitable(&parent);

  $ if (_txMain && _txConsole) txSetConsoleAttr(0x07);

  $ if (_txMain && !externTerm && wnd != NULL) { $ _txSetFinishedText(wnd); }

  $ if (((canvas && _txMain) || (!canvas && _txConsole && !waitableParent)) &&
        !_txExit && thread == _txMainThreadId) {
    $ if (wnd) {
  ...
  $ if (txWindow()) { $ SendNotifyMessage(txWindow(), WM_DESTROY, 0, 0); }

  $ _txWaitFor(!txWindow(), 5 * _TX_TIMEOUT);

  $ if (!txWindow()) {
    $ DeleteCriticalSection(&_txCanvas_LockBackBuf);
    _txCanvas_LockBackBuf = ZERO(CRITICAL_SECTION);
  }

  $ if (_txCanvas_Thread) {
    $ CloseHandle(_txCanvas_Thread) asserted;
...
int _txSetFinishedText(HWND wnd) {
  struct tools {
    static LRESULT getWindowText(HWND window, wchar_t text[], int size) {
      ...
      $ return SendMessageTimeoutW(window, WM_GETTEXT, (WPARAM)size,
                                   (LPARAM)text, SMTO_BLOCK | SMTO_ABORTIFHUNG,
                                   _TX_TIMEOUT, NULL);
    }

    static LRESULT setWindowText(HWND window, wchar_t text[]) {
      $1 return SendMessageTimeoutW(window, WM_SETTEXT, 0, (LPARAM)text,
                                    SMTO_BLOCK | SMTO_ABORTIFHUNG, _TX_TIMEOUT,
                                    NULL);
    }
  };
  ...
  $ MultiByteToWideChar(_TX_CP, 0, " [ЗАВЕРШЕНО]", -1, title + len,
                        (int)SIZEARR(title) - len);

  $ tools::setWindowText(wnd, title);
  $ tools::getWindowText(wnd, title, _TX_BUFSIZE);
  $ if (title[len + 2] == /* 'З' */ (wchar_t)0x0417) return 0;

  $ MultiByteToWideChar(_TX_CP, 0, " [FINISHED]", -1, title + len,
                        (int)SIZEARR(title) - len);
...
void _txPauseBeforeTermination(HWND canvas) {
  ...
  $ for (int i = 1;; i++) {
    ...
    if (!kbWait || (kbRedir && !canvas)) {
      $ break;
    } // No need to run and hide

    if (!wine && _txGetInput() != EOF) {
      $ break;
    } // Somebody hit something.

    if (canvas && !_txCanvas_ThreadId) {
      $ break;
    } // There was a window, and now there is not.

    if (!wine && !(i % 100500))
      printf("\r"
             "[Так нажмите же какую-нибудь клавишу для моего завершения]");
  }
...
bool _txIsParentWaitable(DWORD *parentPID /*= NULL*/) {
  ...
  $ for (char *p = strtok_s(list, ", ", &ctx); p;
         p = strtok_s(NULL, ", ", &ctx)) {
    $ char *gp = strchr(p, ':');
    ...
    $ if (gp) {
      ...
      $ if (info) if (_stricmp(gp, info->szExeFile) ==
                      0) // Was &&, but OMG MSVC /analyze..
      {
        $ return true;
      }
    }
    else {
      $ if (_stricmp(p, parent) == 0) { $ return true; }
    }
  }
...
PROCESSENTRY32 *_txFindProcess(unsigned pid /*= GetCurrentProcessId()*/) {
  ...
  $ for (bool ok = !!Process32First(sshot, &info); ok;
         ok = !!Process32Next(sshot, &info)) if (info.th32ProcessID ==
                                                 pid) break;
...
bool _txInDll() {
  ...
  $ for (bool ok = !!Module32First(sshot, &mod); ok;
         ok = !!Module32Next(sshot, &mod)) {
    $ if (!mod.modBaseAddr) continue;

    $ IMAGE_DOS_HEADER *dosHdr = (IMAGE_DOS_HEADER *)mod.modBaseAddr;
    $ IMAGE_NT_HEADERS *ntHdr =
        (IMAGE_NT_HEADERS *)(mod.modBaseAddr + dosHdr->e_lfanew);
    ...
    $ if (In((BYTE *)(ptrdiff_t)_txInDll, mod.modBaseAddr,
             mod.modBaseAddr + mod.modBaseSize)) break;
  }
...
bool _txKillProcess(DWORD pid) {
  ...
  $ TOKEN_PRIVILEGES priv = {
      1, {{{luid.LowPart, luid.HighPart}, SE_PRIVILEGE_ENABLED}}};
  $ TOKEN_PRIVILEGES old = {0};

  $ DWORD oldSz = 0;
  $ AdjustTokenPrivileges(token, false, &priv, sizeof(priv), &old, &oldSz)
      asserted;
...
unsigned WINAPI _txCanvas_ThreadProc(void *data) {
  ...
  $ if (!txWindow()) return TX_DEBUG_ERROR("\a"
  ...
  $ SetClassLong_(wnd, GCL_HICON_,
                  (DWORD)(ptrdiff_t)(icon32 ? icon32 : _txCreateTXIcon(32)));
  $ SetClassLong_(wnd, GCL_HICONSM_,
                  (DWORD)(ptrdiff_t)(icon16 ? icon16 : _txCreateTXIcon(16)));
  ...
  $ if (icon16)
      DestroyIcon(icon16); // If Explorer is displaying Tray Notification, these
  $ if (icon32) DestroyIcon(
      icon32); // calls will possibly fail, and we'll get resource leak.
...
HWND _txCanvas_CreateWindow(SIZE *size) {
  ...
  $ if (!wndclass) return TX_DEBUG_ERROR(
      "RegisterClass (\"%s\") failed" _ className),
      (HWND)NULL;
  ...
  AdjustWindowRect(&r, _txWindowStyle, false);
  $ SIZE sz = {r.right - r.left, r.bottom - r.top};

  $ HWND wnd = CreateWindowEx(WS_EX_APPWINDOW, className,
                              txGetModuleFileName(false), _txWindowStyle,
                              centered ? scr.cx / 2 - sz.cx / 2 : CW_USEDEFAULT,
                              centered ? scr.cy / 2 - sz.cy / 2 : CW_USEDEFAULT,
                              sz.cx, sz.cy, NULL, NULL, NULL, NULL);

  $ if (!wnd || !txWindow()) return TX_DEBUG_ERROR(
      "Cannot create canvas: CreateWindowEx (\"%s\") failed" _ className),
      (HWND)NULL;
...
inline bool _txCanvas_OK() {
  $1 return _txCanvas_ThreadId && _txCanvas_Window && _txCanvas_BackBuf[0] &&
      _txCanvas_BackBuf[1];
}
...
int _txCanvas_SetRefreshLock(int count) {
  ...
  $ if (_txCanvas_RefreshLock <= 0 || oldCount <= 0) if (txWindow()) {
...
HICON _txCreateTXIcon(int size) {
  ...
  $ const char image32[32 * 32 + 1] = "00000000000000000000000000000000"
                                      "0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0"
                                      "0F0000000000000000000000000000F0"
                                      "0F0000000000000000000000000000F0"
                                      "0F0000000000000099999999999900F0"
                                      "0F0000000000000090300333330900F0"
                                      "0F0000000990000090000000000900F0"
                                      "0F00000099990000900BB000000900F0"
                                      "0F0000039999000090B00090900900F0"
                                      "0F0000009999000090B00999990900F0"
                                      "0F00000009903799900BB090900900F0"
                                      "0F000000009BB70090000010000900F0"
                                      "0F0000000B90000090000000000900F0"
                                      "0F000000B0B0000099999999999900F0"
                                      "0F00007B30B0000090000000000000F0"
                                      "0F00007300B0000090000000000000F0"
                                      "0F00000000B3000090000000000000F0"
                                      "0F0000000B0B000090000000000000F0"
                                      "0F000000B303B00090000000000000F0"
                                      "0F000003B000B00090000000000000F0"
                                      "0F00003B00003B0090000000000000F0"
                                      "0F0000300000030090000000000000F0"
                                      "0F0000000448888888888844000000F0"
                                      "0F00004886E6E6E60E66E6EEEE4400F0"
                                      "0F4488866E0E60E00660E06E66EEE4F0"
                                      "0F868806E06E06E666E66E00E06EE6F0"
                                      "0F08606E66E0066000E006E66E00E6F0"
                                      "0F8666E006600E00006600E006E00EF0"
                                      "0F000E066888888888888888606660F0"
                                      "0F66EEE6EE000E00000E00086EEEE6F0"
                                      "0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0"
                                      "00000000000000000000000000000000";

  $ const char image16[16 * 16 + 1] = "0000000000000000"
                                      "0000000999999990"
                                      "0009000900000090"
                                      "0099900909973090"
                                      "0059700909009390"
  ...
  $ const COLORREF pal['F' - '0' + 1] = {
      0x000000, 0x002b2b, 0x555500, 0x005555, 0x808000, 0x008080,
      0xaaaa00, 0x00aaaa, 0xd5d500, 0x00d5d5, 0,        0,
      0,        0,        0,        0,        0,        0xffff00,
      0x00ffff, 0xffffaa, 0xaaffff, 0xd5d500, 0xffffff};

  $ const char *image = (size == 32) ? image32 : image16;

  $ POINT sz = {size, size};
  $ HDC dcMask = _txBuffer_Create(txWindow(), &sz);
  assert(dcMask);
  $ HDC dcColor = _txBuffer_Create(txWindow(), &sz);
  assert(dcColor);
  ...
  $ ICONINFO info = {true, 0, 0,
                     (HBITMAP)Win32::GetCurrentObject(dcMask, OBJ_BITMAP),
...
LRESULT CALLBACK _txCanvas_WndProc(HWND wnd, UINT msg, WPARAM wpar,
                                   LPARAM lpar) {
  $1 if (msg == WM_KEYDOWN && wpar == VK_F12 && GetKeyState(VK_SHIFT) &&
  ...
  WNDPROC altWndProc =
      _txAltWndProc; // Cache to prevent change from main thread
  if (altWndProc) {
  ...
  switch (msg) {
  case WM_CREATE:
    $ _txCanvas_OnCREATE(wnd);
    break;

  case WM_CLOSE:
    $ if (!_txCanvas_OnCLOSE(wnd)) return 0;
    break;
  case WM_DESTROY:
    $ _txCanvas_OnDESTROY(wnd);
    break;

  case WM_PAINT:
    $ _txCanvas_OnPAINT(wnd);
    break;
  case WM_TIMER:
    $ _txCanvas_OnTIMER(wnd, wpar);
    break;

  case WM_KEYDOWN:
    $ _txCanvas_OnKEYDOWN(wnd, wpar, lpar);
    break;
  case WM_CHAR:
    $ _txCanvas_OnCHAR(wnd, wpar, lpar);
    break;
  ...
  case WM_MBUTTONDOWN:
  case WM_MOUSEMOVE:
    $ _txCanvas_OnMOUSEMOVE(wnd, wpar, lpar);
    break;
  default:
    break;
  }
  ...
  if (msg == WM_SYSCOMMAND)
    ...
    case _TX_IDM_ABOUT:
      $ _txCanvas_OnCmdABOUT(wnd, wpar);
      break;
    case _TX_IDM_CONSOLE:
      $ _txCanvas_OnCmdCONSOLE(wnd, wpar);
      break;
    default:
      break;
    }
...
bool _txCanvas_OnCREATE(HWND wnd) {
  ...
  $ _txCanvas_BackBuf[0] = _txBuffer_Create(wnd);
  assert(_txCanvas_BackBuf[0]);
  $ _txCanvas_BackBuf[1] = _txBuffer_Create(wnd);
  assert(_txCanvas_BackBuf[1]);

  $ SetTimer(wnd, _txCanvas_RefreshTimer, _txWindowUpdateInterval, NULL)
      asserted;
...
bool _txCanvas_OnDESTROY(HWND wnd) {
  ...
  $ if (dcs.size()) {
    $ txNotifyIcon(NIIF_ERROR, NULL, "Вы забыли освободить %d HDC.",
                   (int)dcs.size());
    $ Sleep(_TX_TIMEOUT);
  }

  $ struct _txBuffer_Delete_Wrapper {
    static bool func(HDC dc) { $ return _txBuffer_Delete(&dc); }
  };
...
bool _txCanvas_OnCLOSE(HWND wnd) {
  ...
  $ if (_txMain && _txRunning &&
        txMessageBox("Функция main() не завершена. Программа все еще работает. "
                     "Прервать аварийно?    \n\n"
                     "Лучше подождать, когда main() завершится - это "
...
bool _txCanvas_OnPAINT(HWND wnd) {
  ...
  $ if ((_txCanvas_RefreshLock <= 0 || forceRedraw) && txLock(false)) {
    $ Win32::BitBlt(_txCanvas_BackBuf[1], 0, 0, size.x, size.y, txDC(), 0, 0,
                    SRCCOPY);
  ...
  // the refresh cannot be blocked IRL. Do not use 100501 because it may lead to
  // errors on some compilers and possible may crash the compilers themselves.
  // Yes guys, with all your software installed. :(

  $ if (_txCanvas_RefreshLock != 100500) {
    $ Win32::BitBlt(dc, 0, 0, size.x, size.y, _txCanvas_BackBuf[1], 0, 0,
                    SRCCOPY);
  }
...
bool _txCanvas_OnKEYDOWN(HWND, WPARAM vk, LPARAM info) {
  ...
  $ evt.Event.KeyEvent.uChar.AsciiChar =
      (unsigned char)MapVirtualKey((WORD)vk, 2); // 2 == MAPVK_VK_TO_CHAR
  $ evt.Event.KeyEvent.wVirtualScanCode = (unsigned char)(info >> 16);
  $ evt.Event.KeyEvent.wVirtualKeyCode = (WORD)vk;
  $ evt.Event.KeyEvent.dwControlKeyState =
      (info & (1 << 24)) ? ENHANCED_KEY : 0;

  $ if (evt.Event.KeyEvent.uChar
            .AsciiChar) return false; // Let TranslateMessage() and WM_CHAR do
                                      // the job
...
bool _txCanvas_OnCHAR(HWND, WPARAM ch, LPARAM info) {
  ...
  $ evt.Event.KeyEvent.wVirtualScanCode = (unsigned char)(info >> 16);
  $ evt.Event.KeyEvent.wVirtualKeyCode =
      (WORD)MapVirtualKey((WORD)(info >> 16), 3); // 3 == MAPVK_VSC_TO_VK_EX
...
bool _txCanvas_OnCmdCONSOLE(HWND wnd, WPARAM cmd) {
  ...
  $ CheckMenuItem(GetSystemMenu(wnd, false), (int)cmd,
...
#if defined(__MODULE) || defined(__VERSION) || defined(__DESCRIPTION) ||       \
    defined(__AUTHOR)
...
#define __AUTHOR                                                               \
  ...
  $ _snprintf_s(
      text, sizeof(text) - 1 _TX_TRUNCATE,
    ...
    defined(__AUTHOR)
      ...
      "#define __MODULE to \"your program name\" before including TXLib.h to "
      "use this billboard...\n"
#endif
      ...
      "\t" __FILE__ "\n"
      "Compiled:"
      "\t" __DATE__ " " __TIME__ ", " _TX_BUILDMODE ", " __TX_COMPILER__ "\n"
      "Started:"
      "\t"
      "%.6s %.4s %.8s\n\n"
...
HWND _txConsole_Attach() {
  ...
  // Минздрав РФ предупреждает: чрезмерное употребление wine вредит Вашему
  // здоровью.
  ...
  // Устанавливаем русскую кодовую страницу для стандартной библиотеки, иначе не
  // будут работать Unicode-версии функций (wprintf, ...). Если компилите с
  // помощью gcc и собираетесь использовать L"unicode-строки" с русским языком,
  // укажите опции в командной строке компилятора g++: -finput-charset=CP1251
  // -fexec-charset=CP1251.

  $ setlocale(LC_CTYPE, _TX_LC_CTYPE); //  "Russian"
  $ if (!Win32::wine_get_version)
      _wsetlocale(LC_CTYPE, _TX_LC_CTYPE_W); // L"Russian_Russia.ACP"
  ...
  $ if (!Win32::wine_get_version) { $ _txConsole_SetUnicodeFont(); }

#ifndef _CONSOLE

  // Переоткрываем заново <s>Америку</s> потоки ввода-вывода, если subsystem !=
  // console

  $ *stdin =
      *_fdopen(_open_osfhandle((DWORD)(ptrdiff_t)GetStdHandle(STD_INPUT_HANDLE),
                               _O_TEXT),
               "r");
  $ fflush(stdout);
  *stdout =
      *_fdopen(_open_osfhandle(
                   (DWORD)(ptrdiff_t)GetStdHandle(STD_OUTPUT_HANDLE), _O_TEXT),
               "w");
  $ fflush(stderr);
  *stderr =
      *_fdopen(_open_osfhandle((DWORD)(ptrdiff_t)GetStdHandle(STD_ERROR_HANDLE),
                               _O_TEXT),
               "w");
...
inline bool _txConsole_OK() { $1 return Win32::GetConsoleWindow() != NULL; }
...
bool _txConsole_Draw(HDC dc) {
  ...
  $ COLORREF pal[16] = {0x000000, 0x800000, 0x008000, 0x808000,
                        0x000080, 0x800080, 0x008080, 0xC0C0C0,
                        0x808080, 0xFF0000, 0x00FF00, 0xFFFF00,
                        0x0000FF, 0xFF00FF, 0x00FFFF, 0xFFFFFF};

  $ for (int y = 0; y < size.y; y++) {
    static char chr[_TX_BUFSIZE + 1] =
        ""; // [con.dwSize.X + 1]; maybe will be truncated
    static WORD atr[_TX_BUFSIZE + 1] = {
        0}; // [con.dwSize.X + 1]; maybe will be truncated
    COORD coord = {(short)(con.srWindow.Left), (short)(y + con.srWindow.Top)};
    DWORD read = 0;

    if (!ReadConsoleOutputCharacter(out, chr, SIZEARR(chr) - 1, coord, &read))
      continue;
    if (!ReadConsoleOutputAttribute(out, atr, SIZEARR(atr) - 1, coord, &read))
      continue;
  ...
  $ if (_txConsole_IsBlinking && In(con.dwCursorPosition, con.srWindow) &&
        GetTickCount() % _txCursorBlinkInterval * 2 > _txCursorBlinkInterval &&
        GetForegroundWindow() == txWindow()) {
    $ Win32::TextOut(
        dc, ROUND(fontSz.cx * (con.dwCursorPosition.X - con.srWindow.Left)),
        ROUND(fontSz.cy * (con.dwCursorPosition.Y - con.srWindow.Top)) + 1, "_",
        1) asserted;
  }
...
bool _txConsole_SetUnicodeFont() {
  ...
  $1 if (Win32::GetCurrentConsoleFontEx && Win32::SetCurrentConsoleFontEx) {
    ...
    $ if (!*info.FaceName) info.dwFontSize.Y =
        (SHORT)(info.dwFontSize.Y + 2); // Terminal font is too small
  ...
  $ const unsigned uniFont =
      10; // The Internet and W2K sources know this magic number
  $ const unsigned uniSize =
      20; // Size of the font desired, should be > max of Raster Fonts
  $ bool ok = true;
  ...
  $ if (_TX_SAFECALL(Win32::GetNumberOfConsoleFonts)() <= uniFont) {
    ...
    $ ok = (Win32::ShellExecuteA(NULL, NULL, link, NULL, NULL,
                                 SW_SHOWMINNOACTIVE) > (void *)32);
    if (ok) {
      $ _txWaitFor(FindWindow(NULL, "~txLink"), _TX_TIMEOUT);
    }
  ...
  $ ok &= (cur.nFont >= uniFont) ||
          !!(_TX_SAFECALL(Win32::SetConsoleFont)(out, uniFont));
...
#define _TX_TRY                                                                \
  ...
  __tx_try : {                                                                 \
    int __tx_error = S_OK;                                                     \
    (void)__tx_error;
#define _TX_CHECKED(cmd)                                                       \
  {                                                                            \
    if (FAILED(__tx_error = (cmd)))                                            \
      goto __tx_catch;                                                         \
  }
#define _TX_FAIL                                                               \
  {                                                                            \
...
bool _txCreateShortcut(const char shortcutName[], const char fileToLink[],
                       ...
                       const char iconFile[] /*= NULL*/, int iconIndex /*= 0*/,
                       int fontSize /*= 0*/, COORD bufSize /*= ZERO (COORD)*/,
                       COORD wndSize /*= ZERO (COORD)*/,
                       COORD wndOrg /*=ZERO (COORD)*/) {
  ...
  _TX_TRY {
    $ _TX_CHECKED(
        Win32::CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER,
                                Win32::IID_IShellLink, (void **)&shellLink));
    ...
    $ _TX_CHECKED(shellLink->QueryInterface(Win32::IID_IShellLinkDataList,
                                            (void **)&dataList));
    ...
    $ Win32::NT_CONSOLE_PROPS props = {
        ...
        0x07,
        0xF5,                   // wFillAttribute, wPopupFillAttribute
        ...
        0x36,
        400,
        L"Lucida Console", // uFontFamily, uFontWeight, FaceName. We're dancing
                           // for this!
        15,                // uCursorSize
        0,
        1,
        1,
        0, // bFullScreen, bQuickEdit, bInsertMode, bAutoPosition
        50,
        4,
        0, // uHistoryBufferSize, uNumberOfHistoryBuffers, bHistoryNoDup
    ...
    $ _TX_CHECKED(
        shellLink->QueryInterface(Win32::IID_IPersistFile, (void **)&file));
    $ if (!file) _TX_FAIL;
...
HDC _txBuffer_Create(HWND wnd, const POINT *size, HBITMAP bitmap) {
  ...
  $ if (!(Win32::GetDeviceCaps(wndDC, RASTERCAPS) & RC_BITBLT))
      TX_DEBUG_ERROR("RC_BITBLT not supported");
  ...
  $ if (!dc)
      TX_DEBUG_ERROR("Cannot create buffer: CreateCompatibleDC() failed");

  $ HBITMAP bmap =
      bitmap ? bitmap : Win32::CreateCompatibleBitmap(wndDC, size->x, size->y);
  $ if (!bmap)
      TX_DEBUG_ERROR("Cannot create buffer: CreateCompatibleBitmap() failed");
...
bool _txBuffer_Delete(HDC *dc) {
  ...
  $ if (!Win32::GetObjectType(
            Win32::GetCurrentObject(*dc, OBJ_BITMAP))) return false;
...
#define GET_DESCR_(str, code, descr)                                           \
  case (code): {                                                               \
    (str) = #code ": " descr;                                                  \
    break;                                                                     \
  }
  ...
  switch (sig) {
    ...
    GET_DESCR_(sSig, SIGILL,
               "Попытка выполнить недопустимую операцию. Проверьте указатели "
               "на функции.")
    GET_DESCR_(sSig, SIGABRT,
               "Аварийное завершение программы, вызвана функция abort().")
    GET_DESCR_(sSig, SIGTERM,
               "Получен сигнал принудительного завершения программы.")
    GET_DESCR_(sSig, SIGFPE,
               "Грубая ошибка в вычислениях, деление на 0 или что-нибудь еще")
  default:
    break;
  }
  ...
  if (sig == SIGFPE)
    switch (fpe) {
      ...
      GET_DESCR_(sFPE, _FPE_STACKUNDERFLOW,
                 "В стеке сопроцессора не хватает данных")
...
#undef GET_DESCR_
  ...
  if (sig == SIGFPE && fpe)
    _txError(NULL, 0, NULL,
             "signal (%d, 0x%02X): %s, %s." _ sig _ fpe _ sSig _ sFPE);
  else
...
void _txOnUnexpected() {
  _txError(NULL, 0, NULL,
           "std::unexpected(): Необработанное исключение. Проверьте свои "
           "catch-блоки. Перехватите catch (...).    \n\n"
           "Если вы (зря) используете спецификацию исключений для функций, "
           "проверьте, не нарушена ли она.");
}
...
void _txOnTerminate() {
  ...
  _txError(NULL, 0, NULL,
           "std::terminate(): Программа будет завершена из-за неперехваченного "
           "исключения в функции main() или в деструкторе.    \n\n"
           "Используйте try/catch блоки, перехватывайте catch (...), "
...
void _txOnPureCall() {
  _txError(NULL, 0, NULL,
           "Поздравляем, вам удалось вызвать чисто виртуальную функцию, скорее "
           "всего в конструкторе или деструкторе.    \n\n"
           "Программа будет завершена.");
}

//-----------------------------------------------------------------------------------------------------------------

void _txOnInvalidParameter(const wchar_t *exprW, const wchar_t *funcW,
                           const wchar_t *fileW, unsigned line, uintptr_t) {
  static char expr[_TX_BUFSIZE] = "";
  static char func[_TX_BUFSIZE] = "";
  static char file[_TX_BUFSIZE] = "";

  if (exprW)
    WideCharToMultiByte(_TX_CP, 0, exprW, -1, expr, SIZEARR(expr), NULL, NULL);
  if (funcW)
    WideCharToMultiByte(_TX_CP, 0, funcW, -1, func, SIZEARR(func), NULL, NULL);
  if (fileW)
    WideCharToMultiByte(_TX_CP, 0, fileW, -1, file, SIZEARR(file), NULL, NULL);

  _txError((fileW ? file : NULL), line, (funcW ? func : NULL),
           (exprW ? "Неверный параметр функции. Проверьте, что %s."
...
const char *_txError(const char file[] /*= NULL*/, int line /*= 0*/,
  ...
  bool except = std::uncaught_exception();
  char *excStr = NULL;
  char *excWTF = NULL; // What a Terrible Failure
  DWORD winerr = GetLastError();
  unsigned threadId = GetCurrentThreadId();

#if defined(_GCC_VER) && (_GCC_VER >= 400)

  if (std::type_info *exception_type = abi::__cxa_current_exception_type()) {
    int err = 0;
    excStr = abi::__cxa_demangle(exception_type->name(), NULL, NULL, &err);
    if (!excStr)
      excStr = _strdup(exception_type->name());

    try {
      throw;
    } catch (std::exception &exc) {
      excWTF = _strdup(exc.what());
    } catch (...) {
      excWTF = _strdup("неизвестна");
    }
  }
...
#define SZARG_(n) sizeof(what) - 1 - (n) - (s - what) _TX_TRUNCATE
  ...
  if (file)
    s += _snprintf_s(s, SZARG_(1), "файл: %s, ", file);
  if (line)
    s += _snprintf_s(s, SZARG_(1), "строка: %d, ", line);
  if (func)
    s += _snprintf_s(s, SZARG_(1), "функция: %s.", func);
  s += _snprintf_s(s, SZARG_(1), "\n\n");

  if (msg)
    s += _snprintf_s(s, SZARG_(1),
                     "%s: ", (file || line || func) ? "Сообщение" : "ВНЕЗАПНО"),
        s += _vsnprintf_s(s, SZARG_(1), msg, arg),
        s += _snprintf_s(s, SZARG_(1), "\n\n");

  s += _snprintf_s(s, SZARG_(1), "#%d: %s: Thread: 0x%08X%s", nCalls,
                   _TX_VERSION, threadId,
                   (threadId == _txMainThreadId)      ? " (Main)"
                   : (threadId == _txCanvas_ThreadId) ? " (Canvas)"
                                                      : "");

  if (winerr)
    s += _snprintf_s(s, SZARG_(0), ", GetLastError(): %lu (", winerr),
        s += FormatMessage(
                 FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                 NULL, winerr, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), s,
                 (DWORD)(sizeof(what) - (s - what)), NULL) -
             2,
        s -= (s[-1] == '.') ? 1 : 0, s += _snprintf_s(s, SZARG_(1), ")");

  if (crterr)
    s += _snprintf_s(s, SZARG_(1), ", errno: %d (%s)", crterr,
                     (strerror_s(str, crterr), str));

  if (doserr)
    s += _snprintf_s(s, SZARG_(1), ", _doserrno: %d (%s)", doserr,
                     (strerror_s(str, doserr), str));

  if (except)
    s += _snprintf_s(s, SZARG_(1), ", исключение не перехвачено");

  if (excStr)
    s += _snprintf_s(s, SZARG_(1), ", исключение: %s", excStr);
  if (excWTF)
    s += _snprintf_s(s, SZARG_(1), ", причина: %s", excWTF);
...
#undef SZARG_
  ...
  //  vvvvvvvvvvvvvvvvvv
  DebugBreak(); //>>> Вы в отладчике. Есть шанс посмотреть переменные и
                //разобраться.
                //  ^^^^^^^^^^^^^^^^^^

  return what; //>>> Уходите из функции пошаговой отладкой (F10/F11). Следите за
               //стеком вызовов (Alt+7).
}
...
unsigned txMessageBox(const char *text, const char *header,
                      unsigned flags /*= 0*/) {
  ...
  if (text)
    MultiByteToWideChar(_TX_CP, 0, text, -1, textW, SIZEARR(textW));
  if (header)
    MultiByteToWideChar(_TX_CP, 0, header, -1, headerW, SIZEARR(headerW));

  int ret = MessageBoxW((txWindow() ? txWindow() : Win32::GetConsoleWindow()),
...
bool txNotifyIcon(unsigned flags, const char title[], const char format[],
  ...
  $ nid.hWnd = NULL;
  $ nid.uID = 1;
  $ nid.hIcon = _txCreateTXIcon(16);
  assert(nid.hIcon);
  $ strncpy_s(nid.szTip, "TXLib Information", sizeof(nid.szTip));
  $ strncpy_s(nid.szInfoTitle, (title ? title : "TXLib сообщает"),
              sizeof(nid.szInfoTitle) - 1);
  ...
  $ txOutputDebugPrintf(_TX_VERSION
                        " - Icon notification (NOT displayed): %s: %s\n",
                        title, nid_szInfo);
  $ ok = false;
...
inline const char *txVersion() { return _TX_VERSION; }

//-----------------------------------------------------------------------------------------------------------------

inline unsigned txVersionNumber() { return _TX_VER; }
...
int txGetExtentX() { $1 return txGetExtent().x; }

//-----------------------------------------------------------------------------------------------------------------

int txGetExtentY() { $1 return txGetExtent().y; }

//-----------------------------------------------------------------------------------------------------------------

inline HDC &txDC() { $1 return _txCanvas_BackBuf[0]; }

//-----------------------------------------------------------------------------------------------------------------

inline HWND txWindow() { $1 return _txCanvas_Window; }
...
bool txDestroyWindow() {
  ...
  $ if (SendNotifyMessage(txWindow(), _txMain ? WM_CLOSE : WM_DESTROY, 0, 0) ==
        0) return false;

  $ if (_txMain) {
    $ txNotifyIcon(NIIF_WARNING, NULL,
                   "\n"
                   "Очень, очень плохо завершать программу через "
                   "txDestroyWindow().    \n\n"
...
bool txSetColor(COLORREF color, double thickness /*= 1*/) {
  ...
  $ return _txBuffer_Select(
      Win32::CreatePen((color == TX_TRANSPARENT ? PS_NULL : PS_SOLID),
...
COLORREF txGetColor() {
  ...
  $ return (size == sizeof(LOGPEN)) ? buf.LogPen.lopnColor
                                    : buf.extLogPen.elpColor;
}
...
bool txSetFillColor(COLORREF color) {
  ...
  $ return _txBuffer_Select((color == TX_TRANSPARENT)
                                ? Win32::GetStockObject(HOLLOW_BRUSH)
...
inline bool txPixel(double x, double y, double red, double green, double blue) {
  ...
  $ return txSetPixel(
      x, y, RGB(ROUND(red * 255), ROUND(green * 255), ROUND(blue * 255)));
}
...
bool txRectangle(double x0, double y0, double x1, double y1) {
  ...
  $ txGDI((Win32::Rectangle(txDC(), ROUND(x0), ROUND(y0), ROUND(x1),
                            ROUND(y1)))) asserted;
...
bool txEllipse(double x0, double y0, double x1, double y1) {
  ...
  $ txGDI((Win32::Ellipse(txDC(), ROUND(x0), ROUND(y0), ROUND(x1), ROUND(y1))))
      asserted;
...
bool txArc(double x0, double y0, double x1, double y1, double startAngle,
  ...
  $ return txGDI(!!(Win32::Arc(
      txDC(), ROUND(x0), ROUND(y0), ROUND(x1), ROUND(y1),
...
bool txFloodFill(double x, double y, COLORREF color /*= TX_TRANSPARENT*/,
  ...
  $ return txGDI(
      !!(Win32::ExtFloodFill(txDC(), ROUND(x), ROUND(y), color, mode)));
}
...
bool txDrawText(
    double x0, double y0, double x1, double y1, const char text[],
    unsigned
        format /*= DT_CENTER | DT_VCENTER | DT_WORDBREAK | DT_WORD_ELLIPSIS*/) {
  $1 _TX_IF_TXWINDOW_FAILED return false;
  $ _TX_IF_ARGUMENT_FAILED(text) return false;
  ...
  $ if (Win32::DrawText) txGDI((Win32::DrawText(txDC(), text, -1, &r, format)))
      asserted;
...
bool txSelectFont(const char name[], double sizeY, double sizeX /*= -1*/,
  ...
  $ _txBuffer_Select(
      txFontExist(name)
          ? Win32::CreateFont(
                ROUND(sizeY), ROUND((sizeX >= 0) ? sizeX : sizeY / 3),
...
SIZE txGetTextExtent(const char text[]) {
  ...
  $ txGDI((Win32::GetTextExtentPoint32(txDC(), text, (int)strlen(text), &size)))
      asserted;
...
int txGetTextExtentX(const char text[]) { $1 return txGetTextExtent(text).cx; }

//-----------------------------------------------------------------------------------------------------------------

int txGetTextExtentY(const char text[]) { $1 return txGetTextExtent(text).cy; }
...
LOGFONT *txFontExist(const char name[]) {
  ...
  $ struct tools {
    static int CALLBACK enumFonts(const LOGFONT *fnt, const TEXTMETRIC *, DWORD,
                                  LPARAM data) {
      $ _TX_IF_ARGUMENT_FAILED(fnt) return 0;
      $ _TX_IF_ARGUMENT_FAILED(data) return 0;

#ifndef __STRICT_ANSI__
      $ return _strnicmp(fnt->lfFaceName, ((LOGFONT *)data)->lfFaceName,
                         LF_FACESIZE);

#else
      $ return strncmp(fnt->lfFaceName, ((LOGFONT *)data)->lfFaceName,
                       LF_FACESIZE);
  ...
  $ return txGDI((Win32::EnumFontFamiliesEx(txDC(), &font, tools::enumFonts,
                                            (LPARAM)&font, 0))) == 0
      ? &font
      : NULL;
}
...
HDC txCreateCompatibleDC(double sizeX, double sizeY,
                         HBITMAP bitmap /*= NULL*/) {
  $1 _TX_IF_TXWINDOW_FAILED return NULL;
...
HDC txLoadImage(const char filename[]) {
  ...
  $ HBITMAP image = (HBITMAP)Win32::LoadImage(NULL, filename, IMAGE_BITMAP, 0,
                                              0, LR_LOADFROMFILE);
  ...
  $ if ((long)(time - loadTimes[file]) < _TX_TIMEOUT) {
    $ txNotifyIcon(
        NIIF_WARNING, NULL,
        "Вы загружаете \"%s\" слишком часто, программа будет тормозить.",
        filename);
  }
...
bool txBitBlt(HDC dest, double xDest, double yDest, double width, double height,
  ...
  $ return txGDI(
      !!(Win32::BitBlt(dest, ROUND(xDest), ROUND(yDest), ROUND(width),
                       ROUND(height), src, ROUND(xSrc), ROUND(ySrc), rOp)));
}
...
bool txTransparentBlt(HDC dest, double xDest, double yDest, double width,
  ...
  $ return (Win32::TransparentBlt)
      ? txGDI(!!(Win32::TransparentBlt(
            dest, ROUND(xDest), ROUND(yDest), ROUND(width), ROUND(height), src,
            ROUND(xSrc), ROUND(ySrc), ROUND(width), ROUND(height), transColor)))
      : txGDI(!!(Win32::BitBlt(dest, ROUND(xDest), ROUND(yDest), ROUND(width),
                               ROUND(height), src, ROUND(xSrc), ROUND(ySrc),
                               SRCCOPY))),
      false;
}
...
bool txAlphaBlend(HDC dest, double xDest, double yDest, double width,
  ...
  $ BLENDFUNCTION blend = {AC_SRC_OVER, 0, (BYTE)ROUND(alpha * 255),
                           AC_SRC_ALPHA};

  $ return (Win32::AlphaBlend)
      ? txGDI(!!(Win32::AlphaBlend(
            dest, ROUND(xDest), ROUND(yDest), ROUND(width), ROUND(height), src,
            ROUND(xSrc), ROUND(ySrc), ROUND(width), ROUND(height), blend)))
      : txGDI(!!(Win32::BitBlt(dest, ROUND(xDest), ROUND(yDest), ROUND(width),
                               ROUND(height), src, ROUND(xSrc), ROUND(ySrc),
                               SRCCOPY))),
      false;
...
inline int txMouseX() { $1 return _txMousePos.x; }

//-----------------------------------------------------------------------------------------------------------------

inline int txMouseY() { $1 return _txMousePos.y; }
...
bool txSetConsoleAttr(unsigned color /*= 0x07*/) {
  $1 return !!SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                                      (WORD)color);
}
...
POINT txSetConsoleCursorPos(double x, double y) {
  ...
  $ POINT prev = {
      ROUND(1.0 * (con.dwCursorPosition.X - con.srWindow.Left) / fontSz.x),
...
bool txClearConsole() {
  ...
  $ DWORD written = 0;
  $ FillConsoleOutputCharacter(out, 0x20 /*' '*/, len, start, &written)
      asserted;
...
POINT txGetConsoleFontSize() {
  ...
  $(_TX_SAFECALL(Win32::GetCurrentConsoleFont)(GetStdHandle(STD_OUTPUT_HANDLE),
                                               false, &font))
  asserted;
...
WNDPROC txSetWindowsHook(WNDPROC wndProc /*= NULL*/) {
  $1 WNDPROC old = _txAltWndProc;
  _txAltWndProc = wndProc;
  $ return old;
}
...
bool txIDontWantToHaveAPauseAfterMyProgramBeforeTheWindowWillClose_AndIWillNotBeAskingWhereIsMyPicture() {
  $1 txMessageBox(
      ...
      "txIDontWantToHaveAPauseAfterMyProgramBeforeTheWindowWillClose_"
      "AndIWillNotBeAskingWhereIsMyPicture()\n\n"

      "Хоть вы долго [копировали]набирали это имя, на самом деле эта функция "
      "не реализована. "
      "Есть другая функция, которая убирает авто-паузу в конце программы, но в "
      "хелпе про нее не написано.\n\n"

      "Но не все так плохо. Определение нужной функции есть в исходных текстах "
      "TXLib.h, оно лежит рядом "
      "с определением той функции с длинным названием, которую вы сейчас "
      "вызвали.\n\n"

      "Нажмите в редакторе Ctrl+O, найдите и откройте файл TXLib.h (он лежит в "
      "папке, куда вы "
      "установили TXLib), затем нажмите Ctrl+F и ищите \"txIDontWant\". "
      "Удачи!\n\n",
...
// Bingo! Now you are learned to use the Sources, Luke. And may the Source be
// with you.
...
void txDrawMan(int x, int y, int sizeX, int sizeY, COLORREF color, double handL,
               ...
               double wink, double crazy, double smile, double hair,
               double wind) {
  $1 const char msg[] = "\0/А я - человечек из библиотеки!\0/Меня объясняли на "
                        "уроке!\0/Напиши меня сам!\0/";
  ...
  $ txLine(x, y - (1 + head) * sizeY, x + wind / 10 * sizeX,
           y - (1 + head + hair / 10) * sizeY);
  txLine(x, y - (1 + head) * sizeY, x + (wind / 10 - 0.1) * sizeX,
         y - (1 + head + hair / 10) * sizeY);
  txLine(x, y - (1 + head) * sizeY, x + (wind / 10 + 0.1) * sizeX,
         y - (1 + head + hair / 10) * sizeY);
  ...
  $ txLine(x, y - (0.8 + head - 0.05 * smile / 2) * sizeY, x - 0.05 * sizeY,
           y - (0.8 + head + 0.05 * smile / 2) * sizeY),
      txLine(x, y - (0.8 + head - 0.05 * smile / 2) * sizeY, x + 0.05 * sizeY,
             y - (0.8 + head + 0.05 * smile / 2) * sizeY),
      txNotifyIcon(4, (const char *)!!(L + 'L')[msg], "\n%s\n",
                   msg + (count++ % 3)["\"<"]);

  $ txCircle(x - 0.05 * sizeY, y - (0.9 + head - 0.02 * crazy) * sizeY,
             eyes * (1 + 0.5 * wink) * 0.02 * sizeY),
      txCircle(x + 0.05 * sizeY, y - (0.9 + head + 0.02 * crazy) * sizeY,
               eyes * (1 - 0.5 * wink) * 0.02 * sizeY),
      Sleep(1000 + count % 2);
...
double txQueryPerformance() {
  ...
  $ while (samples++ < maxSamples) {
    ...
    $ for (int y = 0; y < size.y; y += 10) for (int x = 0; x < size.x; x += 10)
        Win32::TextOut(dc, x, y, "*", 1);

    $ Win32::Ellipse(dc, 0, 0, size.x, size.y);
    $ Win32::ExtFloodFill(dc, size.x / 2, size.y / 2, TX_TRANSPARENT,
                          FLOODFILLSURFACE);

    $ txBitBlt(dc, size.x / 2, 0, size.x / 2, size.y / 2, dc, 0, 0) asserted;
    $ txBitBlt(dc, size.x / 2, size.y / 2, size.x / 2, size.y / 2, dc, 0,
               size.y / 2) asserted;
    $ txBitBlt(dc, 0, size.y / 2, size.x / 2, size.y / 2, dc, 0, 0) asserted;
    $ txBitBlt(dc, size.x / 2, size.y / 2, size.x / 2, size.y / 2, dc,
               size.x / 2, 0) asserted;
  }
...
COLORREF txRGB2HSL(COLORREF rgbColor) {
  ...
  $ double m1 = MAX(MAX(r, g), b) / 255.0, m2 = MIN(MIN(r, g), b) / 255.0,
           dm = m1 - m2, sm = m1 + m2,

           ir = r / 255.0, ig = g / 255.0, ib = b / 255.0,
  ...
  $ if (fabs(dm) < prec) {
    ...
    $ double cr = (m1 - ir) / dm, cg = (m1 - ig) / dm, cb = (m1 - ib) / dm;
...
COLORREF txHSL2RGB(COLORREF hslColor) {
  ...
  $ double ih = h / 255.0 * 360.0, il = l / 100.0, is = s / 100.0,
...
template <typename T> inline bool In(T x, T a, T b) { return a <= x && x <= b; }
...
inline int random(int range) { return rand() % range; }
...
INT_PTR txDialog::dialogBox(WORD resourceID) {
  ...
  $ if (!FindResource(NULL, resName, RT_DIALOG)) return TX_DEBUG_ERROR(
      "Не найден ресурс диалога %d" _ resourceID),
      0;
...
INT_PTR txDialog::dialogBox(const txDialog::Layout *layout /*= NULL*/,
                            size_t bufsize /*= 0*/) {
  $1 if (!layout) layout = layout_;
  $ if (!layout) return TX_DEBUG_ERROR(
      "Не установлен динамический шаблон диалога"),
      0;

  $ if (!bufsize) bufsize = 1024;

  $ DLGTEMPLATE *tmpl = (DLGTEMPLATE *)GlobalAlloc(GPTR, bufsize);
  $ if (!tmpl) return TX_DEBUG_ERROR(
      "GlobalAlloc(): Нет памяти для шаблона диалога"),
      0;

  $ const Layout *dlg = &layout[0];
  $ const Layout def = {DIALOG,
                        NULL,
                        0,
                        0,
                        0,
  ...
  $ void *ptr = _tx_DLGTEMPLATE_Create(
      tmpl, bufsize, (dlg->style ? dlg->style : def.style) | DS_SETFONT, 0, 0,
  ...
  $ for (i = 1; layout[i].wndclass != END; ++i) {
    ...
    $ ptr = _tx_DLGTEMPLATE_Add(ptr, bufsize - ((char *)ptr - (char *)tmpl),
                                item->style | WS_VISIBLE, 0, item->x, item->y,
                                item->sx, item->sy, item->id,
                                (const char *)item->wndclass, item->caption);
  }

  $ tmpl->cdit = (unsigned short)(i - 1);
  $ INT_PTR res =
      DialogBoxIndirectParam(NULL, tmpl, NULL, dialogProc__, (LPARAM)this);
...
int txDialog::dialogProc(HWND, UINT, WPARAM, LPARAM) { $1 return FALSE; }

//-----------------------------------------------------------------------------------------------------------------

ptrdiff_t CALLBACK txDialog::dialogProc__(HWND wnd, UINT msg, WPARAM wParam,
                                          LPARAM lParam) {
...
void *_tx_DLGTEMPLATE_Create(void *globalMem, size_t bufsize, DWORD style,
  ...
  $ if (caption) {
    $ pw += MultiByteToWideChar(
        _TX_CP, 0, caption ? caption : "", -1, (wchar_t *)pw,
  ...
  $ if (style & DS_SETFONT) {
    $ *pw++ = fontsize;
    $ pw += MultiByteToWideChar(
        _TX_CP, 0, font ? font : "", -1, (wchar_t *)pw,
        (int)(bufsize ? bufsize - ((char *)pw - (char *)globalMem) : 0xFFFF));
  }
...
void *_tx_DLGTEMPLATE_Add(void *dlgTemplatePtr, size_t bufsize, DWORD style,
  ...
  $ if (HIWORD(wclass) == 0xFFFF) {
    ...
    $ pw += MultiByteToWideChar(
        _TX_CP, 0, const_cast<char *>(wclass), -1, (wchar_t *)pw,
        (int)(bufsize ? bufsize - ((char *)pw - (char *)dlgTemplatePtr)
                      : 0xFFFF));
  }
  else {
    $ *pw++ = 0;
...
//{----------------------------------------------------------------------------------------------------------------
//! @ingroup Misc
//! @brief   Отладочная печать переменной во время вычисления выражения или
//! участка кода
...
//!          @tr <tt> $ (x) </tt>      @td Печать имени и значения переменной @c
//!          x внутри выражения.
//!          @tr <tt> $_(x) </tt>      @td Печать только  значения переменной @c
//!          x внутри выражения.
//!          @tbr
//!          @tr <tt> $$ (expr)  </tt> @td Печать выражения, его вычисление,
//!          печать и возврат значения. @n
//!                                        Если выражение содержит оператор
//!                                        "запятая", не взятый в скобки,
//!                                        необходимо окружать expr еще одной
//!                                        парой скобок.
//!          @tr <tt> $$_(expr)  </tt> @td То же, что и <tt>$$(expr),</tt>  но
//!          вторая печать идет без новой строки.
//!          @tbr
//!          @tr <tt> $$$ (expr) </tt> @td То же, что и <tt>$$(expr),</tt>  но
//!          для операторов или блоков кода (без возврата значения).
//!          @tr <tt> $$$_(expr) </tt> @td То же, что и <tt>$$$(expr),</tt> но
//!          вторая печать идет без новой строки.
//!          @tbr
//!          @tr <tt> $$$$  </tt>      @td Печать местоположения в коде.
//!          @tr <tt> $$$$_ </tt>      @td Печать местоположения в коде (только
//!          имя функции).
//!          @tr <tt> $n    </tt>      @td Перевод строки (печать @c '\\n').
//!          @endtable
//!
//! @title   Установка атрибутов символов консоли: @table
//!          @tr @c $d @td Светло-серый     цвет @td @td @c $T @td Прозрачный
//!          цвет
//!          @tr @c $b @td Светло-синий     цвет @td @td @c $B @td Темно-синий
//!          цвет
//!          @tr @c $g @td Светло-зеленый   цвет @td @td @c $G @td Темно-зеленый
//!          цвет
//!          @tr @c $c @td Светло-бирюзовый цвет @td @td @c $C @td
//!          Темно-бирюзовый цвет
//!          @tr @c $r @td Светло-красный   цвет @td @td @c $R @td Темно-красный
//!          цвет
//!          @tr @c $m @td Светло-малиновый цвет @td @td @c $M @td
//!          Темно-малиновый цвет
//!          @tr @c $y @td Желтый           цвет @td @td @c $Y @td Темно-желтый
//!          цвет
//!          @tr @c $t @td Белый            цвет @td @td @c $D @td Темно-серый
//!          цвет
//! @endtable
//! @title @table
...
//!          @td
//!          @td @c $W @td Warning bold     @td Желтый           на малиновом
//!          @td
//!          @tr @c $e @td Error            @td Светло-красный   на красном @td
//!          @td @c $E @td Error bold       @td Желтый           на красном @td
//!          @tr @c $f @td Fatal            @td Черный           на
//!          светло-красном @td
//!          @td @c $F @td Fatal bold       @td Малиновый        на
//!          светло-красном @td
//!          @tr @c $l @td Location         @td Черный           на темно-сером
//!          @td
//!          @td @c $L @td Location bold    @td Светло-серый     на темно-сером
//!          @td
//! @endtable
//! @title @table
//!          @tr @c $s @td Запомнить атрибуты. При выходе из блока кода атрибуты
//!          восстанавливаются.
//! @endtable
//!
...
template <typename T>
inline const T &_txDump(const T &value, const char *prefix = "",
                        const char *suffix = "") {
...
template <typename T>
inline T &_txDump(T &value, const char *prefix = "", const char *suffix = "") {
  std::cerr << prefix << value << suffix;
  return value;
}
...
struct _txSaveConsoleAttr {
  ...
  inline _txSaveConsoleAttr(WORD attr) : attr_(txGetConsoleAttr()) {
...
// Still block this warnings to avoid reporting about "= {0}" or "= {}" init
// style, and old style cast used in Windows.h RGB() macro.
...
#pragma warning(default : 4351) // new behavior: elements of array will be
                                // default initialized

#pragma warning(default : 4511) // copy constructor could not be generated
#pragma warning(default : 4512) // assignment operator could not be generated
#pragma warning(default : 4663) // C++ language change: to explicitly specialize
                                // class template
#pragma warning(default : 4702) // unreachable code

#if (_MSC_VER >= 1400)           // MSVC 8 (2005) or greater
#pragma warning(default : 26135) // missing locking annotation
#pragma warning(default : 28125) // the function must be called from within a
                                 // try/except block
...
#pragma warning(default : 304) // remark: access control not specified ("public"
                               // by default)
#pragma warning(default : 522) // remark: function redeclared "inline" after
                               // being called
#pragma warning(                                                               \
        default : 981) // remark: operands are evaluated in unspecified order
#pragma warning(                                                               \
        default : 1684) // conversion from pointer to same-sized integral type
                        // (potential portability problem)
