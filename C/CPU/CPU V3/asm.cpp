...
#define READ_WORD_                                                             \
  ...
  if (cond <= 0)                                                               \
    (*error_catcher)(strerr, ASM_SCAN_FAILED);

/**
                assemble					function builds
the array of commands and their arguments reading and analysing the input file
stream

                param[out]		strin		input file stream
                param[out]		strerr		error file stream
                param[out]		pts			the array of
pointers

                return						pointer of
commands double array if successfull, otherwise NULL
**/
...
double *assemble(FILE *strin, FILE *strerr, pointer *pts,
  ...
  while (true) {
    ...
    if ((word[0] == ':') || (strcmp(_strlwr(word), "func") == 0))
      ret_mark =
          asm_is_mrk(strin, strerr, word, &cur_code, pts, (*error_catcher));
