...
/**
			@par				There all math functions, which are used in calculator are contained. They are needed to turn them to the same type of arguments
**/

#define CHECK_POINTERS_									\
	if (argv == NULL) return GRAM_ARGV_NULL;			\
	if (res  == NULL) return GRAM_RES_NULL;				

int c_tan(const double* argv, double* res)
{
	CHECK_POINTERS_;
	if (fabs(cos(argv[0])) < DBL_EPSILON) return GRAM_FUNC_INVAL_ARG1;
	*res = tan(argv[0]);
	return GRAM_OK;
}
...
{
	CHECK_POINTERS_;
	if (fabs(sin(argv[0])) < DBL_EPSILON) return GRAM_FUNC_INVAL_ARG1;
...
{
	CHECK_POINTERS_;
	if (argv[0] < 0) return GRAM_FUNC_INVAL_ARG1;
	*res = log(argv[0]);
	return GRAM_OK;
}
...
int c_log(const double* argv, double* res)
{
	...
	if (argv[0] < 0 || fabs(argv[0] - 1) < DBL_EPSILON) return GRAM_FUNC_INVAL_ARG1;
	if (argv[1] < 0) return GRAM_FUNC_INVAL_ARG2;
...
{
	CHECK_POINTERS_;
	if (fabs(argv[0]) > 1) return GRAM_FUNC_INVAL_ARG1;
	*res = asin(argv[0]);
	return GRAM_OK;
}
