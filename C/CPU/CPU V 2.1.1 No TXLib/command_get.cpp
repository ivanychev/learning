...
/**
                calculator					the first thing
my cpu has to do is to solve Reverse Polish notation expressions. This functions
scans the input file stream for basic set of commands and arguments and proceeds
operations with numbers using th cpu
                @param[out]		strin		input file stream
                @param[out]		strerr		output file stream
                @param[out]		my_cpu		pointer if initialized
cpu
...
int calculator(FILE *strin, FILE *strout, FILE *strerr, cpu *my_cpu,
               double *value,
               int (*error_catcher)(FILE *strerr, cpu *my_cpu,
                                    int error_code)) {
  VERIFY(my_cpu != NULL);
  VERIFY(strin != NULL);
  VERIFY(strout != NULL);
  ...
  while (true) {
    ...
    c = CHECK_COMMAND(word, PUSH) CHECK_COMMAND(word, POP) CHECK_COMMAND(
        ...
        CHECK_COMMAND(word, DIV) CHECK_COMMAND(word, SIN)
            CHECK_COMMAND(word, COS) CHECK_COMMAND(word, TAN)
                CHECK_COMMAND(word, SQRT) CHECK_COMMAND(word, POW)
                    CHECK_COMMAND(word, END) CHECK_COMMAND(word, DUMP)
                        CHECK_COMMAND(word, ENTER) CHECK_COMMAND(word, TAB)
                            CHECK_COMMAND(word, SPACE) CHECK_COMMAND(word, OUT)
                                CHECK_COMMAND(word, MOV)
                                    CHECK_COMMAND(word, SWAP)
                                        CHECK_COMMAND(word, DUB) CMD_NONE;
    ...
    case CMD_PUSH:
      ...
      if (is_value)
        cond = cpu_catch_error(strerr, my_cpu,
                               cpu_push(my_cpu, STR_value, push_value));
      else {
        _strlwr(push_word);
        reg = CONVERT_REG_(push_word, ax) CONVERT_REG_(push_word, bx)
            CONVERT_REG_(push_word, cx) CONVERT_REG_(push_word, dx) STR_NON;
    ...
    case CMD_POP:
      ...
      if (cond == 0)
        return CALC_BAD;
      _strlwr(pop_word);
      reg = CONVERT_REG_(pop_word, ax) CONVERT_REG_(pop_word, bx)
          CONVERT_REG_(pop_word, cx) CONVERT_REG_(pop_word, dx)
              CONVERT_REG_(pop_word, out) STR_NON;
    ...
    case CMD_OUT:
      ...
      if (cond == 0)
        return CALC_BAD;
      _strlwr(out_word);
      reg = CONVERT_REG_(out_word, ax) CONVERT_REG_(out_word, bx)
          CONVERT_REG_(out_word, cx) CONVERT_REG_(out_word, dx)
              CONVERT_REG_(out_word, st) CONVERT_REG_(out_word, ch) STR_NON;
      if (reg == STR_ch) {
        int out_char = -1;
        ...
        cond = cpu_catch_error(strerr, my_cpu,
                               cpu_out(strout, strerr, my_cpu, reg, out_char));
      } else
        cond = cpu_catch_error(strerr, my_cpu,
                               cpu_out(strout, strerr, my_cpu, reg));
      if (cond == CPU_ERROR_CATCHER_BAD)
        return CALC_BAD;
      break;
    ...
    case CMD_MOV:
      ...
      mov_arg1 = CONVERT_REG_(mov_word1, ax) CONVERT_REG_(mov_word1, bx)
          CONVERT_REG_(mov_word1, cx) CONVERT_REG_(mov_word1, dx) STR_NON;
      mov_value = 0;
      ...
      if (cond != 0)
        ...
        mov_arg2 = CONVERT_REG_(mov_word2, ax) CONVERT_REG_(mov_word2, bx)
            CONVERT_REG_(mov_word2, cx) CONVERT_REG_(mov_word2, dx) STR_NON;
      cond = cpu_catch_error(strerr, my_cpu,
...
/*
                ...
                case CMD_SQRT:
                        cond = cpu_catch_error(strerr, my_cpu,
   cpu_sqrt(my_cpu)); if (cond == CPU_ERROR_CATCHER_BAD) return CALC_BAD; break;
                case CMD_POW:
                ...
                case CMD_DUMP:
                        cond = cpu_catch_error(strerr, my_cpu, cpu_dump(strerr,
   my_cpu)); if (cond == CPU_ERROR_CATCHER_BAD) return CALC_BAD; break;


                case CMD_SPACE:
                        cond = cpu_catch_error(strerr, my_cpu, cpu_space(strout,
   my_cpu)); if (cond == CPU_ERROR_CATCHER_BAD) return CALC_BAD; break; case
   CMD_ENTER: cond = cpu_catch_error(strerr, my_cpu, cpu_enter(strout, my_cpu));
                        if (cond == CPU_ERROR_CATCHER_BAD) return CALC_BAD;
                        break;
                case CMD_TAB:
                        cond = cpu_catch_error(strerr, my_cpu, cpu_tab(strout,
   my_cpu)); if (cond == CPU_ERROR_CATCHER_BAD) return CALC_BAD; break;
                        */
