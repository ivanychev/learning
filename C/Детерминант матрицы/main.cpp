/**
	...
	@par			This program calculates the determinant of NxN matrix
...
/**
		main			request N and matrix of float numbers, prints determinant
		@return			0 if arguments are correct, otherwise -1
**/
int main()
{
	const long fake = 1200000000;
	OUT printf("# Hello, world! I was built in %s %s\n"
		...
		"# Type the size of it (edge) and then type the float matrix's numbers using SPACEs and ENTERs\n"
	...
	for (int i = 0; i < n; ++i)																					
        for (int j = 0; j < n; ++j) scanf("%lg", &data[i][j]);
/*
	OUT 
	{
		...
		OUT printf("# OH, there's a bug. Rewriting the source code...\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# All mistake are corrected. Compiling...\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# Linking...\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# Optimizing...\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# Build completed, restoring the input...\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# Analyzing...\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# 33 percent completed\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# 66 percent completed\n");
		...
		OUT printf("# 99 percent completed\n");
		for (long i = 0; i < fake / 2; ++i);
		OUT printf("# WHAT THE HELL! THE SAME MISTAKE!!\n");
		for (long i = 0; i < 1.5 * fake ; ++i);
		OUT printf("# Ignoring...\n");
		for (long i = 0; i < fake ; ++i);
		OUT printf("# Building the pointer of pointer array...\n");
		for (long i = 0; i < fake ; ++i);
		OUT printf("# Function connected. Calculating...\n");
		for (long i = 0; i < fake ; ++i);
		OUT printf("# Completed.\n");
		for (long i = 0; i < 1000000; ++i);
		OUT printf("# Trying to return to the main function\n");
		for (long i = 0; i < fake ; ++i);
		OUT printf("# Completed.\n");
		for (long i = 0; i < 1000000; ++i);
		OUT printf("# Checking the answer...\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# Looks fine, here it is\n\n");
		for (long i = 0; i < 1.5 * fake; ++i);
		OUT printf("# Where's it?\n");
		for (long i = 0; i < fake; ++i);
		OUT printf("# Oh, that's it, sorry\n");
		for (long i = 0; i < fake; ++i);
	}
*/
...
/*double det(double** matrix, const int EDGE)
{
	...
	//for (int i = 0; i < EDGE; ++i) copy_matrix[i] = (double *) calloc(EDGE, sizeof (double));
