...
/*
        Даны два многоразрядных числа a и b, заданных символьными массивами длины n, где n – большое положительное число (10^6… 10^8…10^9).
        Написать параллельную программу, определяющую с=a+b.
        char a[n] ,b[n] ,с[n+1]; элементами массивов a,b,c являются цифры [0,1,2,…,9].
        Параллельный алгоритм должен быть хорошо масштабируемым для любых исходных данных, в том смысле, что, 
        для достаточно большого числа процессоров p должно выполняться соотношение
        T(2p)~ T(p)/2, где T(p) – время вычислений на p процессорах.
*/

/*
        Program gets number, filename1, filename2, result_filename as command prompt arguments. 
        These are relative paths to files with first and second summands and
        a file where the result will be placed.     
*/
...
void fperror(FILE* stream) {
    fprintf(stream, LAST_ERROR);
}
...
int32_t get_args(const int32_t argc, 
             char const* argv[], 
             FILE** f_res,
             char** summand1,
             char** summand2,
