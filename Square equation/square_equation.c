...
/**
main - Requests the quantity of equations, scans for factors, prints answers and sum of roots
    @param  a       x^2 factor
    @param  b       x^1 factor
    @param  c       x^0 factor
    @param  x1      first root
    ...
    @param ret      variable, which contain the information about input condition (whether it is successful)
    @param i        counter

    @note square_eq structure is created, contains factors and quantity of roots

    @param eqs      the array of struct square_eq, contains input equations and their numbers of roots

    @return main: Solves multiple equations with every a,b,c. If it is not possible - returns -1 (for example: n < 0)
...
/**
    ...
    @param          b   b factor
    @param          c   c factor
    @param          d   discriminant, which turns at the square root of discriminant of ax*x+bx+c=0
