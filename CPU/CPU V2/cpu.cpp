...
/**
		cpu_pop							pops one element from cpu stack, saves it to the input pointer and prints it to the file stream
...
/**
		cpu_catch_error					Catches the codes of errors and prints error messages to the file stream, or does nothing if everything is fine
		@param[out]		stream			file stream
		@param[out]		my_cpu			pointer of cpu
...
int cpu_pop(FILE* stream, cpu* my_cpu, double* return_value)
{
	...
	*return_value = stack_pop(my_cpu -> cpu_stack);
	fprintf(stream, "%lg", *return_value);
	return CPU_OK;
}
