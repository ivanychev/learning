/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        @file		main.cpp
        @author		Sergey Ivanychev, sergeyivanychev@gmail.com, MIPT DCAM,
376 group

        @mainpage
        @par		There's a prototype vector realization.
                                -  The size of vector is automatically changing
during push_back
                                -  Support of		vector + vector (even if
sizes are different), double * vector, vector * double, vector[index], vector =
vector operators
                                -  There are MODULE methon inside

        @par		Main function is a complex of vector testing.

        @par		This realization wasn't copied from anywhere and
inspired by C++ standard vector class, which was used by me few years ago.
Theory by Ilya Ruydolphovych and Stephen Pratha.
        ...
        @par		Changelog V1.1
                                ...
                                - added POP_BACK, PUSH_FRONT, POP_FRONT
functions. Vector now could represent Stack, Deck, Queue
                                - added some usable functions: INSERT,
INSERT_ARRAY, SHIFT_LEFT, SHIFT_RIGHT, REMOVE_RANGE
                                - fixed operator [] support. Now return value
type is double&
                                - FIXED: added initialization list of standard
constructor

        @todo		- Write detailed documentation
                                - Add __FUNCTION__ macros synonyms for GCC and
VS using compiler detection
                                - Refactor operator+/- and +=/-= functions
definition using macroses
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
*/
