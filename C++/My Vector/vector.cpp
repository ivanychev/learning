...
Vector::Vector(const Vector &that) {
  MVERIFY(that.check() != VECTOR_OK,
          "VECTOR(const Vector&): Argument is corrupted");
...
Vector::Vector(int array_size, double *array) {
  ...
  MVERIFY(reset_check != VECTOR_OK,
          "Vector(int size, double* array): Reset failed");

  int realloc_check =
      _realloc_memory(MEMORY_BLOCK_SIZE * ((size / MEMORY_BLOCK_SIZE) + 1));
  MVERIFY(realloc_check != VECTOR_OK,
          "Vector(int size, double* array): Realloc failed");
  ...
  MVERIFY(check() != VECTOR_OK,
          "VECTOR(int array_size, double* array): Postcondition failed");
}
...
Vector Vector::operator+(const Vector &that) const {
  MVERIFY(that.check() != VECTOR_OK,
          "VECTOR::operator+(const Vector&): Argument is corrupted");
  MVERIFY(check() != VECTOR_OK,
          "VECTOR::operator+(const Vector&): Current is corrupted");

  Vector out = *this;

  if (that._memory_size > _memory_size) {
    int check_realloc = out._realloc_memory(that._memory_size);
    MVERIFY(check_realloc != VECTOR_OK,
            "VECTOR::operator+: reallocation failed");
  }

  for (int i = 0; i < that._memory_size; ++i)
    out.data[i] += that.data[i];
  if (that.size > out.size)
    out.size = that.size;

  MVERIFY(out.check() != VECTOR_OK,
          "VECTOR::operator+(const Vector&): Construction failed");
  return out;
}
...
Vector Vector::operator-(const Vector &that) const {
  ...
  for (int i = 0; i < that_copy.size; ++i)
    that_copy.data[i] *= -1;
...
Vector Vector::operator*(double value) const {
  MVERIFY(check() != VECTOR_OK,
          "VECTOR::operator*(double value): Current is corrupted");
  ...
  MVERIFY(out.check() != VECTOR_OK,
          "VECTOR::operator*(double value): Construction failed");
  return out;
}
...
Vector operator*(double value, const Vector &that) {
  MVERIFY(
      that.check() != VECTOR_OK,
      "Vector operator*(double value, const Vector& that): that is corrupted");
  Vector out = that * value;

  MVERIFY(out.check() != VECTOR_OK, "Vector operator*(double value, const "
                                    "Vector& that): Construction failed");
  return out;
}
...
void Vector::operator+=(const Vector &that) {
  MVERIFY(check() != VECTOR_OK, "PRECOND: Vector::operator+=(const Vector& "
                                "that):: Current vector broken");
  MVERIFY(
      that.check() != VECTOR_OK,
      "PRECOND: Vector::operator+=(const Vector& that):: Input vector broken");
  ...
  MVERIFY(check() != VECTOR_OK, "POSTCOND: Vector::operator+=(const Vector& "
                                "that):: Current vector broken");
}
...
int Vector::push_back(double value) {
  ...
  if (size % MEMORY_BLOCK_SIZE == 0 && size != 0) {
    ...
    VERIFY(check_extend != VECTOR_OK, VECTOR_NO_MEMORY,
           "Data extending failed");
  }
  data[size++] = value;
...
int Vector::remove(int index) {
  ...
  if (index < 0)
    index +=
        size; // negative are allowed, numerating values of vector from the end
  VERIFY(index < 0, VECTOR_INDEX_BAD,
         "Bad index, the range is [-SIZE , SIZE - 1]");
  ...
  VERIFY(check_optimize != VECTOR_OK, VECTOR_OPTIM_FAIL,
         "REMOVE:: Optimization failed");
...
\
        VERIFY1(next_data != NULL, VECTOR_NO_MEMORY, "Calloc failed in %s",
#name); \
\
...
int Vector::check() const {
  if (this == NULL)
    return VECTOR_THIS_NULL;
  if (data == NULL)
    return VECTOR_DATA_NULL;
  if (size > _memory_size)
    return VECTOR_DATA_LEAK;
  return VECTOR_OK;
}
...
void Vector::print(FILE *stream) {
  ...
  for (int i = 0; i < size - 1; ++i)
    fprintf(stream, "%lg, ", data[i]);
  if (size - 1 >= 0)
    fprintf(stream, "%lg", data[size - 1]);
  fputc(']', stream);
}
...
void Vector::dump(FILE *stream) {
  fprintf(stream,
          "I'm vector [%08x]. I'm %s [%d]\n"
          "Size = %d, Memory = %d\n",
          this, (check() == VECTOR_OK) ? "OK" : "BAD", check(), size,
          _memory_size);

  for (int i = 0; i < size; ++i)
    fprintf(stream, "%dth: %lg\n", i, data[i]);
  putc('\n', stream);
...
int Vector::insert(double value, int place) {
  /*
          ...
          VERIFY(check_set != VECTOR_OK, check_set, "Vector::insert(): SET
     function failed"); for (int i = size - 1; i >= place + 1; ++i) data[i] =
     data[i - 1]; data[place] = value;
...
int Vector::insert_array(double *array_, int array_size, int place) {
  ...
  VERIFY(check_set != VECTOR_OK, check_set,
         "INSERT_ARRAY: Setting size failed");
  for (int i = size; i >= place + array_size; --i)
    data[i] = data[i - array_size];
  for (int i = place; i < place + array_size; ++i)
    data[i] = array_[i - place];
...
int Vector::remove_range(int from, int to) {
  PRECOND;
  VERIFY(!((0 <= from) && (from < size)), VECTOR_INDEX_BAD,
         "REMOVE_RANGE: FROM index invalid");
  VERIFY(!((0 < to) && (to <= size)), VECTOR_INDEX_BAD,
         "REMOVE_RANGE: TO index invalid");
  for (int i = from; i < size - (to - from); ++i)
    data[i] = data[i + (to - from)];
...
int Vector::push_front(double value) {
  ...
  VERIFY(check_insert != VECTOR_OK, check_insert,
         "PUSH_FRONT: Insertion failed");
...
int Vector::shift_left(int step) {
  ...
  for (int i = 0; i < size - step; ++i)
    data[i] = data[i + step];
  for (int i = (size - step < 0) ? 0 : size - step; i < size; ++i)
    data[i] = 0;
  POSTCOND;
  return VECTOR_OK;
}
...
int Vector::_realloc_memory(int new_memory_size) {
  PRECOND;
  VERIFY(new_memory_size < size, VECTOR_NOT_EGH_MEM,
         "VECTOR::_realloc_memory: not enough memoty for current data");
  double *new_data = (double *)calloc(new_memory_size, sizeof(double));
  VERIFY(new_data == NULL, VECTOR_NO_MEMORY,
         "VECTOR::_realloc_memory: calloc failed");
...
int Vector::_set_size(int new_size) {
  ...
  if (_memory_size != req_memblocks * MEMORY_BLOCK_SIZE) {
    ...
    VERIFY(check_realloc != VECTOR_OK, check_realloc,
           "_SET_SIZE: Reallocation failed");
...
int Vector::_optimize_memory() {
  ...
  if (_memory_size - size >= MEMORY_BLOCK_SIZE) {
    int check_realloc = _realloc_memory(
        (size % MEMORY_BLOCK_SIZE != 0)
            ? MEMORY_BLOCK_SIZE * ((size / MEMORY_BLOCK_SIZE) + 1)
            : size);
    VERIFY(check_realloc != VECTOR_OK, check_realloc,
           "_OPTIMIZE_MEMORY: Failed to reallocate memory");
    return VECTOR_OK;
  } else
    return VECTOR_OK;
}
