...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//								VERIFICATION
//MACROSES 	VERIFY  - checks the expression, returns ERR_CODE if it's true and
//prints message to STDOUT 	MVERIFY - checks the expression, prints error message
//in STRLOG if it's true
//
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
#define __LOG_TEMPLATE(place, cond, err_code, msg)                             \
  if (!(_CrtCheckMemory()))                                                    \
    fprintf(stdout, "Memory fucked up, line [%d]", __LINE__);                  \
  if (cond) {                                                                  \
...
#define D_OUT1(thing, arg1)                                                    \
  fprintf(STRLOG, "DEBUG# ");                                                  \
  fprintf(STRLOG, thing, arg1);                                                \
  fputc('\n', STRLOG);
#define D_OUT2(thing, arg1, arg2)                                              \
  fprintf(STRLOG, "DEBUG# ");                                                  \
  fprintf(STRLOG, thing, arg1, arg2);                                          \
  fputc('\n', STRLOG);
#define D_OUT3(thing, arg1, arg2, arg3)                                        \
  fprintf(STRLOG, "DEBUG# ");                                                  \
  fprintf(STRLOG, thing, arg1, arg2, arg3);                                    \
  fputc('\n', STRLOG);
#define DBG_FREE
#else
#define D_OUT(thing)
