...
struct Template_command {
  ...
  Template_command(double command, unsigned int size, char *translation,
                   unsigned int translation_length, ofstream &log);
  ~Template_command();
  void console_dump() const;
};

/**
                @par					The data below is
developed translation templates for each command of Ganymede processor, that are
                                                                stored in binary
format. The machine code have been executed from compiled by MASM32 file, where
all of function have been written and tested. These templates don't contain any
offsets or addresses, for obvious reasons. They will be used while initializing
Command structures when Code would be getting instructions from Ganymede binary
file
...
#define JNE_PART                                                               \
  0x66, 0x8B, 0xC3, 0x66, 0x33, 0x1D, 0x00, 0x00, 0x00,                        \
      0x00,                              /*one of constant flag variables*/    \
      0x0F, 0x85, 0x00, 0x00, 0x00, 0x00 /*destination offset*/

#define NOT_JNE_PART                                                           \
  0x66, 0x8B, 0xC3, 0x66, 0x33, 0x1D, 0x00, 0x00, 0x00, 0x00, 0x0F, 0x84,      \
      0x00, 0x00, 0x00, 0x00

const unsigned char TR_NONE[] = {0x90}; // nop
const unsigned char TR_PUSH[] = {0xDD, 0x05, 0x00,
                                 0x00, 0x00, 0x00}; // fld real8 ptr ...
const unsigned char TR_PUSH_VAL[] = {
    0xEB, 0x08,                                     // JMP 8h
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // DOUBLE IS HERE
    0xDD, 0x05, 0x00, 0x00, 0x00, 0x00};            // FLD it

const unsigned char TR_POP[] = {0xDD, 0x15, 0x00,
                                0x00, 0x00, 0x00}; // fst real8 ptr ...
...
const unsigned char TR_POW[] = {
    0xDD, 0x1D, 0x00,
    0x00, 0x00, 0x00, // 0040101A  |. DD1D 20404000  FSTP QWORD PTR DS:[404020]
    0xDD, 0x1D, 0x00,
    0x00, 0x00, 0x00, // 00401020  |. DD1D 28404000  FSTP QWORD PTR DS:[404028]
    0xFF, 0x35, 0x00,
    0x00, 0x00, 0x00, // 00401026  |. FF35 24404000  PUSH DWORD PTR DS:[404024]
    0xFF, 0x35, 0x00,
    0x00, 0x00, 0x00, // 0040102C  |. FF35 20404000  PUSH DWORD PTR DS:[404020]
    0xFF, 0x35, 0x00,
    0x00, 0x00, 0x00, // 00401032  |. FF35 2C404000  PUSH DWORD PTR DS:[40402C]
    0xFF, 0x35, 0x00,
    0x00, 0x00, 0x00, // 00401038  |. FF35 28404000  PUSH DWORD PTR DS:[404028]
    0xE8, 0x00, 0x00,
    0x00, 0x00,        // 0040103E  |. E8   15150000  CALL <JMP.&msvcrt.pow>
    0x83, 0xC4, 0x10}; // 00401043  |. 83C4 10        ADD ESP,10
const unsigned char TR_END[] = {0x6A, 0x00,                    // PUSH 0
                                0xE8, 0x00, 0x00, 0x00, 0x00}; // Call ...
const unsigned char TR_SPACE[] = {
    0x68, 0x00, 0x00, 0x00, 0x00,  // PUSH POINTER
    0xE8, 0x00, 0x00, 0x00, 0x00}; // Calling printf
const unsigned char TR_ENTER[] = {
    0x68, 0x00, 0x00, 0x00, 0x00,                              // PUSH POINTER
    0xE8, 0x00, 0x00, 0x00, 0x00};                             // Calling printf
const unsigned char TR_TAB[] = {0x68, 0x00, 0x00, 0x00, 0x00,  // PUSH POINTER
                                0xE8, 0x00, 0x00, 0x00, 0x00}; // Calling printf
const unsigned char TR_OUT[] = {
    0xFF, 0x35, 0x00, 0x00,
    0x00, 0x00, // 0040100E  |. FF35 04404000  PUSH DWORD PTR DS:[404004]
    0xFF, 0x35, 0x00, 0x00,
    0x00, 0x00, // 00401014  |. FF35 00404000  PUSH DWORD PTR DS:[404000] ; |
    0x68, 0x00, 0x00, 0x00,
    0x00, // 0040101A  |. 68 31404000    PUSH console_.00404031 ; |format =
          // "%lg"
    0xE8, 0x00, 0x00, 0x00,
    0x00, // 0040101F  |. E8 14150000    CALL <JMP.&msvcrt.printf> ; \printf
...
const unsigned char TR_OUT_CH[] = {
    0xB0, 0x00, // MOV AL, Unsigned hex number we want to print
    0x50,       // PUSH EAX
    0xE8, 0x00, 0x00, 0x00, 0x00, // CALL PUTCHAR
    0x83, 0xC4, 0x04};            // ADD ESP, 4

const unsigned char TR_MOV[] = {
    0xDD, 0x05, 0x00, 0x00, 0x00, 0x00,         // fld to stack
    0xDD, 0x1D, 0x00, 0x00, 0x00, 0x00};        // fstp to destination
const unsigned char TR_MOV_VAL[] = {0xEB, 0x08, // JMP 8h
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                    0x00, 0x00, 0xDD, 0x05, 0x00, 0x00,
                                    0x00, 0x00, // fld to stack
                                    0xDD, 0x1D, 0x00, 0x00, 0x00, 0x00};
const unsigned char TR_SWAP[] = {0xD9, 0xC9}; // fxch st(1)
const unsigned char TR_DUB[] = {0xD9, 0xC0};
const unsigned char TR_JMP[] = {
    0xE9, 0x00, 0x00, 0x00,
    0x00}; // to - from - 5 (this is length of jmp instruction of x86)
const unsigned char TR_JBE_JAE[] = {COND_JUMP_COMMON, NOT_JNE_PART,
                                    NOT_JNE_PART};
...
const unsigned char TR_ASIN[] = {
    ...
    0xDE, 0xC9, 0xD9, 0xE8, 0xDE, 0xE1, 0xD9,
    0xFA, 0xDD, 0x05, 0x00, 0x00, 0x00, 0x00, // well, you know ... DI
    0xDE, 0xF1, 0xD9, 0xE8, 0xD9, 0xF3};
const unsigned char TR_LN[] = {0xD9, 0xE8, 0xD9, 0xC9, 0xD9,
                               0xF1, 0xD9, 0xEA, 0xDE, 0xF9};
...
const unsigned char TR_IN_ST[] = {0x68, 0x00, 0x00, 0x00, 0x00, // where to save
                                  0x68, 0x00, 0x00, 0x00, 0x00, // __FIO
                                  0xE8, 0x00, 0x00, 0x00, 0x00, // scanf address
                                  0x83, 0xC4, 0x08,             // add esp, 8
...
/**
                @par			Static Template_command array below
represents the range of command that are possible to translate via Ganymede-x86.
Each of them initialized with double value, which is the number of Ganymede
command and its size and x86 template and its size.

                                                When CODE loads template object,
it recieves the information about the size of Ganymede instruction with its
                                                arguments ant the template of
it. The particulars will be analyzed later
**/
...
const Template_command GANYMEDE_INSTRUCTIONS[] = {
    ...
    Template_command(1014, 1, (char *)TR_SPACE, sizeof(TR_SPACE),
                     strlog), // SPACE
    Template_command(1015, 1, (char *)TR_ENTER, sizeof(TR_ENTER),
                     strlog),                                          // ENTER
    Template_command(1016, 1, (char *)TR_TAB, sizeof(TR_TAB), strlog), // TAB
    Template_command(1017, 3, (char *)TR_OUT, sizeof(TR_OUT),
                     strlog), // OUT - REMEMBER ABOUT OUT CH!!!
    ...
    Template_command(1022, 2, (char *)TR_JBE_JAE, sizeof(TR_JBE_JAE),
                     strlog), // JBE
    Template_command(1023, 2, (char *)TR_JB_JE_JA, sizeof(TR_JB_JE_JA),
                     strlog), // JB
    Template_command(1024, 2, (char *)TR_JBE_JAE, sizeof(TR_JBE_JAE),
                     strlog), // JAE
    Template_command(1025, 2, (char *)TR_JB_JE_JA, sizeof(TR_JB_JE_JA),
                     strlog), // JA
    Template_command(1026, 2, (char *)TR_JB_JE_JA, sizeof(TR_JB_JE_JA),
                     strlog),                                          // JE
