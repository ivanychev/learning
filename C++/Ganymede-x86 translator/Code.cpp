/**
		...
		@par			The defenitions of all CODE object methods are located here. 
...
/**
			change_endian				function turns order of bytes in UNSIGNED to the opposite.
										Yeah, pointers. That function is not used in build (V 1.1)
...
unsigned int change_endian(unsigned arg)
{
	...
	*(unsigned char*)((unsigned)(&changed) + 3) = *(unsigned char*)((unsigned)(&arg) + 0);
	*(unsigned char*)((unsigned)(&changed) + 2) = *(unsigned char*)((unsigned)(&arg) + 1);
	*(unsigned char*)((unsigned)(&changed) + 1) = *(unsigned char*)((unsigned)(&arg) + 2);
	*(unsigned char*)((unsigned)(&changed) + 0) = *(unsigned char*)((unsigned)(&arg) + 3);
...
/**
		@par			Some of usable function, that are called in translate library are located here.
						Thay are aimed to count jump offsets, differences of addresses, calculating variables
						addresses, and patching into Code object
...
void patch_jmp_offset(unsigned unreversed_from, unsigned unreversed_to, char* dest)
{
	CHECK_PTR(dest);
...
{
	LG_ASSERT((gan_num >= 0) && (IS_ZERO(gan_num - abs(gan_num))), "Wrong gan number format");
	CHECK_PTR(code);
...
{
	LG_ASSERT((number >= 0) && (IS_ZERO(number - abs(number))), "Invalid variable number");
...
/**
		Code::console_print_gan_code					debug print of Code object to console
...
compiled(to_compile)
{
	...
	CALL(incr_check = increase_buffer(), "Buffer increase failed. Terminating...");
...
/**
			increse_buffer()				The array increases the buffer of Command pointers
											on the value of block_size
**/
...
bool Code::increase_buffer()
{
	...
	if (size != 0) memcpy(new_buffer, command_array, sizeof(Command*)*size);
	for (unsigned int i = size; i < buffer_size; ++i)	new_buffer[i]	 = NULL;
	if (size != 0) for (unsigned int i = 0; i < size; ++i)				command_array[i] = NULL;

	if (size != 0) delete command_array;
...
#define START_CODE_CHECK_REPORT																\
	if (condition == true) log_stream << "Unsuccessfull Code buffer report started...\n";		\
	is_reported = true;																		\
...
/**
			add_command					that function gets input Command pointer and saves it in Command* buffer,
										increasing it, if there isn't any free cells
**/
...
/**
			get_code					the main function of scanning the input Ganymede binary file for
										Ganymede commands and arguments. Function scans for the next command, 
										creates the Command object for it and saves it.
...
bool Code::get_code()
{
	...
	{
		CALL(getting = new Command(log_stream), "Construction new command object failed");
		RT_ASSERT(getting -> get_command(source) == true, "Getting next command failed.");
		RT_ASSERT(add_command(getting) == true, "Adding scanned command failed.");
		
		if (IS_ZERO(getting -> gan_codes[0] - CMD_END)) end_reached = true;
	}
...
/**
			revive_code				The code, that is scanned and got its x86 template is not executable yet
									due to the lack of addresses and offsets there. Reviving is calculating and pasting
									everything in Code object to get ready to patch it in the output file
...
bool Code::revive_code()
{
	...
	for (unsigned i = 0; i < size; ++i) if (IS_ZERO(command_array[i] -> gan_codes[0] - (double)CMD_DEF))
		count_vars++;
	...
	CALL(calculate_addresses(), "Failed to insert x86 adresses in Code structure");
...
void Code::calculate_addresses()
{
	...
	for (unsigned i = 1; i < size; ++i)
		command_array[i] -> x86_address = 
			command_array[i-1] -> x86_address +
			command_array[i-1] -> x86_length;
...
#define RELOAD_CMD(name, gan_command, gan_len, template_)																							\
			...
			{																																		\
				...
				for (int j = 0; j < temp_size; ++j) temp_buffer[j] = command_array[i] -> gan_codes[j];	/* copying gan_codes to temp buffer*/		\
				delete command_array[i];																											\
				command_array[i] = new Command(log_stream);																							\
				(*(command_array[i])).load_template(Template_command(gan_command, gan_len, (char*)template_  ,sizeof(template_), log_stream));		\
				for (int j = 0; j < temp_size; ++j) command_array[i] -> gan_codes[j] = temp_buffer[j];												\
			}																																		\
...
void Code::deal_with_out_ch()
{
	...
	{
		cout << command_array[i] -> gan_codes[0] << " " << command_array[i] -> gan_codes[1] << endl;
...
/**
			print_code				debug print of x86 translation, contained in Code object
**/
...
/**
		patch_exe				function is responsible for patching output file, which is template.exe (in v 1.1).
								Contains lots of debug print
...
void Code::patch_exe(ofstream& exe) const
{
	...
	CALL(first_jump = first_jump_patch(), "Failed to allocate memory for ganymede variables");
	...
	for (unsigned i = 0; i < size; ++i) 
	{
		RT_ASSERT((exe.fail() == false)&&(exe.bad()  == false), "File reading error");
		command_array[i] ->console_print_ganymede_command();
		WRITE(command_array[i] -> x86_codes, command_array[i] -> x86_length);
	}
	printf("%u", CODE_BEGIN);
	if (first_jump != NULL) delete first_jump;
