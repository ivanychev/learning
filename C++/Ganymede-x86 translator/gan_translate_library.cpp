...
/**
                ...
                @par				This library is one of the key
things in translation which is aimed to finish building code located in Command
structures of Code object. There's a little DSL there.

                                                        Each function has one
argument - unsigned integer which is the number of Command object, analysing in
function.

                                                        PTR(NUM)	- the
pointer of NUM'th byte, located in x86_translation array of the Command.
                                                        VALUE(NUM)	- the
value, contained in NUM'th cell of double Ganymede command array ADDR(NUM)
- the address in virtual memory space of executing translation of NUM'th byte in
                                                                                  x86_translation
                                                        STREAM		- often,
stream arguments of Ganymede commands are located in the second cell of Ganymede
commands array. This macros get's it and translates it into the address in
memory space EQUAL(v1, v2)
...
#define EMPTY_CHANGE_FUNCS(func)                                               \
  void Code::##func##_translate(unsigned number) { CHECK; };
...
#define COND2_JUMP(name, cond1, cond2)                                         \
  void Code::##name##_translate(unsigned number) {                             \
    ...
    CALL(patch_to_gan(this, PTR(begin + 12), VALUE(1), ADDR(begin + 11)),      \
         PATCH_FAILED);                                                        \
                                                                               \
    begin += 16;                                                               \
                                                                               \
...
unsigned char *Code::first_jump_patch() const {
  ...
  CALL(patch_jmp_offset(FIRST_JUMP_ADDR, CODE_BEGIN,
                        (char *)(&(JMP_AND_VARS[1]))),
       "Failed to patch the head");
