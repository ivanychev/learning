...
Command::Command(ofstream &log)
    : gan_length(0), x86_length(0), x86_codes(NULL), x86_address(0),
      gen_address(0), log_stream(log) {
  CHECK_PTR(this);
  for (int i = 0; i < MAXGANCOMMAND; ++i)
    gan_codes[i] = 0;
};
...
Command::~Command() {
  if (x86_codes != NULL) {
    for (unsigned int i = 0; i < x86_length; ++i)
      x86_codes[i] = char_poison;
    delete x86_codes;
    x86_codes = NULL;
    x86_length = int_poison;
  }
  for (unsigned int i = 0; i < gan_length; ++i)
    gan_codes[i] = double_poison;
...
bool Command::check() {
  ...
  if (gan_length > 4)
    condition = false;
  if (x86_codes == NULL && x86_length != 0)
    condition = false;
...
bool Command::get_command(ifstream &source) {
  ...
  */
  ...
  CALL(get_double(source, gan_codes),
       "Failed to get next double value from source");

  int ganymede_command_num = (int)(*gan_codes) - CMD_NONE;

  LG_ASSERT(
      (0 <= ganymede_command_num && ganymede_command_num <= CMD_IN - CMD_NONE),
      "Unknown command scanned");

  CALL(load_template(GANYMEDE_INSTRUCTIONS[ganymede_command_num]),
       "Failed while copying from template");

  for (unsigned int i = 1; i < gan_length; ++i)
    get_double(source, &(gan_codes[i]));
...
void Command::console_print_ganymede_command() const {
  for (unsigned int i = 0; i < gan_length; ++i)
    cout << gan_codes[i] << endl;
  for (unsigned int i = 0; i < x86_length; ++i)
    printf("%ud:%02hhx ", i, x86_codes[i]);
  cout << "\n";
}
...
Template_command::Template_command(double command, unsigned int size,
                                   char *translation,
                                   unsigned int translation_length,
                                   ofstream &log)
    : ganymede_command(command), ganymede_command_size(size), log_stream(log),
...
void Command::print_x86(ostream &str) const {
  if (x86_codes == NULL)
    ...
    for (unsigned i = 0; i < x86_length; ++i) {
      if (i % 5 == 0)
        str << "\n";
