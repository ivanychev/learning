...
/**
		asm_is_mrk							function figures out whether the word is "func" or mark, and calls asm_mark, depending on result

		@param[out]		strin				input file stream
		@param[out]		strerr				error file stream
		@param[out]		word				analysing word
		@param[out]		cur_code			current memory pointer
		@param[out]		pts					the array of memory pointers
		@param[out]		(*error_catcher)	error catcher function

		@return								ASM_NOT_MARK
											ASM_ERROR_CHECKER_BAD or ASM_ERROR_CHECKER_OK, depending of (*error_catcher)
**/
int asm_is_mrk(FILE* strin, FILE* strerr, char* word, int* cur_code, pointer* pts, int (*error_catcher)(FILE* strerr, int cond));
/**
		asm_error_catcher					function analyses error code, prints the message, if there's mistake
		@param[out]		strerr				error file stream
		@param			cond				error code

		@return								ASM_ERROR_CHECKER_BAD or ASM_ERROR_CHECKER_OK, depending of is there mistake or not

**/
int asm_error_catcher(FILE* strerr, int cond);
/**
		asm_cmd_jump_like					this function deals with functions, that may jump to the mark

		@param[out]		strin				input file stream
		@param[out]		codes				the memory array
		@param[out]		cur_code			current memory pointer
		@param			arg					variable, that brings constant of function
		@param[out]		pts					the array of memory pointers

		@return								ASM_BAD_CUR_CODE_POINTER - the pointer of cur_code is broken
											ASM_STRIN_BAD			 - strin pointer is NULL
...
**/
int asm_cmd_jump_like(FILE* strin, double* codes, int* cur_code, const int arg, pointer* pts);
/**

		asm_cmd_mov							function deals with mov instruction
...
/**
		asm_cmd_out							function deals with out instruction
...
/**
		asm_cmd_pop							function deals with pop instruction
...
/**
		asm_cmd_in							function deals with in instruction
...
/**
		asm_str_convert						function converts the word as a stream (defined by constants in header) into constant

		@param[out]		word				analysing word
		@param[out]		reg					variable that will contain the converted value
...
/**
		asm_cmd_push						function deals with push instruction
...
/**
		...
		asm_push_code						function saves the instruction to memory and increases the pointer
...
/**
		asm_cmd_convert						function converts the word with command into constant value from table

		@param[out]		word				the word to be converted
		@param[out]		c					the variable where the value would be saved

		@return								ASM_BAD_COMMAND_TOKEN			- the command is invalid
											ASM_OK							- ok!
**/
int asm_cmd_convert(const char* word, int* c);
/**
		asm_mark							the function that initializes new mark

		@param[out]		word				analysing word
		@param			cur_code			current memory pointer
		@param[out]		pts					the array of pointers

		@return								ASM_MARK_NOT_MARK		- invalid mark
...
int error_asm = 0;  //!< Global variable created for better error testing in SEC_RUN_

#define SEC_RUN_(func)										\
	if ((error_asm = (func)) != ASM_OK) return error_asm;
...
int asm_mark(char* word, int cur_code, pointer* pts)
{
	if (word[0] != ':') return ASM_MARK_NOT_MARK;
...
int asm_push_code(double* codes, int* cur_code, double value)
{
	...
	if ((0 > *cur_code)||(*cur_code >= MAXCODES - 1)) return ASM_CUR_CODE_NUM_OUT_OF_RANGE;
...
int asm_cmd_push(FILE* strin, var* vars,  double* codes, int* cur_code)
{
	...
	int cond = fscanf(strin, "%s", push_word /*, CMD_MAXLEN*/);
	...
	if (ret != ASM_OK) return ASM_BAD_PUSH_STR_TOKEN;

	if (reg == STR_var)
	{
		char push_word_var[CMD_MAXLEN] = {};
		int cond = fscanf(strin, "%s", push_word_var/*, CMD_MAXLEN*/);
		if (cond <= 0) return ASM_SCAN_FAILED;
		ret = search_word(vars, push_word_var);
...
int asm_str_convert(char* word, double* reg)
{
	...
	double ret =		CONVERT_REG_(word, ax)
						CONVERT_REG_(word, bx)
						CONVERT_REG_(word, cx)
						CONVERT_REG_(word, dx)
						CONVERT_REG_(word, st)
						CONVERT_REG_(word, out)
						CONVERT_REG_(word, ch)
						CONVERT_REG_(word, var)
						STR_NON;
	if (ret == STR_NON) return ASM_BAD_STR_TOKEN;
	*reg = ret;
	return ASM_OK;
}
...
int asm_cmd_in(FILE* strin, double* codes, int* cur_code)
{
	...
	int cond = fscanf(strin, "%s", in_word/*, CMD_MAXLEN*/);
	if (cond <= 0) return ASM_SCAN_FAILED;
...
int asm_cmd_pop(FILE* strin, var* vars, double* codes, int* cur_code)
{
	...
	int cond = fscanf(strin, "%s", pop_word/*, CMD_MAXLEN*/);
	if (cond <= 0) return ASM_SCAN_FAILED;
	...
	if (reg == STR_var)
	{
		...
		int cond = fscanf(strin, "%s", pop_word_var/*, CMD_MAXLEN*/);
		if (cond <= 0) return ASM_SCAN_FAILED;
...
int asm_cmd_out(FILE* strin, double* codes, int* cur_code)
{
	...
	int cond = fscanf(strin, "%s", out_word/*, CMD_MAXLEN*/);
	if (cond <= 0) return ASM_SCAN_FAILED;

	double reg = 0;
	int ret = asm_str_convert(_strlwr(out_word), &reg);

	if (ret != ASM_OK) return ASM_BAD_OUT_STR_TOKEN;

	if (reg == STR_ch)
	{
		int out_char = -1;
		cond = fscanf(strin, "%d", &out_char);
		if (cond <= 0) return ASM_OUT_BAD_CHAR_ARGUMENT;
...
int asm_cmd_mov(FILE* strin, double* codes, int* cur_code)
{
	...
	int cond = fscanf(strin, "%s", mov_word1/*, CMD_MAXLEN*/);
	if (cond <= 0) return ASM_SCAN_FAILED;

	cond = fscanf(strin, "%s", mov_word2/*, CMD_MAXLEN*/);
	if (cond <= 0) return ASM_SCAN_FAILED;

	double mov_arg1 = 0;

	int ret = asm_str_convert(mov_word1, &mov_arg1);
	if (ret != ASM_OK) return ASM_BAD_MOV_TOKEN_1;
	...
	else
	{
		...
		if (ret != ASM_OK) return ASM_BAD_MOV_TOKEN_2;
...
int asm_cmd_jump_like(FILE* strin, double* codes, int* cur_code, const int arg, pointer* pts)
{
	...
	int cond = fscanf(strin, "%s", jump_mark/*, CMD_MAXLEN*/);

	if (cond <= 0) return ASM_SCAN_FAILED;
	if (jump_mark[0] != ':') return ASM_JUMP_NOT_MARK;
	...
	if (i != NUM_OF_POINTERS)
	{
		if (arg == CMD_FUNC) cur_code--;
		SEC_RUN_(asm_push_code(codes, cur_code, pts[i].mark));
		return ASM_OK;
	}
	i = 0;
	while ((pts[i].name[0] != 0) && (i < NUM_OF_POINTERS)) i++;
	if (i == NUM_OF_POINTERS) return ASM_NO_FREE_MARKS;
...
int asm_is_mrk(FILE* strin, FILE* strerr, char* word, int* cur_code, pointer* pts, int (*error_catcher)(FILE* strerr, int cond))
{
	...
	if (strcmp(_strlwr(word), "func") == 0) is_func = true;
	...
	if ((word[0] != ':')&&(is_func == true)) return (*error_catcher)(strerr, ASM_FUNC_BAD_MARK);
	if (word[0] != ':') return ASM_NOT_MARK;
...
int search_word(var* vars, char* word)
{
	...
	for (int i = 0; i < MAXVARS; ++i)
		if (strcmp(word, (vars[i]).name) == 0) return i;
	return -1;
}
...
int asm_cmd_def(FILE* strin, var* vars, double* codes, int* cur_code)
{
	...
	char var_word[CMD_MAXLEN] = {};

	int cond = fscanf(strin, "%s", var_word/*, CMD_MAXLEN*/);
	if (strlen(var_word) >= MAX_VAR_NAME) return ASM_TOO_LONG_VAR;

	if (cond <= 0) return ASM_SCAN_FAILED;

	//!!!!!!if (search_word(vars, var_word) != -1) return ASM_VAR_ALREADY_DEFINED;
	int ptr = search_last_free(vars);
	if (ptr == -1) return ASM_TOO_MUCH_VARS;

	strcpy((vars[ptr]).name, var_word);

	SEC_RUN_(asm_push_code(codes, cur_code, (double)ptr));
	return ASM_OK;
}
