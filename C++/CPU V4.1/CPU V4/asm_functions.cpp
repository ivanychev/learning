...
int asm_cmd_convert(const char *word, int *c) {
  *c = CHECK_COMMAND(word, PUSH) CHECK_COMMAND(word, POP) CHECK_COMMAND(
      ...
      CHECK_COMMAND(word, DIV) CHECK_COMMAND(word, SIN) CHECK_COMMAND(
          word, COS) CHECK_COMMAND(word, TAN) CHECK_COMMAND(word, SQRT)
          CHECK_COMMAND(word, POW) CHECK_COMMAND(word, END) CHECK_COMMAND(
              word, DUMP) CHECK_COMMAND(word, ENTER) CHECK_COMMAND(word, TAB)
              CHECK_COMMAND(word, SPACE) CHECK_COMMAND(word, OUT) CHECK_COMMAND(
                  word, MOV) CHECK_COMMAND(word, SWAP) CHECK_COMMAND(word, DUB)
                  CHECK_COMMAND(word, JMP) CHECK_COMMAND(word, JBE)
                      CHECK_COMMAND(word, JB) CHECK_COMMAND(word, JAE)
                          CHECK_COMMAND(word, JA) CHECK_COMMAND(word, JE)
                              CHECK_COMMAND(word, JNE) CHECK_COMMAND(word, FUNC)
                                  CHECK_COMMAND(word, CALL) CHECK_COMMAND(
                                      word, RET) CHECK_COMMAND(word, IN)
...
int asm_str_convert(char *word, double *reg) {
  ...
  double ret =
      CONVERT_REG_(word, ax) CONVERT_REG_(word, bx) CONVERT_REG_(word, cx)
          CONVERT_REG_(word, dx) CONVERT_REG_(word, st) CONVERT_REG_(word, out)
              CONVERT_REG_(word, ch) CONVERT_REG_(word, var) STR_NON;
  if (ret == STR_NON)
...
int asm_cmd_def(FILE *strin, var *vars, double *codes, int *cur_code) {
  ...
  if (strlen(var_word) >= MAX_VAR_NAME)
    return ASM_TOO_LONG_VAR;
