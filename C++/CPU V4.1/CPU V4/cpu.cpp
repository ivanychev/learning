...
int cpu_check(cpu* my_cpu)
{
	...
	if	(											  \
	(my_cpu -> cpu_stack -> ok() == OK)			&&\
	(my_cpu -> func_stack -> ok() == OK)			&&\
...
#define REG_PUSH_(name)														\
	STR_##name:																\
		my_cpu -> cpu_stack -> push(my_cpu -> name);						\
		return CPU_OK;

int cpu_push(cpu* my_cpu, int push_arg1, double push_arg2)
{
	if (my_cpu -> cpu_stack -> full() != 0) return CPU_ERROR_STACK_FULL;
...
int cpu_pop(cpu* my_cpu, int pop_arg1, int pop_arg2, double* return_value)
{
	...
	if (my_cpu -> cpu_stack -> empty() == 1) return CPU_ERROR_STACK_EMPTY;
...
{
	if (cpu_check(my_cpu) != CPU_CHECK_OK) return CPU_BROKEN_START;
	if (my_cpu -> cpu_stack -> empty())  return CPU_NOT_ENOUGH_ARGS;
...
{
	if (cpu_check(my_cpu) != CPU_CHECK_OK) return CPU_BROKEN_START;
	if ((my_cpu -> cpu_stack -> _size) < 2) return CPU_NOT_ENOUGH_ARGS;
	double val1 = 0;
	double val2 = 0;
