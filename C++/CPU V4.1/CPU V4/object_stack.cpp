...
{
	if (this == NULL) return;								
	for (int i = 0; i < _maxsize; ++i) _data[i] = POISON;
	free(_data);
	_data = NULL;
	_size = 0;
}
...
int stack::print()
{
	...
	for (int i = _size - 1; i >= 0; --i) 
	{
		...
		if (fprintf(stdout, "%lg", _data[i]) < 0) return ERR_PRINT;
		putchar('\n');
	}
		return OK;
}
...
int stack::dump(FILE* stream)
{
...

	{
		...
		if (fprintf(stream, "%c %lg \n",  ((_size > i)&&(i >= 0)) ? '*' : ' ', _data[i]) < 0 ) return ERR_PRINT;
