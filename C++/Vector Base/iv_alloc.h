...
struct ivAllocTable
{
    struct ptr_meta
        ...
        size_t                       size;
                                     ...
                                     ptr_meta(void* pointer, size_t alloc_size): ptr(pointer), size(alloc_size), deleted(false) {};
                                    ~ptr_meta()   {ptr = NULL; deleted = false; size = 0;};
        };

                            ivAllocTable()  {};
    vector<ptr_meta>        table;
    void                    sorted()    {
                                        sort (table.begin(), table.end(), [](ptr_meta x, ptr_meta y) {return (uintptr_t)(x.ptr) < (uintptr_t)(y.ptr); });
                                        };
...
int ivAllocTable::search(void* searched)
{
    int i = 0;
    int tsiz = table.size();
    while (((uintptr_t)table[i].ptr < (uintptr_t) searched)
            &&
            (i < tsiz))  ++i;
    if (i == tsiz)                                          return -1;
    if ((uintptr_t)table[i].ptr == (uintptr_t) searched)    return  i;
                                                            return -1;
}
...

{
    ...
    table.push_back(ptr_meta(new_ptr, new_size)); sorted();
}
...
void operator delete (void* ptr) noexcept
{
    ...
    if (found == -1)
    {
        fprintf("Attmept to free memory, that hasn't been allocated.\nPointer:\t%08x\n Terminating...", ptr);
        getchar();
        abort();
    }
    if (AllocTable.table[found].deleted == true)
    {
        fprintf("Attmept to free memory, that has already been freed.\nPointer:\t%08x\n Terminating...", ptr);
        getchar();
        abort();
    }
