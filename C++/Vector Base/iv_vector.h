...
*           @file               iv_vector.h
*
*           @par                IV_VECBASE is vector template-based object with potential of creating
*                               inheritors. Thanks God, it would be easy with IV_VECBASE. There are only
*                               essential methods are implemented, but they are enough to base some inheritors
*                               on it.
**/
...
const char*  CONST_ERROR =  "Vecbase: Error while constructing objects in constructor";
...
_size(0),
_alloc_size(VECBASE_STANDARD_BLOCK_SIZE),
...
iv_vecbase<arg_type>::iv_vecbase  (const iv_vecbase& copy_vector) throw(ivException):
...
template <typename arg_type>
iv_vecbase<arg_type>::iv_vecbase  (
                                    iv_vecbase<arg_type>& copy_vector,
                                    const unsigned  from_inclusive,
                                    const unsigned  to_exclusive
                                  )
                                  throw(ivException):
...
_alloc_size(to_exclusive - from_inclusive)
{
   ...
   LG_ASSERT(from_inclusive <= to_exclusive, "Vecbase: FROM index must not be higher than TO");
...
template <typename arg_type>
bool iv_vecbase<arg_type>::ok()  const
{
    if (this == NULL)                                       return false;
    if (_alloc_size < _size)                                return false;
    if (_data == NULL)                                      return false;
    for (int i = 0; i < _size; ++i) if (_data[i] == NULL)   return false;
                                                            return true;
}

template <typename arg_type>
char* iv_vecbase<arg_type>::detailed_ok()    const
{
    if (this == NULL)                                       return "Vecbase detailed check: THIS pointer is NULL";
    if (_alloc_size < _size)                                return "Vecbase detailed check: allocated_size if below size of vector";
    if (_data == NULL)                                      return "Vecbase detailed check: pointer of pointer array is NULL";
    for (int i = 0; i < _size; ++i) if (_data[i] == NULL)   return "Vecbase detailed check: one of pointers is NULL";
                                                            return "Vecbase detailed check: OK";
}
...
void iv_vecbase<arg_type>::dump (FILE* stream)  const
{
    ...
    for (int i = 0; i < _alloc_size; ++i) fprintf(stream, "%p\n", _data[i]);
...
template <typename arg_type>
unsigned iv_vecbase<arg_type>::size () const
{
    CHECK_THIS;
    return _size;
}
...
void iv_vecbase<arg_type>::set_alloc_size (unsigned new_size)
{
   ...
   //                         ));
    VEC_DUMP(this);
    if (new_size == _alloc_size) return;
    if (new_size <  _alloc_size)
    {
        CALL(cut(new_size, _size), "Vecbase: cutting failed");
        _data = (arg_type**)realloc (
                                        (void*)_data,
                                         sizeof(arg_type*) * new_size
                                    );
...
//      HERE IF NEW_SIZE > SIZE
    ...
    _size = new_size;
    CALL(this -> memory_opimize(), "Vecbase: memory optimisation failed");      // what the hell I had paste "this ->" ??
...
template <typename arg_type>
void iv_vecbase<arg_type>::memory_optimize()
{
    VEC_DUMP(this);
    detailed_ok();
    CHECK_THIS;
    ...
    if (empty_blocks != 0)
    {
        CALL (set_alloc_size(_alloc_size - empty_blocks * _block_size), "Vecbase: setting new allocation size failed");
        changed = true;
    }
...
void iv_vecbase<arg_type>::push_back(const arg_type& added)
{
    ...
    if (ok() == false) throw (ivRuntime_error(__FILE__, __FUNCSIG__, __LINE__, "Vector verification failed"));           // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1

    VEC_DUMP(this);
    if (_size == _alloc_size)
    {
        CALL(set_alloc_size(_size + _block_size), "Vecbase: extanding memory failed");
    }
    VEC_DUMP(this);
...
template <typename arg_type>
arg_type iv_vecbase<arg_type>::pop_back()
{
    CHECK_THIS;
...
template <typename arg_type>
void iv_vecbase<arg_type>::_pull_apart  (
                                        unsigned was,
                                        unsigned became
                                        )
{
    ...
    if (_alloc_size < _size + extend) set_alloc_size(_size + extend);
    VEC_DUMP(this);
                        ...
                        )
{
    ...
    CHECK_THIS;
    VEC_DUMP(this);
    CALL(   _pull_apart(   position, position + 1),
            "Vecbase: pulling apart failed"
        );
    VEC_DUMP(this);
...
template <typename arg_type>
void iv_vecbase<arg_type>::insert   (
                                        iv_vecbase& vec,
                                        const unsigned position
                                    )
{
    ...
    CALL(
            _pull_apart(   position, new_position),
            "Vecbase: pulling apart failed"
        );
    ...
    {
        pasting.dump(stdout);
        CALL (_data[i] = new arg_type(pasting[i - position]), "Vecbase: copying failed");
    }
    _size += pasting.size();
                        ...
                        )
{
    ...
    CALL(   _pull_apart(   position, new_position),
            "Vecbase: pulling apart failed"
        );
     for (int i = position; i < new_position; ++i)
    {
