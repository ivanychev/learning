...
select top 1 DEPARTMENT.department_id, DEPARTMENT.name, sum(SALES_ORDER.total)/count(distinct EMPLOYEE.employee_id)
...
order by sum(SALES_ORDER.total)/count(distinct EMPLOYEE.employee_id) desc;
