...
void* f_in(void* data)
{
    ...
    if (time - t_now[k] > 0)
        printf("Полоса %d для взлета: запрошенное время : %d, время ожидания: %d мин, вылет в %d  \n",k+1, time,  time-t_now[k], max(t_now[k], time));
    else printf("Полоса %d для взлета: запрошенное время %d, время задержки: %d мин, вылет в %d \n",k+1, time, t_now[k] - time, max(t_now[k], time));
...
void* f_out(void* data)
{
    ...
    if (time - t_now[k] > 0)
        printf("Полоса %d для посадки: запрошенное время %d, время ожидания: %d мин, вылет в %d  \n",k+1, time,  time-t_now[k], max(t_now[k], time));
    else printf("Полоса %d для посадки: запрошенное время %d, время задержки: %d мин, вылет в %d \n",k+1, time,  t_now[k] - time, max(t_now[k], time));
    t_now[k] = max(t_now[k], time) + 3;
    return NULL;
...

{
    ...
    printf(    "Please type data in format\n"
               ...
               "First %d cases will be analysed", PLANE_CASES);
