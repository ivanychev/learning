...
/**
    ...
    @par            Checking of pointers is working. If x = y, there's nothing to swap. If (x == NULL) or (y == NULL) - everything's bad
...
/**
    get_word:               scans the input for the word, saves it into char array, counts its length
...
/**
    ...
    @return                 1 if the piece has been printed. If ASSERT is switched, the program wold be terminated
**/
int print_word(char *word, int start, int finish);

/**
    main -          functions, which filters input into output recognizing the words.
    It uses other functions to invert them and to print.
...
int main()
{
    OUT printf( "# Hello, I'm the program, which solves the problem which's written below\n"
                ...
                "# I'm filtering and printing your input in the way of inverting all of your words"
                "# For example, if you write \"Tear down the wall!\" it'll print \"raeT nwod eht llaw!\"\n"
                "# Don't forget to type the character of the end of the file after the last ENTER! It's Ctrl-Z on Windows machines\n"
                "# Have fun!\n", __FILE__, __TIME__);
...
{
    ASSERT(word);
    ASSERT(0 <= from); ASSERT(from <= to); ASSERT(to < siz);

    int i = from;
    int length = to - from;
    for (i = 0; i <= length / 2; ++i) swap_chars(word + i, word + (length - i));
...
int print_word(char *word, int start, int finish)
{
    ...
    for (i = start; i <= finish; ++i) putchar(word[i]);
    return 1;
}
